...
/* 目标绝不允许等于当前类型或非法值：desc 侧写丢失/脏值时在组内另选 */
if(key&&p[key]!=null&&pool.indexOf(p[key])>=0&&p[key]!==cur)return p[key];
var cands=pool.filter(function(t){return t!==cur;});
if(!cands.length)return cur;
/* 确定性选取（hint/apply 两次调用结果一致），代替 Math.random */
var idx=(cur*7+(p["age"]||0)*13+(p["ovr"]||0)*3)%cands.length;
if(!(idx>=0))idx=0;
return cands[idx];
}
...

{
  ...
  'weight': 0x2d,
  'cn': !0x0,
  'desc': "比赛刚结束，两边球迷在停车场动起手来。你的车，正好堵在最中间。",
...
return!p["inChina"]&&p["leagueRe"+'p']>=0x4&&p["hasCont"]&&p["roleRank"]>=0x3;
  ...
  'desc': "大赛当前，全世界的镜头都对准了你所在的这支球队。赛前新闻发布会上，记者问了一个尖锐的问题——你接过话筒，全场安静。",
  'options': [
    {
        'label': "把这场比赛当成自己"+"的决赛",
        'p': function(p){
return f(0.34,[[p["ovr"],0x4e,0.011],[p["fame"],0x2d,0.003]],0.1,0.72);
        ...
        'hint': function(p,q){
return g(q,"你成了全场的"+'主角',"被打成了背景板");
},
        'apply': function(p,q,s){
return d(q,s)?{'ovr':0x3,'fame':0x22,'roleDelta':0x1,'mult':{'cont':1.5},'text':h(p,{'gk':"你全场扑了九次"+"，终场前那个单刀"+"你用腿封住了，保"+"住了平局。",'def':"你全场一次没让对"+"方过掉，终场前那"+"次回追解围保住了"+"平局。",'other':"你一球一助攻，终"+"场前那次关键传球"+"保住了平局。"})+("赛后你的名字出现在"+"所有体育头条，国"+"内的热搜挂了整整"+"一天。")}:{'ovr':-0x2,
'fame':0xa,'text':"全场结束，对手进了四个球。"+h(p,{'gk':"第四个球进的时候"+"你还坐在门线上没"+"起来，镜头给了你"+"很久，",'other':"你被换下的时候镜"+"头给了你很久，"})+("第二天那张脸出现"+"在所有的赛后总结"+'里。')};
}
    },
    {
        'label': "照平时那样踢",
        'hint': "不出彩，也不难看",
        'apply': function(){
return{'fame':0x8,'guanxi':0x6,'text':"你守住了自己那块，球队输了一个球。回到更衣室，没有人指责谁。教练说：下一次，我们还会来。"};
}
    }
  ]
},
...
return p["caps"]>=0xa;
  ...
  'desc': "大赛当前，压力从四面八方涌来——赛前所有人只说一句「至少全力以赴」。",
  'options': [
    {
        'label': "全力以赴拼一场",
...
{'id':"tier_main_ceiling",'title':"天花板之问",'icon':'🧱','weight':0x3e,'when':function(p){return p["ovr"]>=0x50&&p["ovr"]<0x5a;},



'desc':"专栏文章把你排进「联赛十大令人失望球员」候选，理由是：天赋肉眼可见，进步却停在了两年前。第二天教练把你叫进办公室，桌上放着你的体测报告。",'options':[{'label':"直面瓶颈，加练突破",'p':function(p){return f(0.55,[[p["talent"],1,0.3]],0.25,0.85);},'hint':function(p,q){return g(q,"捅破窗户纸","原地踏步");},'apply':function(p,q,s){return d(q,s)?{'ovr':0x3,'fame':0x4,'text':"你和教练一起重看了两年的比赛录像，找到了那个被忽略的坏习惯。改掉它的第四场，你打进赛季最佳进球。那篇专栏作者发了条道歉动态。"}:{'ovr':-0x1,'text':"你加练了，很苦，但赛季结束数据几乎没变。有些墙撞不开，可能真的需要换个姿势——或者换堵墙。"};}},{'label':"接受现在的自己",'hint':"心态平稳",'apply':function(){return{'guanxi':0x5,'text':"你对教练说：我知道自己几斤几两，把我放在合适的位置，我能踢得很稳。之后的十场比赛你零失误。稳定，也是一种没人夸但人人需要的品质。"};}}]},

{'id':"tier_rot_wheel",'title':"轮换的齿轮",'icon':'🔄','weight':0x42,'when':function(p){return p["ovr"]>=0x46&&p["ovr"]<0x50;},
...
{'id':"tier_rot_loan",'title':"租借名单上的名字",'icon':'📦','weight':0x40,'when':function(p){return p["ovr"]>=0x46&&p["ovr"]<0x50&&p["clubRep"]>=0x3;},
...
return p["_friend"]===0x1&&p["age"]>=0x12;
  ...
  'desc': "当年青训宿舍上下铺的那个他，如今真的踢出了名堂——一份来自另一座城市的职业合同。临走前一晚，他请你在老地方吃了顿饭。",
  'options': [
    {
      'label': "保持联系",
      'hint': "关系+4",
...

{
  ...
  'desc': "俱乐部又砸下一名大牌外援，转会费是你身价的十倍。亮相仪式上，他穿着你最想要的那个号码。更衣室里，教练拍拍你：「位置是抢来的，不是让来的。」",
  'options': [
    {
        'label': "加练抢回位置",
      'p': function(p){return f(0.52,[[p["talent"],1,0.25],[p["ovr"],0x46,0.006]],0.22,0.83);},
        'hint': function(p,q){return g(q,"把外援按上替补席","沦为他的替补");},
        'apply': function(p,q,s){return d(q,s)?{'ovr':0x2,'fame':0x6,'text':"赛季前六轮你场场首发，进球比他还多。看台上开始有人喊你的名字，而不是那个天价外援的。教练在发布会上说：位置，是靠表现说话的。"}:{'roleDelta':-0x1,'fame':-0x3,'text':"他确实比你强一档。你被挤到替补席，只能在垃圾时间上场。经纪人劝你：换个能踢上球的地方吧。"};}
    },
    {
        'label': "虚心学他的跑位",
      'apply': function(){return{'ovr':0x1,'guanxi':0x4,'text':"你主动找他加练，学他的无球跑动和临门一脚。他话不多，但每次都多留你二十分钟。半个赛季后，你发现自己的射门变了。"};}
    }
  ]
},
