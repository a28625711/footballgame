...
- **联赛金靴**（`by["name"]+"金靴"`，如「英超金靴」）：att 且 `bz.apps≥19` 且**联赛进球** `bz.lgGoals ≥ 联赛门槛`。门槛由该联赛最强队的联赛进球推出：`round(maxTeamGf × (0.26+0.08×rand))`（`_leagueTopTarget`，每季每联赛缓存一次）。命中即 **100% 获奖**，不再掷骰、不再用全赛事进球（2026-09 真实化）。
