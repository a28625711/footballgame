...
- **联赛金靴**（`by["name"]+"金靴"`，如「英超金靴」）：att 且进球数 ≥ 门槛（按联赛 rep：`[15,18,20,24,27,30][rep]`，中超20/英超30），概率 0.45。取代原「中超金靴」限定。
- **联赛最佳球员**（`by["name"]+"最佳球员"`，如「中超最佳球员」）：ovr≥74 且本赛季夺联赛冠军概率 0.35、国内杯赛冠军 0.08（叠加），再 ×(1+0.2×本季奖杯数)。
- **欧洲金靴联动**：欧洲金靴（boot）命中时，若本季未拿联赛金靴则补发（`b5(by.name+"金靴")`）；补发函数须返回 true，否则会短路 `&&b5(boot)` 导致欧洲金靴永不发放（已修）。
- cheat 分支同步提供上述两奖（联赛金靴 0.45、联赛最佳 0.35）。
