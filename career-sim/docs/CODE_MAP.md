...
- **联赛金靴**（`by["name"]+"金靴"`，如「英超金靴」）：`bz.apps≥19` 且**联赛进球** `bz.lgGoals ≥ 联赛门槛`。门槛由该联赛最强队的联赛进球推出：`round(maxTeamGf × (0.26+0.08×rand))`（`_leagueTopTarget`，每季每联赛缓存一次）。命中即 **100% 获奖**，不再掷骰、不再用全赛事进球（2026-09 真实化）。**2026-09 起不再限制位置**（旧版硬门槛 `posGroup==='att'` 只有边锋/中锋能拿；现实中前腰/影锋等中场同样能拿，改为按进球数决定；后卫/门将因进球模型天然达不到门槛）。
...
- **欧洲金靴联动**：欧洲金靴（boot）门槛为五大联赛（epl/liga/seri/bund/l1）门槛中的最高值（`_euroTopTarget`），要求 rep≥4 且 `bz.lgGoals ≥ 该值`；命中时若本季未拿联赛金靴则补发（`b5(by.name+"金靴")`）。补发函数须返回 true，否则会短路 `&&b5(boot)` 导致欧洲金靴永不发放（已修）。
- cheat 分支同步提供上述两奖（同样按联赛进球 ≥ 门槛）。
- `bq()` 统计 `poyCount`（awards 含"最佳球员"）/`topCount`（含"金靴"），供新成就 `poymaster`/`topmaster` 判定（各 3 次，无加成）。
- 旧 `AWARDS.cslmvp/cslboot` 条目保留但不再被引用。
- 数据重平衡（2026-09）：`_pMatchContrib` 加 88+ 世界级加成 `1.10+0.03×(ovr-88)`；`TYPE_MODS` 射手 1.40→1.50、支点 1.20→1.28、B2B 1.05→0.78、铁腰 0.45→0.35；`_pSeasonContrib` 分联赛/杯赛/洲际统计，赛季记录新增 `lgGoals/lgAssists`。
- **进球模型位置份额重标定（2026-09）**：`_pMatchContrib` 份额表键名原为 `'fwd'`，但位置分组是 `'att'/'mid'/'def'/'gk'`——前锋匹配不到、落到默认值，**与中场同份额**，位置完全不影响进球（只有类型影响），导致影锋中场（×1.70）进球反超中锋（×1.50）。修正为按位置组取份额，并重标 `sh`/`TYPE_MODS`：
  - `sh`（位置基础份额，进球/助攻）：`att [0.24,0.11]`、`mid [0.15,0.19]`、`def [0.07,0.045]`、`gk [0,0]`。
  - `TYPE_MODS`（g,a）：射手 1.30/0.60、组织 0.85/1.45、全能 1.10/1.10、速度型 1.08/0.95、支点 1.18/0.70、影锋 1.45/0.80、B2B 0.95/1.05、铁腰 0.40/0.70、边后卫 1.00/1.60、自由人 0.90/1.15、铁卫 0.55/0.50、门将 1.00/1.00。
  - 公式：`P(进球)=sh[位置组].g × f(ovr) × TYPE_MODS[type].g`（每粒球队进球，含球队进球数/场次/对手强度 diff）。标定后位置分层 att>mid>def 成立（同类型 ovr90：中锋 27.5 > 前腰 16.6 > 中卫 7.9 球），中锋 32、影锋中场 23、后卫助攻回到 5 量级。回归 `test_stats_balance`（新增位置分层断言）、`test_boot_position`。
- 联赛赛制（2026-09）：`_lgRR` 表，默认双循环；mls/arg/mx 单循环、cpl 4 循环、kl 3 循环。
...
### 存档 / 续玩（2026-09 修复）
- 主档 `gyrs_save`（`aA()` 写，剥离 WH_KEYS）+ 世界历史 `gyrs_world`（`_whWrite()`，超限自动裁剪最老赛季）；加载 `aI()` 校验 `ver===SAVE_VER` 并把 world 合并回主档。存档在 `nextStep`(`as()`)、选择(`bQ`)、继续(`bR`)、提交事件(`bP`) 后写入——**青训期同样写档**（`bj()` 每年推进后产生 pending，解决即存）。
- **青训期存档续玩修复**：旧版续玩条件 `seasons.length>0` 把青训存档（`phase='youth'`、`seasons` 为空）排除——刷新既不自动续玩、点「开始」也不提示，等于作废。现改为 `seasons.length>0 || phase==='youth'`，两处：DOMContentLoaded 自动续玩、开始按钮续玩提示。回归 `tests/test_save_resume.py`（注入青训存档并触发 DOMContentLoaded→断言续玩且能跑完）。
- 注：DOMContentLoaded 回调在测试 DOM mock 下不会自动触发，需手动执行 `window.__DOCEV['DOMContentLoaded']`（见 `test_save_resume.py`）。

### 各级决赛回填与世界面板签表（2026-09 修复）
- **洲际决赛「让位」AI 结算**（`_contAiSettle`，被更高优先级大场面驱逐时）：必须 `delete _tie["pd"]`。`pd:1`=待定，世界面板 `_wlBracket` 见 `pd` 即渲染 `vs`/`待定`；只写 `sa/sb/w/champion` 不清 `pd` 会让签表永远显示"未开始"（`a5e306d` 当时只修了奖杯记错赛季）。交互路径（`_bmFinish` 的 `a2["_contRun"]` 块）同样回填且已清 `pd`。
...
- **2026-09 国家队数据统一 + 真实赛程（sim.js b2 国家队块）**：`caps` 不再等于小组全部赛程数——只取中国队的真实场次（资格赛 `_chnGroup` 过滤 + 正赛小组 + 淘汰赛 `_chnRounds` + 友谊赛），出场数按角色出勤率抽样；进球/助攻/零封统一走 `_pMatchContrib`（对手强度 `_natStrOf`、自身 60、`natB` boost），与俱乐部 `_pSeasonContrib` 同法。现实赛程：非大赛年 6-8 场友谊赛、大赛年 资格赛+正赛+2-3 场友谊赛（`_runFriendlies(min,max)`）。赛季记录新增 `natGames`。
- **2026-09 `newState` origin 解析**：`config.origin` 允许传 id 字符串或对象，字符串用 `bp(id)` 解析（此前只接受对象；测试传字符串会令 ovr/guanxi/money 变 NaN，导致国家队路径长期未被覆盖）。
...
## 球队实力三层模型（2026-09 重标定）

原模型 `_teamAbs = 联赛str + (rep-2)*3 + dev`，`dev` 单层快速均值回归（半衰期 ~4 年、std 1.5）→ 联赛顶部被压缩（EPL 冠军 ~76 分、头尾差 ~51，现实 ~90/~65），且无可见的鼎盛/重建周期。现拆成三个时间尺度（`_devTick`，sim.js:2756 附近）：

- **档位（静态）**：`_repGap` 表 `[-7,-4,0,4.5,9,16]`（原 `(rep-2)*3`，4→5 从 4.5 拉大到 7）——低档保护底部、豪门真正高出一档，提供跨档实力差，零噪声、无正反馈。rep4 数量不变。
...
- 效果（无玩家全 AI 200 季，引擎实测）：EPL 冠军 76→**87**、头尾差 51→**63**（现实 ~65）、连冠 5（现实 4）、distinct 冠军 8；liga/bund/l1 distinct 8-11。
- 探针：`SIM.eraOf/formOf/hangOf/lgChampStreakOf(tid)`。
- 顺带修复：`_bmFinish` 加时块未判联赛德比/保级 → 90 分钟战平仍进加时点球（违反"联赛大场面允许平局"），现跳过加时直接按平局回填。
- 顺带修复：`news.js` 的 `rnd()` 用 `Math.random` → 新闻/事件层非种子化，同种子世界不可复现（`test_league_engine` 确定性失败）；改为优先走 `SIM.rnd`（种子化），同种子世界完全可复现。
