(function(a,b){var d2=_gam_0b,c=a();
while(!![]){try{var d=parseInt("483837AScdNr")/0x1+-parseInt("181822ytwoJf")/0x2+-parseInt("649956OxbBSD")/0x3*(-parseInt("4bixcnH")/0x4)+-parseInt("884245Rycesi")/0x5*(-parseInt("18AQJELH")/0x6)+parseInt("797804OrQJGu")/0x7+parseInt("3019544CzGDql")/0x8+-parseInt("12262959jOGuPV")/0x9;
if(d===b)break;
else c['push'](c['shift']());
}catch(e){c['push'](c['shift']());
}}}(_gam_0a,0x41abd),!(function(){'use strict';
var d3=_gam_0b;
var a0=window["DATA"],a1=window["EVENTS"],a2="gyrs_",a3="足一把-生涯模拟"+'器',a4="2026-08-"+'14',a5="zuyibaga"+"me.com",a6=window["SIM"],
a7=a6["SAVE_VER"],a8=a6["MODES"],a9=a6["ORIGINS"],aa=a6["NAT_SHOR"+'T'],ab=a6["isNear"],ac=a6["isHome"],ad=a6["clamp"],af=(a6["rnd"],
a6["rint"],a6["rpick"],a6["shuffle"],a6["rweight"],a6["hashStr"]),ag=a6["teamById"],ah=(a6["leagueBy"+'Id'],a6["posById"]),
ai=a6["curTeam"],aj=a6["curLeagu"+'e'],ak=(a6["inChina"],a6["originBy"+'Id']),al=a6["fmtMoney"],am=a6["fmtValue"],an=a6["valueOf"],
ao=(a6["snap"],a6["stageOf"],a6["interpol"+"ate"]),ap=(a6["pickRiva"+'l'],a6["pickEven"+'t'],a6["applyRes"+"ult"],a6["growthRa"+"nge"],
a6["computeR"+"ole"],a6["posRates"],a6["starPowe"+'r'],a6["national"+"Odds"],a6["natBest"]),aq=(a6["runPerio"+'d'],a6["addAward"],
a6["simulate"+"OneSeaso"+'n'],a6["doTransf"+'er'],a6["pickOffe"+'rs'],a6["offerOpt"+"ion"],a6["buildPro"+"file"]);
function ar(){var d4=d3;
au&&("summary"===au["phase"]?bO():bN());
}function as(){var d5=d3;
a6["nextStep"](),aA(),ar();
}var au=null,av=null;
function aw(bW){var d6=d3;
return document["getEleme"+"ntById"](bW);
}function ax(bW){var d7=d3;
return String(null==bW?'':bW)["replace"](/&/g,"&amp;")["replace"](/</g,"&lt;")["replace"](/>/g,"&gt;")["replace"](/"/g,"&quot;")["replace"](/'/g,
"&#39;");
}function ay(bW,bX){var d8=d3;
try{var bY=localStorage["getItem"](a2+bW);
return null===bY?bX:JSON["parse"](bY);
}catch(bZ){return bX;
}}function az(bW,bX){var d9=d3;
try{localStorage["setItem"](a2+bW,JSON["stringif"+'y'](bX));
}catch(bY){}}function aA(){var da=d3;
au&&az("save",au);
}function aB(){var db=d3;
return a6["normLega"+'cy'](ay("legacy",null));
}function aC(bW){var dc=d3;
az("legacy",bW);
}function aD(){var dd=d3;
try{localStorage["removeIt"+'em'](a2+"legacy");
}catch(bW){}}function aE(bW){var de=d3;
if(!bW)return null;
if(bW["legacy"])return a6["normLega"+'cy'](bW["legacy"]);
var bX=0x0;
return(bW["honours"]||[])["forEach"](function(bY){var df=de;
bX+=bY["count"]||0x1;
}),a6["normLega"+'cy'](a6["legacyFr"+'om']({'gen':bW["gen"]||0x1,'banned':!!bW["banned"],'maxOvr':bW["maxOvr"]||0x0,'caps':bW["caps"]||0x0,
'trophies':bX,'careerEarnings':bW["earned"]||0x0}));
}var aF=0x3c;
function aG(){var dg=d3,bW=ay("archive",null);
return "[object "+"Array]"===Object["prototyp"+'e']["toString"]["call"](bW)?bW:[];
}function aH(bW){var dh=d3,bX={},bY=0x0,bZ=bW["length"];
bW["forEach"](function(c2){var di=dh;
bp(c2)["forEach"](function(c3){bX[c3]=0x1;
}),c2["maxOvr"]>bY&&(bY=c2["maxOvr"]);
});
var c0=0x0;
for(var c1 in bX)Object["prototyp"+'e']["hasOwnPr"+"operty"]["call"](bX,c1)&&c0++;
return{'lives':bZ,'best':bY,'endings':c0,'total':a0["ENDINGS"]["length"]};
}function aI(){var dj=d3,bW=ay("save",null);
return bW&&bW["ver"]===a7?(bW["natRuns"]||(bW["natRuns"]=[]),null==bW["banGames"]&&(bW["banGames"]=0x0),bW["choices"]||(bW["choices"]=[],bW["rid"]=null),
void 0x0===bW["rid"]&&(bW["rid"]=null),bW):null;
}function aJ(){var dk=d3;
try{localStorage["removeIt"+'em'](a2+"save");
}catch(bW){}}function aK(bW){var dl=d3,bX=String(bW)["replace"]('#','');
return 0x3===bX["length"]&&(bX=bX[0x0]+bX[0x0]+bX[0x1]+bX[0x1]+bX[0x2]+bX[0x2]),(0x12b*parseInt(bX["slice"](0x0,0x2),0x10)+0x24b*parseInt(bX["slice"](0x2,0x4),0x10)+0x72*parseInt(bX["slice"](0x4,0x6),0x10))/0x3e8>0x96;
}function aL(bW,bX){var dm=d3,bY="assets/j"+"ersey/"+bW['id'];
return function(bZ){var dn=dm,c0=aV('jg');
return "<svg cla"+"ss=\"j-fa"+"llback\" "+"viewBox="+"\"0 0 100"+" 100\" ar"+"ia-hidde"+"n=\"true\""+"><defs><"+"linearGr"+"adient i"+"d=\""+c0+("\" x1=\"0\""+" y1=\"0\" "+"x2=\"0\" y"+"2=\"1\"><s"+"top offs"+"et=\"0\" s"+"top-colo"+"r=\"")+bZ['c1']+("\"/><stop"+" offset="+"\"1\" stop"+"-color=\"")+aN(bZ['c1'],
-0.28)+("\"/></lin"+"earGradi"+"ent></de"+"fs><path"+" d=\"M34 "+"20 L44 1"+"5 Q50 21"+" 56 15 L"+"66 20 L8"+"4 29 L77"+" 44 L69 "+"40 L69 8"+"6 L31 86"+" L31 40 "+"L23 44 L"+"16 29 Z\""+" fill=\"u"+"rl(#")+c0+(")\" strok"+"e=\"rgba("+"0,0,0,.3"+"5)\" stro"+"ke-width"+"=\"1.2\" s"+"troke-li"+"nejoin=\""+"round\"/>"+"<path d="+"\"M44 15 "+"Q50 21 5"+"6 15 L54"+" 20 Q50 "+"25 46 20"+" Z\" fill"+"=\"rgba(0"+",0,0,.32"+")\"/></sv"+'g>');
}(bW)+("<picture"+" class=\""+"j-pic\"><"+"source s"+"rcset=\"")+bY+(".webp\" t"+"ype=\"ima"+"ge/webp\""+"><img cl"+"ass=\"j-i"+"mg\" src="+'\x22')+bY+(".png\" al"+"t=\"")+ax(bX)+("\" onerro"+"r=\"this."+"parentNo"+"de.class"+"List.add"+"('gone')"+"\"></pict"+"ure>");
}function aM(bW,bX,bY){var dp=d3,bZ="text-anc"+"hor=\"mid"+"dle\" fil"+"l=\""+(aK(bW['c1'])?"#17171c":"#ffffff")+("\" stroke"+'=\x22')+(aK(bW['c1'])?"#ffffff":"#17171c")+("\" paint-"+"order=\"s"+"troke\" s"+"troke-li"+"nejoin=\""+"round\" f"+"ont-fami"+"ly=\"Inte"+"r, PingF"+"ang SC, "+"Microsof"+"t YaHei,"+" sans-se"+"rif\" fon"+"t-weight"+"=\"800\"");
return "<div cla"+"ss=\"jers"+"ey\">"+aL(bW,'球衣')+("<svg cla"+"ss=\"j-pr"+"int\" vie"+"wBox=\"0 "+"0 100 10"+"0\" aria-"+"hidden=\""+"true\"><t"+"ext x=\"5"+"0\" y=\"31"+'\x22\x20')+bZ+(" font-si"+"ze=\"9.5\""+" stroke-"+"width=\"1"+".9\" lett"+"er-spaci"+"ng=\".5\">")+ax(String(bX||'')["slice"](0x0,0x6)["toUpperC"+"ase"]())+("</text><"+"text x=\""+"50\" y=\"6"+"3\" ")+bZ+(" font-si"+"ze=\"31\" "+"stroke-w"+"idth=\"3."+"6\">")+ax(String(bY))+("</text><"+"/svg></d"+"iv>");
}function aN(bW,bX){var dq=d3,bY=String(bW)["replace"]('#','');
return 0x3===bY["length"]&&(bY=bY[0x0]+bY[0x0]+bY[0x1]+bY[0x1]+bY[0x2]+bY[0x2]),'#'+[parseInt(bY["slice"](0x0,0x2),0x10),parseInt(bY["slice"](0x2,0x4),0x10),
parseInt(bY["slice"](0x4,0x6),0x10)]["map"](function(bZ){var dr=dq,c0=Math["round"](bX<0x0?bZ*(0x1+bX):bZ+(0xff-bZ)*bX);
return('0'+Math["max"](0x0,Math["min"](0xff,c0))["toString"](0x10))["slice"](-0x2);
})["join"]('');
}var aO={'bay':{'s':"circle",'p':"quarters",'c2':"#0066B2",'c3':"#ffffff",'o':"star"},'bar':{'s':"shield",'p':"vstripes",'c2':"#004D98",
'c3':"#EDBB00",'o':"cross"},'rma':{'s':"circle",'p':"solid",'c2':"#00529F",'c3':"#ffffff",'o':"crown"},'juv':{'s':"shield",
'p':"vstripes",'c2':"#ffffff",'c3':"#000000",'o':"star"},'int':{'s':"circle",'p':"solid",'c2':"#000000",'c3':"#C9A227",'o':"ring"},
'acm':{'s':"shield",'p':"halves",'c2':"#000000",'c3':"#ffffff",'o':"none"},'liv':{'s':"shield",'p':"solid",'c2':"#00B2A9",
'c3':"#F6EB61",'o':"ball"},'mun':{'s':"circle",'p':"solid",'c2':"#FBE122",'c3':"#000000",'o':"ball"},'psg':{'s':"shield",'p':"vstripes",
'c2':"#DA291C",'c3':"#ffffff",'o':"star"}},aP=["shield","circle","rounded"],aQ=["solid","vstripes","sash","halves","hoop",
"quarters"],aR=["none","star","ball","ring"],aS={'shield':"M100,10 "+"L178,34 "+"L178,100"+" C178,14"+"6 142,17"+"6 100,19"+"0 C58,17"+"6 22,146"+" 22,100 "+"L22,34 Z",
'circle':"M100,12 "+"A88,88 0"+" 1 1 99."+"9,12 Z",'rounded':"M40,16 H"+"160 A24,"+"24 0 0 1"+" 184,40 "+"V150 A24"+",24 0 0 "+"1 160,17"+"4 H40 A2"+"4,24 0 0"+" 1 16,15"+"0 V40 A2"+"4,24 0 0"+" 1 40,16"+'\x20Z'};
function aT(bW){var ds=d3;
if(!bW)return'';
var bX=window["CREST_UR"+'LS']&&window["CREST_UR"+'LS'][bW['id']];
if(bX){var bY=window["CREST_PL"+"ATE"]&&window["CREST_PL"+"ATE"][bW['id']]?" plate":'',bZ="<img cla"+"ss=\"cres"+'t'+bY+"\" src=\""+ax(bX["replace"](/\.webp$/,".png"))+("\" alt=\"\""+" loading"+"=\"lazy\">");
return/\.webp$/["test"](bX)?"<picture"+" class=\""+"crest-pi"+"c\"><sour"+"ce srcse"+"t=\""+ax(bX)+("\" type=\""+"image/we"+"bp\">")+bZ+("</pictur"+'e>'):"<img cla"+"ss=\"cres"+'t'+bY+"\" src=\""+ax(bX)+("\" alt=\"\""+" loading"+"=\"lazy\">");
}var c0=function(c8){var dt=ds,c9=aO[c8['id']],ca=af(c8['id']);
if(!c9){var cb=a6["leagueOf"+"Team"](c8),cc=cb&&cb['cn'];
c9={'s':cc?"circle":aP[ca%0x3],'p':cc?["solid","hoop","quarters"][ca%0x3]:aQ[(ca>>0x3)%0x6],'c2':aN(c8["color"],-0.5),'c3':aN(c8["color"],0.55),
'o':cc?"ring":aR[(ca>>0x7)%0x4]};
}return c9;
}(bW),c1=aV('cc'),c2=aV('cg'),c3=bW["color"],c4=aN(c3,-0.55),c5=aS[c0['s']]||aS["shield"],c6='';
"vstripes"===c0['p']?c6="<rect x="+"\"58\" y=\""+"0\" width"+"=\"26\" he"+"ight=\"20"+"0\" fill="+'\x22'+c0['c2']+("\"/><rect"+" x=\"116\""+" y=\"0\" w"+"idth=\"26"+"\" height"+"=\"200\" f"+"ill=\"")+c0['c2']+"\"/>":"halves"===c0['p']?c6="<rect x="+"\"100\" y="+"\"0\" widt"+"h=\"100\" "+"height=\""+"200\" fil"+"l=\""+c0['c2']+"\"/>":"hoop"===c0['p']?c6="<rect x="+"\"0\" y=\"7"+"6\" width"+"=\"200\" h"+"eight=\"4"+"8\" fill="+'\x22'+c0['c2']+"\"/>":"sash"===c0['p']?c6="<path d="+"\"M0,148 "+"L200,18 "+"L200,66 "+"L0,196 Z"+"\" fill=\""+c0['c2']+"\"/>":"quarters"===c0['p']&&(c6="<rect x="+"\"0\" y=\"0"+"\" width="+"\"100\" he"+"ight=\"10"+"0\" fill="+'\x22'+c0['c2']+("\"/><rect"+" x=\"100\""+" y=\"100\""+" width=\""+"100\" hei"+"ght=\"100"+"\" fill=\"")+c0['c2']+"\"/>");
var c7='';
return "star"===c0['o']?c7="<path d="+"\"M100,26"+" l7,15 1"+"6,2 -12,"+"12 3,17 "+"-14,-8 -"+"14,8 3,-"+"17 -12,-"+"12 16,-2"+" z\" fill"+'=\x22'+c0['c3']+"\"/>":"ball"===c0['o']?c7="<circle "+"cx=\"100\""+" cy=\"46\""+" r=\"15\" "+"fill=\""+c0['c3']+("\"/><path"+" d=\"M100"+",35 l6,5"+" -2,7 h-"+"8 l-2,-7"+" z\" fill"+'=\x22')+c4+"\"/>":"ring"===c0['o']?c7="<circle "+"cx=\"100\""+" cy=\"100"+"\" r=\"66\""+" fill=\"n"+"one\" str"+"oke=\""+c0['c3']+("\" stroke"+"-width=\""+"7\" opaci"+"ty=\".85\""+'/>'):"cross"===c0['o']?c7="<rect x="+"\"92\" y=\""+"24\" widt"+"h=\"16\" h"+"eight=\"5"+"2\" fill="+'\x22'+c0['c3']+("\"/><rect"+" x=\"74\" "+"y=\"42\" w"+"idth=\"52"+"\" height"+"=\"16\" fi"+"ll=\"")+c0['c3']+"\"/>":"crown"===c0['o']&&(c7="<path d="+"\"M76,42 "+"l8,16 16"+",-20 16,"+"20 8,-16"+" 4,26 h-"+"56 z\" fi"+"ll=\""+c0['c3']+"\"/>"),
"<svg cla"+"ss=\"cres"+"t\" viewB"+"ox=\"0 0 "+"200 200\""+" aria-hi"+"dden=\"tr"+"ue\"><def"+"s><clipP"+"ath id=\""+c1+("\"><path "+"d=\"")+c5+("\"/></cli"+"pPath><l"+"inearGra"+"dient id"+'=\x22')+c2+("\" x1=\"0\""+" y1=\"0\" "+"x2=\".35\""+" y2=\"1\">"+"<stop of"+"fset=\"0\""+" stop-co"+"lor=\"#ff"+"f\" stop-"+"opacity="+"\".2\"/><s"+"top offs"+"et=\"1\" s"+"top-colo"+"r=\"#000\""+" stop-op"+"acity=\"."+"28\"/></l"+"inearGra"+"dient></"+"defs><g "+"clip-pat"+"h=\"url(#")+c1+(")\"><rect"+" x=\"0\" y"+"=\"0\" wid"+"th=\"200\""+" height="+"\"200\" fi"+"ll=\"")+c3+"\"/>"+c6+c7+("<rect x="+"\"0\" y=\"0"+"\" width="+"\"200\" he"+"ight=\"20"+"0\" fill="+"\"url(#")+c2+(")\"/></g>"+"<path d="+'\x22')+c5+("\" fill=\""+"none\" st"+"roke=\"")+c4+("\" stroke"+"-width=\""+"12\"/><te"+"xt x=\"10"+"0\" y=\"14"+"5\" text-"+"anchor=\""+"middle\" "+"fill=\"")+(aK(c3)?"#16160f":"#ffffff")+("\" stroke"+'=\x22')+c4+("\" stroke"+"-width=\""+"5\" paint"+"-order=\""+"stroke\" "+"stroke-l"+"inejoin="+"\"round\" "+"font-fam"+"ily=\"Int"+"er, Ping"+"Fang SC,"+" Microso"+"ft YaHei"+", sans-s"+"erif\" fo"+"nt-size="+"\"58\" fon"+"t-weight"+"=\"800\">")+ax(String(bW["name"]||'')["slice"](0x0,0x2))+("</text><"+"/svg>");
}var aU=0x0;
function aV(bW){return bW+ ++aU;
}var aW={'ST':[0x32,0x9],'LW':[0xf,0xf],'RW':[0x55,0xf],'CAM':[0x32,0x1b],'LM':[0xb,0x28],'CM':[0x32,0x2c],'RM':[0x59,0x28],
'CDM':[0x32,0x3c],'LB':[0xe,0x47],'RB':[0x56,0x47],'CB':[0x32,0x4e],'GK':[0x32,0x5d]},aX=0x0,aY=[{'title':'出身','sub':"决定起步能力、家"+"里的底子，和你认"+"识多少人"},
{'title':'位置','sub':"在场上站哪儿"},{'title':'身份','sub':"名字、号码、位置、惯用脚是"+"随机生成的，也可"+"以自己改；若随机"+"到中国球星会有小"+"加成"},{'title':"心仪球队",'sub':"12 岁那步一定"+"能选到它的青训营"+"；之后够得上它的"+"那一年，报价单上"+"也一定有它"}],
aZ=null;
function b0(){var du=d3,bW=aY[aX];
aw("step-tit"+'le')["textCont"+"ent"]=bW["title"],aw("step-sub")["textCont"+"ent"]=bW["sub"],aw("step-pro"+"gress")["style"]["width"]=(aX+0x1)/aY["length"]*0x64+'%';
var bX='';
if(0x0===aX)bX="<div cla"+"ss=\"pane"+"l\"><div "+"class=\"p"+"ick-list"+'\x22>'+a9["map"](function(c2){var dv=du;
return "<button "+"class=\"p"+"ick"+(av["origin"]['id']===c2['id']?" selecte"+'d':'')+("\" data-o"+"rigin=\"")+c2['id']+'\x22>'+function(c3){var dw=dv;
return "<span cl"+"ass=\"jer"+"sey mini"+'\x22>'+aL(c3,'')+"</span>";
}(c2)+("<span cl"+"ass=\"pic"+"k-main\">"+"<span cl"+"ass=\"pic"+"k-name\">")+ax(c2["name"])+("</span><"+"span cla"+"ss=\"pick"+"-desc\">")+ax(c2["desc"])+("</span><"+"/span></"+"button>");
})["join"]('')+("</div></"+"div>");
else{if(0x1===aX){var bY=ah(av["pos"]);
bX="<div cla"+"ss=\"pitc"+"h\">"+(function(){var dx=du;
for(var c2=aV('pg'),c3='',c4=0x0;
c4<0x8;
c4++)c4%0x2!=0x0&&(c3+="<rect x="+"\"0\" y=\""+0x34*c4+("\" width="+"\"300\" he"+"ight=\"52"+"\" fill=\""+"#fff\" fi"+"ll-opaci"+"ty=\".055"+"\"/>"));
return "<svg vie"+"wBox=\"0 "+"0 300 41"+"5\" prese"+"rveAspec"+"tRatio=\""+"none\" ar"+"ia-hidde"+"n=\"true\""+"><defs><"+"linearGr"+"adient i"+"d=\""+c2+("\" x1=\"0\""+" y1=\"0\" "+"x2=\".4\" "+"y2=\"1\"><"+"stop off"+"set=\"0\" "+"stop-col"+"or=\"#155"+"63e\"/><s"+"top offs"+"et=\".5\" "+"stop-col"+"or=\"#104"+"330\"/><s"+"top offs"+"et=\"1\" s"+"top-colo"+"r=\"#0b32"+"24\"/></l"+"inearGra"+"dient></"+"defs><re"+"ct width"+"=\"300\" h"+"eight=\"4"+"15\" fill"+"=\"url(#")+c2+")\"/>"+c3+("<g fill="+"\"none\" s"+"troke=\"#"+"eafff6\" "+"stroke-o"+"pacity=\""+".45\" str"+"oke-widt"+"h=\"2\"><r"+"ect x=\"1"+"0\" y=\"10"+"\" width="+"\"280\" he"+"ight=\"39"+"5\" rx=\"2"+"\"/><line"+" x1=\"10\""+" y1=\"207"+"\" x2=\"29"+"0\" y2=\"2"+"07\"/><ci"+"rcle cx="+"\"150\" cy"+"=\"207\" r"+"=\"42\"/><"+"rect x=\""+"72\" y=\"1"+"0\" width"+"=\"156\" h"+"eight=\"5"+"8\"/><rec"+"t x=\"112"+"\" y=\"10\""+" width=\""+"76\" heig"+"ht=\"24\"/"+"><rect x"+"=\"72\" y="+"\"347\" wi"+"dth=\"156"+"\" height"+"=\"58\"/><"+"rect x=\""+"112\" y=\""+"381\" wid"+"th=\"76\" "+"height=\""+"24\"/></g"+"><circle"+" cx=\"150"+"\" cy=\"20"+"7\" r=\"3."+"5\" fill="+"\"#eafff6"+"\" fill-o"+"pacity=\""+".6\"/><ci"+"rcle cx="+"\"150\" cy"+"=\"52\" r="+"\"3\" fill"+"=\"#eafff"+"6\" fill-"+"opacity="+"\".45\"/><"+"circle c"+"x=\"150\" "+"cy=\"363\""+" r=\"3\" f"+"ill=\"#ea"+"fff6\" fi"+"ll-opaci"+"ty=\".45\""+"/></svg>");
}())+a0["POSITION"+'S']["map"](function(c2){var dy=du,c3=aW[c2['id']]||[0x32,0x32];
return "<button "+"class=\"p"+"os-btn"+(av["pos"]===c2['id']?" selecte"+'d':'')+("\" data-p"+"os=\"")+c2['id']+("\" style="+"\"left:")+c3[0x0]+"%;top:"+c3[0x1]+"%\">"+c2['id']+("</button"+'>');
})["join"]('')+("</div><p"+" class=\""+"pitch-hi"+"nt\">已选 <"+'b>')+ax(bY["name"])+'（'+bY['id']+("）</b></p"+'>');
}else{if(0x3===aX){var bZ=av["dreamId"]?ag(av["dreamId"]):null,c0=bZ?a6["leagueOf"+"Team"](bZ):null,c1=aZ||(bZ?bZ["league"]:"csl");
bX="<button "+"class=\"d"+"ream-non"+'e'+(bZ?'':" selecte"+'d')+("\" data-d"+"ream=\"\">"+"<span cl"+"ass=\"dre"+"am-none-"+"t\">随缘</s"+"pan><spa"+"n class="+"\"dream-n"+"one-d\">")+(bZ?"现在指定的是 "+ax(bZ["name"])+'（'+ax(c0?c0["name"]:'')+'）'+(c0&&!c0['cn']?" · 12 岁送"+"出国要花 "+a0["YOUTH_AB"+"ROAD_FEE"]+(" 万，家底不够就"+"是欠债去的"):''):"不指定，报价单照"+"旧全凭运气")+("</span><"+"/button>"+"<div cla"+"ss=\"lg-t"+"abs\">")+a0["LEAGUES"]["map"](function(c2){var dz=du;
return "<button "+"class=\"l"+"g-tab"+(c1===c2['id']?" selecte"+'d':'')+("\" data-d"+"reamlg=\"")+c2['id']+'\x22>'+ax(c2["name"])+("</button"+'>');
})["join"]('')+("</div><d"+"iv class"+"=\"dream-"+"grid\">")+a0["TEAMS"]["filter"](function(c2){var dA=du;
return c2["league"]===c1;
})["map"](function(c2){var dB=du;
return "<button "+"class=\"d"+"ream-tea"+'m'+(av["dreamId"]===c2['id']?" selecte"+'d':'')+("\" data-d"+"ream=\"")+c2['id']+'\x22>'+aT(c2)+"<span>"+ax(c2["name"])+("</span><"+"/button>");
})["join"]('')+"</div>";
}else bX="<div cla"+"ss=\"jers"+"ey-wrap\""+'>'+aM(av["origin"],av["name"]||'球员',av["number"])+("</div><d"+"iv class"+"=\"form-r"+"ow\"><div"+"><span c"+"lass=\"fi"+"eld-labe"+"l\">姓名</s"+"pan><inp"+"ut class"+"=\"input\""+" id=\"in-"+"name\" ma"+"xlength="+"\"6\" plac"+"eholder="+"\"李亦非\" au"+"tocomple"+"te=\"off\""+" value=\"")+ax(av["name"])+("\"></div>"+"<div cla"+"ss=\"narr"+"ow\"><spa"+"n class="+"\"field-l"+"abel\">号码"+"</span><"+"input cl"+"ass=\"inp"+"ut\" id=\""+"in-numbe"+"r\" type="+"\"number\""+" min=\"1\""+" max=\"99"+"\" value="+'\x22')+av["number"]+("\"></div>"+"</div><d"+"iv style"+"=\"margin"+"-top:1.1"+"rem\"><sp"+"an class"+"=\"field-"+"label\">惯"+"用脚</span"+"><div cl"+"ass=\"seg"+"mented\" "+"id=\"foot"+"-seg\"><b"+"utton cl"+"ass=\"seg")+("left"===av["foot"]?" selecte"+'d':'')+("\" data-f"+"oot=\"lef"+"t\">左脚</b"+"utton><b"+"utton cl"+"ass=\"seg")+("right"===av["foot"]?" selecte"+'d':'')+("\" data-f"+"oot=\"rig"+"ht\">右脚</"+"button><"+"/div></d"+"iv>")+("<div st"+"yle=\"ma"+"rgin-to"+"p:.9re"+"m\"><bu"+"tton ty"+"pe=\"butt"+"on\" dat"+"a-rerol"+"l class"+"=\"seg\">"+"换个随机</"+"button><"+"span st"+"yle=\"ma"+"rgin-le"+"ft:.6re"+"m;font-"+"size:.7"+"2rem;o"+"pacity:.65\">"+"姓名、号"+"码、惯用"+"脚都会重"+"新随机</sp"+"an></di"+"v>")+(bU2(av["name"],
av["number"])?"<div st"+"yle=\"ma"+"rgin-to"+"p:.8re"+"m;color"+":#34d39"+"3\">抽中"+"了球星"+bU2(av["name"],av["number"])+"，会有"+"一点点属"+"性加成</"+"div>":"");
}}aw("step-bod"+'y')["innerHTM"+'L']=(function(){var dC=du,c2=aB();
if(!c2)return'';
var c3=[];
return c2["ovr"]&&c3["push"]("能力 +"+c2["ovr"]),c2["talent"]&&c3["push"]("天赋 +"+c2["talent"]["toFixed"](0x2)),c2["guanxi"]&&c3["push"]("关系 +"+c2["guanxi"]),
c2["money"]&&c3["push"]("家底 +"+al(c2["money"])),"<div cla"+"ss=\"lega"+"cy-banne"+"r\"><div "+"class=\"l"+"egacy-ge"+"n\">第 "+c2["gen"]+(" 世</div>"+"<div cla"+"ss=\"lega"+"cy-bits\""+'>')+(c3["length"]?ax(c3["join"](" · ")):"上一世什么也没留"+'下')+("</div></"+"div>");
}())+bX,aw("btn-next")["textCont"+"ent"]=aX===aY["length"]-0x1?"开始生涯":'继续',aw("btn-back")["textCont"+"ent"]=0x0===aX?'返回':"上一步";
}function b1(bW,bX,bY,bZ){var dD=d3;
return "<div cla"+"ss=\"ev-h"+"ead\"><sp"+"an class"+"=\"ev-ico"+"n\">"+bW+("</span><"+"div clas"+"s=\"ev-he"+"ad-main\""+"><div cl"+"ass=\"ev-"+"tag")+(bZ?" decisio"+'n':'')+'\x22>'+ax(bX)+("</div><d"+"iv class"+"=\"ev-tit"+"le\">")+ax(bY)+("</div></"+"div></di"+'v>');
}var b2=null;
function b3(bW){var dE=d3,bX=aw("career-r"+"oot");
return bX&&bX["querySel"+"ector"]?bX["querySel"+"ector"](bW):null;
}var b4={'apps':"<svg cla"+"ss=\"mi\" "+"viewBox="+"\"0 0 24 "+"24\" aria"+"-hidden="+"\"true\"><"+"rect x=\""+"2.5\" y=\""+"4.5\" wid"+"th=\"19\" "+"height=\""+"15\" rx=\""+"2\" fill="+"\"#1c3a28"+"\" stroke"+"=\"#4ade8"+"0\" strok"+"e-width="+"\"1.6\"/><"+"path d=\""+"M12 4.5v"+"15\" stro"+"ke=\"#4ad"+"e80\" str"+"oke-widt"+"h=\"1.4\"/"+"><circle"+" cx=\"12\""+" cy=\"12\""+" r=\"3.1\""+" fill=\"n"+"one\" str"+"oke=\"#4a"+"de80\" st"+"roke-wid"+"th=\"1.4\""+"/><path "+"d=\"M2.5 "+"9h3v6h-3"+"M21.5 9h"+"-3v6h3\" "+"fill=\"no"+"ne\" stro"+"ke=\"#4ad"+"e80\" str"+"oke-widt"+"h=\"1.4\"/"+"></svg>",
'goals':"<svg cla"+"ss=\"mi\" "+"viewBox="+"\"0 0 24 "+"24\" aria"+"-hidden="+"\"true\"><"+"circle c"+"x=\"12\" c"+"y=\"12\" r"+"=\"9\" fil"+"l=\"#f4f6"+"f2\"/><pa"+"th d=\"M1"+"2 6.6l3."+"4 2.5-1."+"3 4h-4.2"+"l-1.3-4z"+"\" fill=\""+"#20241d\""+"/><path "+"d=\"M12 3"+"v3.6M4.2"+" 9.6l3.7"+" 1.5M19."+"8 9.6l-3"+".7 1.5M7"+".4 20l2."+"5-3M16.6"+" 20l-2.5"+"-3\" stro"+"ke=\"#202"+"41d\" str"+"oke-widt"+"h=\"1.3\"/"+"></svg>",
'ast':"<svg cla"+"ss=\"mi\" "+"viewBox="+"\"0 0 24 "+"24\" aria"+"-hidden="+"\"true\"><"+"path d=\""+"M2.5 16."+"2h5.6c.7"+" 2 2.6 2"+".9 5 3.3"+" 2 .4 3."+"4.7 3.9 "+"1.4H2.5z"+"\" fill=\""+"#f4f6f2\""+"/><path "+"d=\"M2.5 "+"12.4h4.2"+"c.4 1.6."+"9 2.8 1."+"4 3.8H2."+"5z\" fill"+"=\"#c9d2c"+"4\"/></sv"+'g>',
'cs':"<svg cla"+"ss=\"mi\" "+"viewBox="+"\"0 0 24 "+"24\" aria"+"-hidden="+"\"true\"><"+"path d=\""+"M7 7.5c0"+"-1.6 3-1"+".6 3 0v2"+"c.6-2.2 "+"3.6-1.6 "+"3.4.3.8-"+"1.7 3.5-"+"1.1 3.3."+"9l-.6 5."+"8c-.3 2."+"4-2.5 3."+"5-5 3.5-"+"2.8 0-4."+"5-1.7-4."+"5-4.2z\" "+"fill=\"#f"+"4f6f2\"/>"+"</svg>",
'ga':"<svg cla"+"ss=\"mi\" "+"viewBox="+"\"0 0 24 "+"24\" aria"+"-hidden="+"\"true\"><"+"path d=\""+"M3 6h18v"+"12H3z\" f"+"ill=\"non"+"e\" strok"+"e=\"#e08a"+"7a\" stro"+"ke-width"+"=\"1.6\"/>"+"<path d="+"\"M7 6v12"+"M11 6v12"+"M15 6v12"+"M19 6v12"+"M3 10h18"+"M3 14h18"+"\" stroke"+"=\"#e08a7"+"a\" strok"+"e-width="+"\".9\" opa"+"city=\".7"+"\"/></svg"+'>'};
function b5(bW){var dF=d3;
return bW>=0x58?"plat":bW>=0x4e?"gold":bW>=0x44?"silver":bW>=0x3a?"bronze":"base";
}function b6(bW,bX,bY){var dG=d3;
return "<div cla"+"ss=\"bar\""+"><span c"+"lass=\"ba"+"r-l\">"+bW+("</span><"+"div clas"+"s=\"bar-t"+"\"><div c"+"lass=\"ba"+"r-f\" sty"+"le=\"widt"+'h:')+ad(bX,
0x0,0x64)+("%;backgr"+"ound:")+bY+("\"></div>"+"</div><s"+"pan clas"+"s=\"bar-v"+'\x22>')+Math["round"](bX)+("</span><"+"/div>");
}function b7(){var dH=d3,bW=ag(au["youthTea"+"mId"]);
return bW?a6["academyN"+"ame"](bW):'';
}function b8(bW,bX,bY,bZ,c0,c1,c2){var dI=d3;
return "<div cla"+"ss=\"tl-r"+"ow tl-co"+"ls "+bW+'\x22>'+bX+bY+bZ+("<span cl"+"ass=\"tl-"+"n r\">")+c0+("</span><"+"span cla"+"ss=\"tl-n"+" r\">")+c1+("</span><"+"span cla"+"ss=\"tl-n"+" r hide-"+"xs\">")+c2+("</span><"+"/div>");
}function b9(bW){var dJ=d3,bX=ah(au["pos"])["group"],bY="<div cla"+"ss=\"time"+"line\"><d"+"iv class"+"=\"tl-hea"+"d tl-col"+"s\"><span"+">年龄</spa"+"n><span>"+"俱乐部</spa"+"n><span "+"class=\"r"+"\">能力</sp"+"an><span"+" class=\""+"r\">"+b4["apps"]+("</span><"+"span cla"+"ss=\"r\">")+('gk'===bX?b4['cs']:b4["goals"])+("</span><"+"span cla"+"ss=\"r hi"+"de-xs\">")+('gk'===bX?b4['ga']:b4["ast"])+("</span><"+"/div><di"+"v class="+"\"tl-scro"+"ll\">");
if((au["youthLog"]||[])["forEach"](function(c2){var dK=dJ,c3=ag(c2["teamId"]);
bY+=b8("done you"+'th',"<span cl"+"ass=\"age"+"-chip\">"+c2["age"]+"</span>","<span cl"+"ass=\"tl-"+"club\">"+(c3?aT(c3):'')+("<span cl"+"ass=\"tl-"+"club-nam"+"e\">")+(c3?ax(a6["academyN"+"ame"](c3)):'青训')+"</span>"+(c2["cut"]?"<span cl"+"ass=\"tl-"+"badges\">"+"<span cl"+"ass=\"min"+"i-badge "+"bad\">被刷下"+"来</span>"+"</span>":'')+"</span>",
"<span cl"+"ass=\"r\">"+"<span cl"+"ass=\"ovr"+"-pill "+b5(c2["ovr"])+'\x22>'+c2["ovr"]+("</span><"+"/span>"),'','','');
}),function(){var dL=dJ,c2=a8[au["mode"]]["seasons"],c3=[],c4=null,c5=null,c6=0x0;
function c7(){var dM=dL;
c4&&c5&&(c4["age"]=c5["age"],c4["teamName"]=c5["teamName"],c4["color"]=c5["color"],c4["ovr"]=c5["ovrEnd"],c4["teamId"]=c5["teamId"],
c3["push"](c4),c4=null,c6=0x0);
}return au["seasons"]["forEach"](function(c8,c9){var dN=dL;
c4&&c5&&c8["teamId"]!==c5["teamId"]&&c7(),c4||(c4={'apps':0x0,'goals':0x0,'assists':0x0,'cs':0x0,'ga':0x0,'caps':0x0,'natGoals':0x0,'natAssists':0x0,'natCs':0x0,'trophies':[],'notes':[],'nats':[],'moves':[]}),
c4["apps"]+=c8["apps"],c4["goals"]+=c8["goals"],c4["assists"]+=c8["assists"],c4['cs']+=c8['cs'],c4['ga']+=c8['ga'],c4["caps"]+=c8["caps"]||0x0,
c4["natGoals"]+=c8["natGoals"]||0x0,c4["natAssis"+'ts']+=c8["natAssis"+'ts']||0x0,c4["natCs"]+=c8["natCs"]||0x0,c8["trophies"]["length"]&&(c4["trophies"]=c4["trophies"]["concat"](c8["trophies"])),
c8["note"]&&c4["notes"]["push"](c8["note"]),c8["nat"]&&c4["nats"]["push"](c8["nat"]),c8["move"]&&c4["moves"]["push"](c8["move"]),
c5=c8,(++c6>=c2||c9===au["seasons"]["length"]-0x1)&&c7();
}),c3;
}()["forEach"](function(c2){var dO=dJ,c3='';
c2["caps"]&&(c3="国家队 "+c2["caps"]+'\x20场','gk'===bX?c2["natCs"]&&(c3+=" · "+c2["natCs"]+" 零封"):(c2["natGoals"]&&(c3+=" · "+c2["natGoals"]+'\x20球'),c2["natAssis"+'ts']&&(c3+=" · "+c2["natAssis"+'ts']+'\x20助')));
var c4=(c3?["<span cl"+"ass=\"min"+"i-badge "+"nat\">"+ax(c3)+"</span>"]:[])["concat"](c2["notes"]["map"](function(c5){var dP=dO;
return "<span cl"+"ass=\"min"+"i-badge "+"bad\">"+ax(c5)+"</span>";
}))["concat"]((c2["nats"]||[])["map"](function(c5){var dQ=dO;
return "<span cl"+"ass=\"min"+"i-badge "+"nat\">"+ax(c5)+"</span>";
}))["concat"]((c2["moves"]||[])["map"](function(c5){var dR=dO;
return "<span cl"+"ass=\"min"+"i-badge "+(0x0===c5["indexOf"]('升')?'up':"bad")+'\x22>'+ax(c5)+"</span>";
}))["concat"](c2["trophies"]["map"](function(c5){var dS=dO;
return "<span cl"+"ass=\"min"+"i-badge\""+'>'+ax(c5)+"</span>";
}))["join"]('');
bY+=b8("done","<span cl"+"ass=\"age"+"-chip\""+(c2["color"]?" style=\""+"backgrou"+"nd:"+c2["color"]+(";color:#"+"fff\""):'')+'>'+c2["age"]+"</span>",
"<span cl"+"ass=\"tl-"+"club\">"+aT(ag(c2["teamId"]))+("<span cl"+"ass=\"tl-"+"club-nam"+"e\">")+ax(c2["teamName"])+"</span>"+(c4?"<span cl"+"ass=\"tl-"+"badges\">"+c4+"</span>":'')+"</span>",
"<span cl"+"ass=\"r\">"+"<span cl"+"ass=\"ovr"+"-pill "+b5(c2["ovr"])+'\x22>'+c2["ovr"]+("</span><"+"/span>"),c2["apps"],'gk'===bX?c2['cs']:c2["goals"],
'gk'===bX?c2['ga']:c2["assists"]);
}),!bW){var bZ=ai(),c0=au["pending"]&&("academy"===au["pending"]["type"]||"transfer"===au["pending"]["type"]||"retire_f"+"orced"===au["pending"]["type"]),
c1=au["pending"]&&"youth_pa"+'th'===au["pending"]["type"];
bY+=b8("now","<span cl"+"ass=\"age"+"-chip\">"+au["age"]+"</span>","<span cl"+"ass=\"tl-"+"club\">"+(bZ&&!c0?aT(bZ):'')+("<span cl"+"ass=\"tl-"+"club-nam"+"e\">")+(c1?"选青训营…":c0?"选择俱乐部…":"youth"===au["phase"]?b7()||'青训':ax(bZ?bZ["name"]:"自由身"))+("</span><"+"/span>"),
"<span cl"+"ass=\"r\">"+"<span cl"+"ass=\"ovr"+"-pill "+b5(au["ovr"])+'\x22>'+Math["round"](au["ovr"])+("</span><"+"/span>"),
'','','');
}return bY+("</div></"+"div>");
}function ba(bW,bX,bY,bZ){var dT=d3;
return "<button "+"class=\"o"+"pt\" data"+"-opt=\""+bW+("\"><span "+"class=\"o"+"pt-label"+'\x22>')+ax(bX)+"</span>"+(bZ?aT(bZ):'')+(bY?"<span cl"+"ass=\"opt"+"-hint\">"+ax(bY)+"</span>":'')+("</button"+'>');
}function bb(bW,bX){var dU=d3;
return bW["length"]?"<div cla"+"ss=\"opts"+(bX?'\x20'+bX:'')+("\" data-n"+'=\x22')+bW["length"]+'\x22>'+bW["join"]('')+"</div>":'';
}var bc={'key':'','ord':null},bd=/^\s*(\d+)\s*%\s*([^\/]+?)\s*\/\s*(\d+)\s*%\s*(.+?)\s*$/;
function be(){var dV=d3;
return "function"==typeof matchMedia&&matchMedia("(prefers"+"-reduced"+"-motion:"+" reduce)")["matches"];
}var bf=!0x1;
function bg(bW,bX,bY,bZ,c0){var dW=d3,c1=a6["leagueOf"+"Team"](bX),c2=["保级队","中下游",'中游','争冠','豪门',"顶级豪门"][bX["rep"]],c3=c0?a6["offerBri"+'ef'](bX['id']):null;
return "<button "+"class=\"o"+'pt'+(au["dreamId"]===bX['id']?" dream":'')+("\" data-o"+"pt=\"")+bW+'\x22>'+(bZ?"<span cl"+"ass=\"opt"+"-lead\">"+ax(bZ)+"</span>":'')+(au["dreamId"]===bX['id']?"<span cl"+"ass=\"opt"+"-dream\">"+"心仪</span"+'>':'')+("<span cl"+"ass=\"opt"+"-label\">")+ax(bY)+"</span>"+aT(bX)+("<span cl"+"ass=\"opt"+"-hint\">")+ax(c1["name"]+" · "+c2)+"</span>"+(c3?"<span cl"+"ass=\"opt"+"-offer\">"+"<b>"+ax(al(c3["wage"]))+("</b>/赛季<"+"span cla"+"ss=\"opt-"+"role rol"+'e-')+c3["role"]+'\x22>'+ax(c3["roleName"])+("</span><"+"/span>"):'')+("</button"+'>');
}var bh=[[/欧冠/,"ucl"],[/欧联/,"uel"],[/亚冠/,"challeng"+'e'],[/世界杯冠军/,'wc'],[/打进世界杯/,"wcgold"],[/亚洲杯/,"asiancup"],[/亚洲足球先生/,"afcpoy"],
[/中超冠军/,"csl"],[/足协杯/,"cn-facup"],[/世俱杯/,"cwc"],[/金球/,"ballon"],[/金靴/,"boot"],[/金手套/,"glove"],[/最佳球员/,"best"],[/国王杯/,"kingcup"],
[/德国杯/,"dfb"],[/足总杯/,"facup"],[/意大利杯|法国杯|荷兰杯|葡萄牙杯|比利时杯|天皇杯|韩国杯|公开杯/,"pedestal"],[/冠军$/,"league"]];
function bi(bW){var dX=d3;
for(var bX=String(bW||''),bY=0x0;
bY<bh["length"];
bY++)if(bh[bY][0x0]["test"](bX))return "assets/t"+"rophies/"+bh[bY][0x1]+".png";
return null;
}function bj(bW,bX){var dY=d3,bY=String(bW||''),bZ=bi(bY);
if(bZ)return "<img cla"+"ss=\"trop"+"hy "+(bX||'')+"\" src=\""+ax(bZ)+("\" alt=\"\""+" loading"+"=\"lazy\">");
var c0=/欧冠|亚冠|欧联/["test"](bY)?"ear":/世界杯冠军/["test"](bY)?'wc':/世俱杯/["test"](bY)?"globe":/金靴/["test"](bY)?"boot":/金手套/["test"](bY)?"glove":/金球/["test"](bY)?"ball":/打进世界杯|亚洲杯/["test"](bY)?"cup":/联赛|中超|中甲|英超|西甲|德甲|意甲|法甲|荷甲|葡超|日职|K联赛|沙特联|美职联|比甲|西乙|德乙|英冠/["test"](bY)?"shield":"cup",
c1="ear"===c0||"cup"===c0||"glove"===c0,c2=aV('tg');
return "<svg cla"+"ss=\"trop"+"hy "+(bX||'')+("\" viewBo"+"x=\"0 0 2"+"00 160\" "+"aria-hid"+"den=\"tru"+"e\"><defs"+"><linear"+"Gradient"+" id=\"")+c2+("\" x1=\".1"+"\" y1=\"0\""+" x2=\".7\""+" y2=\"1\">")+(c1?"<stop of"+"fset=\"0\""+" stop-co"+"lor=\"#ff"+"ffff\"/><"+"stop off"+"set=\".35"+"\" stop-c"+"olor=\"#d"+"5dee4\"/>"+"<stop of"+"fset=\".6"+"2\" stop-"+"color=\"#"+"9aa8b2\"/"+"><stop o"+"ffset=\"1"+"\" stop-c"+"olor=\"#6"+"d7a84\"/>":"<stop of"+"fset=\"0\""+" stop-co"+"lor=\"#ff"+"f3c4\"/><"+"stop off"+"set=\".35"+"\" stop-c"+"olor=\"#f"+"5cf62\"/>"+"<stop of"+"fset=\".6"+"2\" stop-"+"color=\"#"+"dda51f\"/"+"><stop o"+"ffset=\"1"+"\" stop-c"+"olor=\"#9"+"d6c0c\"/>")+("</linear"+"Gradient"+"></defs>")+("ear"===c0?"<path d="+"\"M72,26 "+"h56 c0,3"+"4 -10,54"+" -28,62 "+"c-18,-8 "+"-28,-28 "+"-28,-62 "+"z\" fill="+"\"url(#"+c2+(")\"/><pat"+"h d=\"M72"+",30 C36,"+"28 24,52"+" 32,76 C"+"39,96 58"+",100 72,"+"92\" fill"+"=\"none\" "+"stroke=\""+"url(#")+c2+(")\" strok"+"e-width="+"\"10\" str"+"oke-line"+"cap=\"rou"+"nd\"/><pa"+"th d=\"M1"+"28,30 C1"+"64,28 17"+"6,52 168"+",76 C161"+",96 142,"+"100 128,"+"92\" fill"+"=\"none\" "+"stroke=\""+"url(#")+c2+(")\" strok"+"e-width="+"\"10\" str"+"oke-line"+"cap=\"rou"+"nd\"/><re"+"ct x=\"94"+"\" y=\"88\""+" width=\""+"12\" heig"+"ht=\"24\" "+"fill=\"ur"+"l(#")+c2+(")\"/><ell"+"ipse cx="+"\"100\" cy"+"=\"118\" r"+"x=\"30\" r"+"y=\"8\" fi"+"ll=\"url("+'#')+c2+(")\"/><rec"+"t x=\"64\""+" y=\"122\""+" width=\""+"72\" heig"+"ht=\"14\" "+"rx=\"4\" f"+"ill=\"#00"+"04\"/>"):'wc'===c0?"<circle "+"cx=\"100\""+" cy=\"40\""+" r=\"19\" "+"fill=\"ur"+"l(#"+c2+(")\"/><pat"+"h d=\"M10"+"0,58 C86"+",58 78,7"+"2 82,90 "+"C86,106 "+"94,116 1"+"00,124 C"+"106,116 "+"114,106 "+"118,90 C"+"122,72 1"+"14,58 10"+"0,58 z\" "+"fill=\"ur"+"l(#")+c2+(")\"/><pat"+"h d=\"M10"+"0,62 v60"+"\" stroke"+"=\"#0003\""+" stroke-"+"width=\"3"+"\"/><rect"+" x=\"74\" "+"y=\"124\" "+"width=\"5"+"2\" heigh"+"t=\"10\" r"+"x=\"3\" fi"+"ll=\"url("+'#')+c2+(")\"/><rec"+"t x=\"64\""+" y=\"134\""+" width=\""+"72\" heig"+"ht=\"14\" "+"rx=\"4\" f"+"ill=\"#00"+"04\"/>"):"globe"===c0?"<circle "+"cx=\"100\""+" cy=\"60\""+" r=\"32\" "+"fill=\"ur"+"l(#"+c2+(")\"/><ell"+"ipse cx="+"\"100\" cy"+"=\"60\" rx"+"=\"13\" ry"+"=\"32\" fi"+"ll=\"none"+"\" stroke"+"=\"#0003\""+" stroke-"+"width=\"3"+"\"/><line"+" x1=\"68\""+" y1=\"60\""+" x2=\"132"+"\" y2=\"60"+"\" stroke"+"=\"#0003\""+" stroke-"+"width=\"3"+"\"/><rect"+" x=\"94\" "+"y=\"92\" w"+"idth=\"12"+"\" height"+"=\"22\" fi"+"ll=\"url("+'#')+c2+(")\"/><ell"+"ipse cx="+"\"100\" cy"+"=\"118\" r"+"x=\"28\" r"+"y=\"7\" fi"+"ll=\"url("+'#')+c2+(")\"/><rec"+"t x=\"66\""+" y=\"122\""+" width=\""+"68\" heig"+"ht=\"14\" "+"rx=\"4\" f"+"ill=\"#00"+"04\"/>"):"shield"===c0?"<path d="+"\"M100,20"+" L158,40"+" L158,84"+" C158,11"+"6 132,14"+"0 100,15"+"0 C68,14"+"0 42,116"+" 42,84 L"+"42,40 z\""+" fill=\"u"+"rl(#"+c2+(")\"/><pat"+"h d=\"M10"+"0,50 l11"+",23 25,3"+" -18,18 "+"4,25 -22"+",-12 -22"+",12 4,-2"+"5 -18,-1"+"8 25,-3 "+"z\" fill="+"\"#0002\"/"+'>'):"boot"===c0?"<path d="+"\"M54,60 "+"h30 c4,2"+"2 20,32 "+"44,36 c1"+"4,4 20,1"+"2 20,22 "+"v8 H54 z"+"\" fill=\""+"url(#"+c2+(")\"/><pat"+"h d=\"M54"+",96 h94\""+" stroke="+"\"#0002\" "+"stroke-w"+"idth=\"4\""+"/><rect "+"x=\"48\" y"+"=\"126\" w"+"idth=\"10"+"6\" heigh"+"t=\"14\" r"+"x=\"4\" fi"+"ll=\"#000"+"4\"/>"):"glove"===c0?"<path d="+"\"M64,56 "+"c0,-13 2"+"3,-13 23"+",0 v16 c"+"5,-17 27"+",-13 25,"+"2 c7,-13"+" 27,-9 2"+"5,6 l-5,"+"40 c-2,1"+"9 -19,28"+" -37,28 "+"c-21,0 -"+"33,-13 -"+"33,-31 z"+"\" fill=\""+"url(#"+c2+(")\"/><pat"+"h d=\"M70"+",110 h58"+"\" stroke"+"=\"#0002\""+" stroke-"+"width=\"5"+"\"/>"):"ball"===c0?"<circle "+"cx=\"100\""+" cy=\"56\""+" r=\"32\" "+"fill=\"ur"+"l(#"+c2+(")\"/><pat"+"h d=\"M10"+"0,34 l13"+",10 -5,1"+"5 h-16 l"+"-5,-15 z"+"\" fill=\""+"#0003\"/>"+"<path d="+"\"M74,44 "+"l8,14 M1"+"26,44 l-"+"8,14 M84"+",80 l6,-"+"12 M116,"+"80 l-6,-"+"12\" stro"+"ke=\"#000"+"3\" strok"+"e-width="+"\"3\"/><re"+"ct x=\"92"+"\" y=\"88\""+" width=\""+"16\" heig"+"ht=\"20\" "+"fill=\"ur"+"l(#")+c2+(")\"/><pat"+"h d=\"M70"+",108 h60"+" l8,28 h"+"-76 z\" f"+"ill=\"url"+'(#')+c2+")\"/>":"<path d="+"\"M70,30 "+"h60 c0,2"+"8 -10,44"+" -30,50 "+"c-20,-6 "+"-30,-22 "+"-30,-50 "+"z\" fill="+"\"url(#"+c2+(")\"/><pat"+"h d=\"M70"+",34 C48,"+"32 40,48"+" 46,62 C"+"51,73 62"+",76 70,7"+"2\" fill="+"\"none\" s"+"troke=\"u"+"rl(#")+c2+(")\" strok"+"e-width="+"\"9\" stro"+"ke-linec"+"ap=\"roun"+"d\"/><pat"+"h d=\"M13"+"0,34 C15"+"2,32 160"+",48 154,"+"62 C149,"+"73 138,7"+"6 130,72"+"\" fill=\""+"none\" st"+"roke=\"ur"+"l(#")+c2+(")\" strok"+"e-width="+"\"9\" stro"+"ke-linec"+"ap=\"roun"+"d\"/><rec"+"t x=\"94\""+" y=\"80\" "+"width=\"1"+"2\" heigh"+"t=\"24\" f"+"ill=\"url"+'(#')+c2+(")\"/><ell"+"ipse cx="+"\"100\" cy"+"=\"110\" r"+"x=\"26\" r"+"y=\"7\" fi"+"ll=\"url("+'#')+c2+(")\"/><rec"+"t x=\"68\""+" y=\"114\""+" width=\""+"64\" heig"+"ht=\"14\" "+"rx=\"4\" f"+"ill=\"#00"+"04\"/>"))+"</svg>";
}function bk(){var e0=d3,bW={},bX=[];
function bY(bZ){var dZ=_gam_0b;
bW[bZ]||(bW[bZ]=0x0,bX["push"](bZ)),bW[bZ]++;
}return au["trophies"]["forEach"](function(bZ){var e1=e0;
bY(bZ["name"]);
}),au["awards"]["forEach"](function(bZ){var e2=e0;
bY(bZ["name"]);
}),bX["map"](function(bZ){return{'name':bZ,'count':bW[bZ]};
});
}function bl(){var e3=d3,bW={},bX=[];
return au["seasons"]["forEach"](function(bY){var e4=e3;
if(bY["teamId"]){bW[bY["teamId"]]||(bW[bY["teamId"]]={'teamId':bY["teamId"],'name':bY["teamName"],'color':bY["color"],'apps':0x0,'goals':0x0,'assists':0x0,'cs':0x0,'ga':0x0,'trophies':[],'from':bY["age"],'to':bY["age"],'seasons':0x0},bX["push"](bY["teamId"]));
var bZ=bW[bY["teamId"]];
bY["age"]<bZ["from"]&&(bZ["from"]=bY["age"]),bY["age"]>bZ['to']&&(bZ['to']=bY["age"]),bZ["seasons"]++,bZ["apps"]+=bY["apps"],
bZ["goals"]+=bY["goals"],bZ["assists"]+=bY["assists"],bZ['cs']+=bY['cs'],bZ['ga']+=bY['ga'],bY["trophies"]["forEach"](function(c0){var e5=e4;
/世界杯|亚洲杯/["test"](c0)||bZ["trophies"]["push"](c0);
});
}}),bX["map"](function(bY){return bW[bY];
});
}function bm(bW){var e6=d3;
for(var bX=0x0;
bX<a0["ENDINGS"]["length"];
bX++)if(a0["ENDINGS"][bX]['id']===bW)return a0["ENDINGS"][bX];
return a0["ENDINGS"][a0["ENDINGS"]["length"]-0x1];
}var bn=[{'re':/^世界杯冠军$/,'label':"世界杯冠军",'rank':0x1},{'re':/^金球奖$/,'label':"金球奖",'rank':0x2},{'re':/^欧冠冠军$/,'label':"欧冠冠军",
'rank':0x3},{'re':/^亚洲杯冠军$/,'label':"亚洲杯冠军",'rank':0x4},{'re':/^亚洲足球先生$/,'label':"亚洲足球先生",'rank':0x5},{'re':/^欧洲金靴$/,'label':"欧洲金靴",
'rank':0x6},{'re':/^金手套$/,'label':"金手套",'rank':0x6},{'re':/^(英超|西甲|意甲|德甲|法甲)冠军$/,'label':"联赛冠军",'rank':0x7},{'re':/^欧联冠军$/,
'label':"欧联冠军",'rank':0x8},{'re':/^世俱杯冠军$/,'label':"世俱杯冠军",'rank':0x9},{'re':/^中超最佳球员$/,'label':"中超最佳球员",'rank':0xa},{'re':/^中超冠军$/,
'label':"中超冠军",'rank':0xb},{'re':/^足协杯冠军$/,'label':"足协杯冠军",'rank':0xc},{'re':/杯冠军$/,'label':"杯赛冠军",'rank':0xd}],bo=["没走到那一步",
"抹不掉的",'封神','巅峰','出色',"怎么收场","走过的路",'兜底'];
function bp(bW){var e7=d3;
return bW["endings"]&&bW["endings"]["length"]?bW["endings"]:bW["ending"]?[bW["ending"]]:[];
}function bq(){var e8=d3;
bM("view-cod"+'ex'),(function(){var e9=e8,bW=aG(),bX={},bY={},bZ={};
bW["forEach"](function(c5){var ea=e9;
bp(c5)["forEach"](function(c6){bX[c6]||(bX[c6]=0x1);
}),c5["ending"]&&(bX[c5["ending"]]=0x2,bY[c5["ending"]]=(bY[c5["ending"]]||0x0)+0x1,c5["endingTi"+"tle"]&&!bZ[c5["ending"]]&&(bZ[c5["ending"]]=c5["endingTi"+"tle"]));
});
var c0=a0["ENDINGS"]["length"],c1=0x0;
a0["ENDINGS"]["forEach"](function(c5){bX[c5['id']]&&c1++;
});
var c2="<div cla"+"ss=\"sect"+"ion-head"+"\"><h2>结局"+"图鉴</h2><"+"span>"+c1+" / "+c0+("</span><"+"/div><di"+"v class="+"\"codex-b"+"ar\"><div"+" class=\""+"codex-ba"+"r-fill\" "+"style=\"w"+"idth:")+(c0?Math["round"](c1/c0*0x64):0x0)+("%\"></div"+"></div><"+"div clas"+"s=\"codex"+"-note\">见"+"过的会亮起来。没"+"见过的只写达成条"+"件 —— 一条生"+"涯只会落到其中一"+"条上，同时满足几"+"条时，越靠上的越"+"先算。标着「够到"+"过」的，是那一局"+"条件也满足了、只"+"是被更重的那条压"+"下去没说出口。<"+"/div>"),
c3={},c4=[];
a0["ENDINGS"]["forEach"](function(c5){var eb=e9,c6=null==c5["tier"]?0x7:c5["tier"];
c3[c6]||(c3[c6]=[],c4["push"](c6)),c3[c6]["push"](c5);
}),c4["sort"](function(c5,c6){return c5-c6;
}),c4["forEach"](function(c5){var ec=e9;
c2+="<div cla"+"ss=\"code"+"x-tier\">"+ax(bo[c5]||'第\x20'+c5+'\x20档')+"<span>"+c3[c5]["filter"](function(c6){return bX[c6['id']];
})["length"]+" / "+c3[c5]["length"]+("</span><"+"/div>"),c2+="<div cla"+"ss=\"code"+"x-grid\">"+c3[c5]["map"](function(c6){var ee=ec,
c7=bX[c6['id']]||0x0,c8=c7>0x0,c9=function(cc){var ed=_gam_0b,cd=[cc["title"]];
if(cc["pos"]){for(var ce in cc["pos"])if(Object["prototyp"+'e']["hasOwnPr"+"operty"]["call"](cc["pos"],ce)){var cf=cc["pos"][ce]&&cc["pos"][ce]["title"];
cf&&cd["indexOf"](cf)<0x0&&cd["push"](cf);
}}return cd;
}(c6),ca=0x2===c7&&bZ[c6['id']]||c6["title"],cb=c9["filter"](function(cc){return cc!==ca;
});
return "<div cla"+"ss=\"code"+"x-item"+(0x2===c7?" got":0x1===c7?" touch":'')+("\"><div c"+"lass=\"co"+"dex-titl"+"e\">")+(c8?ax(ca):"？？？")+(bY[c6['id']]>0x1?"<span cl"+"ass=\"cod"+"ex-x\">×"+bY[c6['id']]+"</span>":'')+(0x1===c7?"<span cl"+"ass=\"cod"+"ex-touch"+"\">够到过</s"+"pan>":'')+"</div>"+(c8&&cb["length"]?"<div cla"+"ss=\"code"+"x-alias\""+">按位置也叫："+ax(cb["join"]('、'))+"</div>":'')+("<div cla"+"ss=\"code"+"x-desc\">")+ax(c8?c6["desc"]:c6["hint"]||'')+"</div>"+(c8&&c6["hint"]?"<div cla"+"ss=\"code"+"x-hint\">"+ax(c6["hint"])+"</div>":'')+"</div>";
})["join"]('')+"</div>";
}),c2+="<div cla"+"ss=\"btn-"+"stack\"><"+"button c"+"lass=\"bt"+"n btn-pr"+"imary\" d"+"ata-act="+"\"codex-b"+"ack\">回首页"+"</button"+"></div>",
aw("codex-ar"+'ea')["innerHTM"+'L']=c2;
}());
}function br(bW){var bX=aG()[bW];
bX&&bH(function(bY){var ef=_gam_0b;
return{'name':bY["name"],'gen':bY["gen"]||0x1,'number':bY["number"],'posId':ah(bY["pos"])['id'],'posName':ah(bY["pos"])["name"],
'group':bY["group"]||ah(bY["pos"])["group"],'originName':ak(bY["originId"])["name"],'maxOvr':bY["maxOvr"],'age':bY["age"],
'seasons':bY["seasons"],'clubCount':(bY["clubs"]||[])["length"],'apps':bY["apps"],'goals':bY["goals"],'assists':bY["assists"],
'cs':bY['cs'],'ga':bY['ga'],'caps':bY["caps"],'money':bY["money"],'spells':(bY["clubs"]||[])["map"](function(bZ){var eg=ef;
return{'teamId':bZ['id'],'color':bZ["color"]};
}),'honours':bY["honours"]||[],'nat':bY["nat"]||[],'endingTitle':bY["endingTi"+"tle"]||'','endingChoices':bY["endings"]&&bY["endings"]["length"]>0x1?bD(bY["endings"],
bY["ending"],bY["group"]||ah(bY["pos"])["group"])["map"](function(bZ){var eh=ef;
return bZ['id']===bY["ending"]&&bY["endingTi"+"tle"]?{'id':bZ['id'],'title':bY["endingTi"+"tle"]}:bZ;
}):[],'legacy':aE(bY)};
}(bX));
}function bs(){var ei=d3;
bM("view-arc"+"hive"),(function(){var ej=ei,bW=aG(),bX=aH(bW),bY="<div cla"+"ss=\"sect"+"ion-head"+"\"><h2>生涯"+"历史档案</h2"+"><span>"+(bW["length"]?bW["length"]+'\x20段':'')+("</span><"+"/div>");
if(!bW["length"])return bY+="<div cla"+"ss=\"arc-"+"empty\">还"+"没有退役过的生涯"+"。<br>走完一"+"局，这里会留下一"+"份。</div>"+"<div cla"+"ss=\"btn-"+"stack\"><"+"button c"+"lass=\"bt"+"n btn-pr"+"imary\" d"+"ata-act="+"\"arc-bac"+"k\">回首页</"+"button><"+"/div>",
void(aw("archive-"+"area")["innerHTM"+'L']=bY);
bY+="<div cla"+"ss=\"arc-"+"stats\"><"+"div><b>"+bX["lives"]+("</b><spa"+"n>段生涯</s"+"pan></di"+"v><div><"+'b>')+bX["endings"]+" / "+bX["total"]+("</b><spa"+"n>见过的结局<"+"/span></"+"div><div"+"><b>")+bX["best"]+("</b><spa"+"n>最高 OVR"+"</span><"+"/div></d"+"iv>"),
bY+="<div cla"+"ss=\"arc-"+"list\">"+bW["map"](function(bZ,c0){var ek=ej,c1=bZ["youthCut"]?[bZ["age"]+'\x20岁',(bZ["youthPat"+"hName"]||'青训')+" 第 "+Math["max"](0x1,bZ["age"]-0xb)+" 年被刷下来"]["join"](" · "):[bZ["age"]+'\x20岁',bZ["seasons"]+" 个赛季",(bZ["clubs"]?bZ["clubs"]["length"]:0x0)+" 家俱乐部"]["join"](" · ");
return "<div cla"+"ss=\"arc-"+"item\" da"+"ta-arc=\""+c0+("\"><div c"+"lass=\"ar"+"c-head\">"+"<div cla"+"ss=\"ovr-"+"badge sm"+'\x20')+b5(bZ["maxOvr"])+("\"><div c"+"lass=\"ov"+"r-badge-"+"l\">OVR</"+"div><div"+" class=\""+"ovr-badg"+"e-v\">")+bZ["maxOvr"]+("</div></"+"div><div"+" class=\""+"arc-main"+"\"><div c"+"lass=\"ar"+"c-name\">")+ax(bZ["name"])+(bZ["gen"]>0x1?"<span cl"+"ass=\"arc"+"-gen\">第 "+bZ["gen"]+(" 世</span"+'>'):'')+("</div><d"+"iv class"+"=\"arc-en"+"ding\">")+ax(bZ["endingTi"+"tle"]||'')+("</div><d"+"iv class"+"=\"arc-me"+"ta\">")+c1+("</div></"+"div><div"+" class=\""+"arc-righ"+"t\"><div "+"class=\"a"+"rc-when\""+'>')+ax(function(c2){var el=ek,c3=Date["now"]()-(Number(c2)||0x0);
if(!(c3>=0x0))return'';
var c4=Math["floor"](c3/0xea60);
if(c4<0x1)return'刚刚';
if(c4<0x3c)return c4+" 分钟前";
var c5=Math["floor"](c4/0x3c);
if(c5<0x18)return c5+" 小时前";
var c6=Math["floor"](c5/0x18);
if(c6<0x1e)return c6+" 天前";
var c7=new Date(Number(c2));
return c7["getMonth"]()+0x1+" 月 "+c7["getDate"]()+'\x20日';
}(bZ['at']))+("</div><d"+"iv class"+"=\"arc-mo"+"re\">总结卡 "+"›</div><"+"/div></d"+"iv>")+(null!=bZ["seed"]?"<div cla"+"ss=\"arc-"+"seed\"><s"+"pan clas"+"s=\"seed-"+"l\">种子</s"+"pan><cod"+"e class="+"\"seed-v\""+'>'+ax(String(bZ["seed"]))+("</code><"+"button c"+"lass=\"mi"+"ni-btn\" "+"data-act"+"=\"copy-s"+"eed\" dat"+"a-seed=\"")+ax(String(bZ["seed"]))+("\">复制</bu"+"tton></d"+"iv>"):'')+"</div>";
})["join"]('')+"</div>",bY+="<div cla"+"ss=\"btn-"+"stack\"><"+"button c"+"lass=\"bt"+"n btn-pr"+"imary\" d"+"ata-act="+"\"arc-bac"+"k\">回首页</"+"button><"+"button c"+"lass=\"bt"+"n arc-cl"+"ear\" dat"+"a-act=\"a"+"rc-clear"+"\">清空档案</"+"button><"+"/div>",
aw("archive-"+"area")["innerHTM"+'L']=bY;
}());
}var bu=0x168,bv=0x3,bw=!0x0,bx=null;
function by(bW){return new Promise(function(bX){var em=_gam_0b,bY=new Image();
bY["onload"]=function(){bX(bY);
},bY["onerror"]=function(){bX(null);
},bY["src"]=bW;
});
}function bz(bW,bX,bY,bZ,c0,c1){var en=d3;
c1=Math["min"](c1,bZ/0x2,c0/0x2),bW["beginPat"+'h'](),bW["moveTo"](bX+c1,bY),bW["arcTo"](bX+bZ,bY,bX+bZ,bY+c0,c1),bW["arcTo"](bX+bZ,
bY+c0,bX,bY+c0,c1),bW["arcTo"](bX,bY+c0,bX,bY,c1),bW["arcTo"](bX,bY,bX+bZ,bY,c1),bW["closePat"+'h']();
}function bA(bW,bX){var eo=d3;
return(bX||0x190)+'\x20'+bW+("px \"Inte"+"r\", \"Pin"+"gFang SC"+"\", \"Micr"+"osoft Ya"+"Hei\", sa"+"ns-serif");
}function bB(bW,bX,bY){var ep=d3;
bW["font"]=bA(0x9,0x2ee),bW["fillStyl"+'e']="rgba(255"+",255,255"+",.42)",bW["textAlig"+'n']="center";
var bZ=String(bX)["split"]('')["join"]('\x20');
bW["fillText"](bZ,bu/0x2,bY);
}function bC(){var eq=d3;
return{'name':au["name"],'gen':au["gen"]||0x1,'number':au["number"],'posId':ah(au["pos"])['id'],'posName':ah(au["pos"])["name"],
'group':ah(au["pos"])["group"],'originName':ak(au["originId"])["name"],'maxOvr':Math["round"](au["maxOvr"]),'age':au["age"],
'seasons':au["seasons"]["length"],'clubCount':au["clubsPla"+"yed"]["length"],'apps':au["totals"]["apps"],'goals':au["totals"]["goals"],
'assists':au["totals"]["assists"],'cs':au["totals"]['cs'],'ga':au["totals"]['ga'],'caps':au["caps"],'money':au["money"],'spells':bl()["map"](function(bW){var er=eq;
return{'teamId':bW["teamId"],'color':bW["color"]};
}),'honours':bk(),'nat':ap(),'endingTitle':a0["endingVi"+'ew'](bm(au["ending"]),ah(au["pos"])["group"])["title"],'endingChoices':bD(au["endingsA"+'ll'],
au["ending"],ah(au["pos"])["group"])};
}function bD(bW,bX,bY){var es=d3;
return(bW&&bW["length"]?bW["slice"]():bX?[bX]:[])["map"](bm)["filter"](Boolean)["sort"](function(bZ,c0){var et=es;
return bZ['id']===bX?-0x1:c0['id']===bX?0x1:(null==bZ["tier"]?0x9:bZ["tier"])-(null==c0["tier"]?0x9:c0["tier"]);
})["map"](function(bZ){var eu=es;
return{'id':bZ['id'],'title':a0["endingVi"+'ew'](bZ,bY)["title"]};
});
}function bE(bW){var ev=d3,bX=aw("share-ca"+"nvas");
if(!bX||!bX["getConte"+'xt'])return Promise["resolve"]();
var bY=(bW=bW||bG||bC())["group"],bZ=bW["spells"],c0=bW["honours"],c1=bZ["map"](function(c2){var ew=ev,c3=ag(c2["teamId"]),c4=c3&&window["CREST_UR"+'LS']&&window["CREST_UR"+'LS'][c3['id']];
return c4?by(c4["replace"](/\.webp$/,".png")):Promise["resolve"](null);
})["concat"](c0["map"](function(c2){var ex=ev,c3=bi(c2["name"]);
return c3?by(c3):Promise["resolve"](null);
}));
return Promise["all"](c1)["then"](function(c2){var ey=ev,c3=c2["slice"](0x0,bZ["length"]),c4=c2["slice"](bZ["length"]),c5=0x0,c6=0x10,c7=0x6e,c8=c5=0x7e,c9=Math["ceil"](bZ["length"]/0x6)||0x0,ca=c9?Math["ceil"](bZ["length"]/c9):0x0;
bZ["length"]&&(c5+=0xe+0x32*(c9-0x1)+0x2e+0x12);
var cb=c5,cc=c0["length"]>0xa?0x6:c0["length"]>0x8?0x5:0x4,cd=Math["ceil"](c0["length"]/cc)||0x0,ce=cd?Math["ceil"](c0["length"]/cd):0x0,
cf=ce?Math["min"](0x3e,(bu-0x20)/ce):0x0;
c0["length"]&&(c5+=0xe+0x3e*cd+0xe);
var cg=c5+0x2,ch=(c5=cg+0x3a+0x8)+0x6,ci=ch+0x40,cj=bv;
bX["width"]=bu*cj,bX["height"]=ci*cj;
var ck=bX["getConte"+'xt']('2d');
ck["setTrans"+"form"](cj,0x0,0x0,cj,0x0,0x0),ck["textBase"+"line"]="alphabet"+'ic',ck["fillStyl"+'e']="#12140f",bz(ck,0x0,
0x0,bu,ci,0xe),ck["fill"](),ck["strokeSt"+"yle"]="rgba(255"+",255,255"+",.09)",ck["lineWidt"+'h']=0x1,bz(ck,0.5,0.5,bu-0x1,
ci-0x1,0xe),ck["stroke"](),ck["fillStyl"+'e']=function(cF,cG){var ez=ey,cH=cF["createLi"+"nearGrad"+"ient"](0x10,0x10,0x4e,0x5a),
cI="plat"===cG?["#fffaea","#e8ac3c"]:"gold"===cG?["#ffd75e","#d99b1c"]:"silver"===cG?["#cfd8de","#8d9aa6"]:"bronze"===cG?["#c98a45","#8f5a22"]:["#8a6a44","#5f4426"];
return cH["addColor"+"Stop"](0x0,cI[0x0]),cH["addColor"+"Stop"](0x1,cI[0x1]),cH;
}(ck,b5(bW["maxOvr"])),bz(ck,0x10,0x10,0x3e,0x4a,0xa),ck["fill"](),ck["textAlig"+'n']="center",ck["fillStyl"+'e']="rgba(0,0"+",0,.55)",
ck["font"]=bA(0x8,0x2bc),ck["fillText"]("O V R",0x2f,0x24),ck["fillStyl"+'e']="#16110a",ck["font"]=bA(0x1f,0x320),ck["fillText"](String(bW["maxOvr"]),
0x2f,0x44);
var cl=0x58;
[{'t':bW["originNa"+'me']},{'t':am(an(bW["maxOvr"],0x1a)),'sub':"最高身价"},{'t':'#'+bW["number"]},{'t':bW["posId"],'accent':!0x0}]["forEach"](function(cF){var eA=ey;
ck["font"]=bA(cF["sub"]?0xb:0xc,0x2ee);
var cG=ck["measureT"+"ext"](cF['t'])["width"]+0x12;
cF["sub"]&&(cG+=0x4),ck["fillStyl"+'e']=cF["accent"]?"rgba(74,"+"222,128,"+".16)":"rgba(255"+",255,255"+",.06)",bz(ck,cl,0x10,
cG,0x1a,0x7),ck["fill"](),ck["fillStyl"+'e']=cF["accent"]?"#4ade80":"#e6ece4",ck["textAlig"+'n']="center",ck["fillText"](cF['t'],
cl+cG/0x2,0x22),cl+=cG+0x6;
});
var cm=bu-c6-0x58;
ck["fillStyl"+'e']="rgba(255"+",255,255"+",.045)",bz(ck,0x58,0x30,cm,0x2a,0x8),ck["fill"](),('gk'===bY?[['出场',bW["apps"]],['零封',bW['cs']],['失球',bW['ga']]]:[['出场',bW["apps"]],['进球',bW["goals"]],['助攻',bW["assists"]]])["forEach"](function(cF,cG){var eB=ey,cH=cm/0x3,cI=0x58+cH*cG+cH/0x2;
ck["textAlig"+'n']="center",ck["fillStyl"+'e']="rgba(255"+",255,255"+",.42)",ck["font"]=bA(0x8,0x2bc),ck["fillText"](cF[0x0]["split"]('')["join"]('\x20'),
cI,0x3f),ck["fillStyl"+'e']="#f2f6ef",ck["font"]=bA(0x11,0x320),ck["fillText"](String(cF[0x1]),cI,0x52),cG&&(ck["strokeSt"+"yle"]="rgba(255"+",255,255"+",.08)",
ck["beginPat"+'h'](),ck["moveTo"](0x58+cH*cG,0x38),ck["lineTo"](0x58+cH*cG,0x52),ck["stroke"]());
});
var cn,co=bW["seasons"]+" 个赛季 · "+bW["clubCoun"+'t']+" 家俱乐部";
if(ck["textAlig"+'n']="left",bw){ck["font"]=bA(0xf,0x320);
var cp=ck["measureT"+"ext"](bW["name"])["width"];
ck["fillStyl"+'e']="#f2f6ef",ck["fillText"](bW["name"],c6,c7),ck["font"]=bA(0xa,0x258),ck["fillStyl"+'e']="rgba(255"+",255,255"+",.4)",
cn=c6+cp+0x9,ck["fillText"](co,cn,c7);
}else ck["font"]=bA(0xb,0x28a),ck["fillStyl"+'e']="rgba(255"+",255,255"+",.5)",cn=c6,ck["fillText"](co,cn,c7);
if((bW["gen"]||0x1)>0x1){var cq='第\x20'+bW["gen"]+'\x20世';
ck["font"]=bw?bA(0xa,0x258):bA(0xb,0x28a);
var cr=cn+ck["measureT"+"ext"](co)["width"]+0x8;
ck["font"]=bA(0xa,0x320);
var cs=ck["measureT"+"ext"](cq)["width"]+0xc;
ck["fillStyl"+'e']="rgba(226"+",180,86,"+".18)",bz(ck,cr,0x63,cs,0xf,0x5),ck["fill"](),ck["strokeSt"+"yle"]="rgba(226"+",180,86,"+".55)",
ck["lineWidt"+'h']=0x1,bz(ck,cr+0.5,99.5,cs-0x1,0xe,0x5),ck["stroke"](),ck["fillStyl"+'e']="#e8c477",ck["textAlig"+'n']="center",
ck["fillText"](cq,cr+cs/0x2,c7),ck["textAlig"+'n']="left";
}if(bZ["length"]&&(bB(ck,"效力过",c8+0xa),bZ["forEach"](function(cF,cG){var eC=ey,cH=Math["floor"](cG/ca),cI=Math["min"](ca,bZ["length"]-cH*ca),cJ=(bu-(0x28*cI+0xa*(cI-0x1)))/0x2,cK=c3[cG],cL=cJ+cG%ca*0x32,cM=c8+0x14+0x32*cH;
if(cK){var cN=Math["min"](0x28/cK["width"],0x28/cK["height"]);
ck["drawImag"+'e'](cK,cL+(0x28-cK["width"]*cN)/0x2,cM+(0x28-cK["height"]*cN)/0x2,cK["width"]*cN,cK["height"]*cN);
}else ck["fillStyl"+'e']=cF["color"]||"#39423a",bz(ck,cL+0x4,cM+0x4,0x20,0x20,0x8),ck["fill"]();
})),c0["length"]){bB(ck,'荣誉',cb+0xa);
var cu=ce;
c0["forEach"](function(cF,cG){var eD=ey,cH,cI,cJ=Math["floor"](cG/cu),cK=Math["min"](cu,c0["length"]-cJ*cu),cL=(bu-cK*cf)/0x2+cG%cu*cf,cM=cb+0x14+0x3e*cJ,cN=c4[cG];
if(cN){var cO=0x26/cN["height"];
ck["drawImag"+'e'](cN,cL+(cf-cN["width"]*cO)/0x2,cM,cN["width"]*cO,0x26);
}if(cF["count"]>0x1){ck["font"]=bA(0x9,0x320);
var cP='×'+cF["count"],cQ=ck["measureT"+"ext"](cP)["width"]+0xa,cR=Math["min"](cL+cf/0x2+0x8,cL+cf-cQ-0x1);
ck["fillStyl"+'e']="#12140f",bz(ck,cR,cM+0x18,cQ,0xe,0x7),ck["fill"](),ck["strokeSt"+"yle"]="rgba(255"+",255,255"+",.28)",
ck["lineWidt"+'h']=0x1,bz(ck,cR+0.5,cM+24.5,cQ-0x1,0xd,0x7),ck["stroke"](),ck["fillStyl"+'e']="#e6ece4",ck["textAlig"+'n']="center",
ck["fillText"](cP,cR+cQ/0x2,cM+0x22);
}ck["textAlig"+'n']="center",ck["fillStyl"+'e']="rgba(255"+",255,255"+",.6)",ck["font"]=bA(0x8,0x28a),ck["fillText"]((cH=cF["name"],
(cI=String(cH||'')["replace"](/冠军$/,''))["length"]>0x6?cI["slice"](0x0,0x5)+'…':cI),cL+cf/0x2,cM+0x32);
}),ck["textAlig"+'n']="left";
}var cv=bu-0x20,cw=ck["createLi"+"nearGrad"+"ient"](0x0,cg,0x0,cg+0x3a);
cw["addColor"+"Stop"](0x0,"rgba(226"+",180,86,"+".22)"),cw["addColor"+"Stop"](0x1,"rgba(226"+",180,86,"+".05)"),ck["fillStyl"+'e']=cw,
bz(ck,0x10,cg,cv,0x3a,0xa),ck["fill"](),ck["strokeSt"+"yle"]="rgba(226"+",180,86,"+".55)",ck["lineWidt"+'h']=0x1,bz(ck,16.5,
cg+0.5,cv-0x1,0x39,0xa),ck["stroke"]();
var cx=ck["createLi"+"nearGrad"+"ient"](0x10,0x0,0x10+cv,0x0);
cx["addColor"+"Stop"](0x0,"rgba(226"+",180,86,"+'0)'),cx["addColor"+"Stop"](0.5,"rgba(255"+",214,130"+",.95)"),cx["addColor"+"Stop"](0x1,
"rgba(226"+",180,86,"+'0)'),ck["strokeSt"+"yle"]=cx,ck["beginPat"+'h'](),ck["moveTo"](0x1c,cg+0.5),ck["lineTo"](0x10+cv-0xc,
cg+0.5),ck["stroke"](),ck["textAlig"+'n']="center";
var cy=bW["gen"]||0x1;
ck["fillStyl"+'e']="rgba(226"+",180,86,"+".8)",ck["font"]=bA(0x8,0x2ee),ck["fillText"](cy>0x1?'第\x20'+cy+" 世 · 结 局":"结 局",
bu/0x2,cg+0x12);
var cz=0x15;
for(ck["font"]=bA(cz,0x320);
cz>0xd&&ck["measureT"+"ext"](bW["endingTi"+"tle"])["width"]>cv-0x1c;
)cz-=0x1,ck["font"]=bA(cz,0x320);
var cA=ck["createLi"+"nearGrad"+"ient"](0x0,cg+0x18,0x0,cg+0x30);
cA["addColor"+"Stop"](0x0,"#ffe9b0"),cA["addColor"+"Stop"](0.55,"#e2b456"),cA["addColor"+"Stop"](0x1,"#c68f33"),ck["shadowCo"+"lor"]="rgba(226"+",180,86,"+".6)",
ck["shadowBl"+'ur']=0x10,ck["fillStyl"+'e']=cA,ck["fillText"](bW["endingTi"+"tle"],bu/0x2,cg+0x2c),ck["shadowBl"+'ur']=0x0,
ck["shadowCo"+"lor"]="transpar"+"ent",ck["textAlig"+'n']="left",ck["strokeSt"+"yle"]="rgba(255"+",255,255"+",.08)",ck["lineWidt"+'h']=0x1,
ck["beginPat"+'h'](),ck["moveTo"](c6,ch),ck["lineTo"](bu-c6,ch),ck["stroke"]();
var cB=function(cF,cG,cH,cI){var eE=ey;
if(!cG||!window['QR'])return 0x0;
var cJ=window['QR']["make"](cG);
if(!cJ)return 0x0;
var cK=cJ["length"],cL=Math["floor"](0x2e*bv/(cK+0x4))/bv;
if(cL<=0x0)return 0x0;
var cM=cL*(cK+0x4);
cF["fillStyl"+'e']="#ffffff",bz(cF,cH,cI,cM,cM,0x3),cF["fill"](),cF["fillStyl"+'e']="#12140f";
for(var cN=0x0;
cN<cK;
cN++)for(var cO=0x0;
cO<cK;
cO++)cJ[cN][cO]&&cF["fillRect"](cH+(cO+0x2)*cL,cI+(cN+0x2)*cL,cL,cL);
return cM;
}(ck,(function(){var eF=ey;
if(a5)return "https://"+a5+'/';
try{return location["origin"]&&"null"!==location["origin"]?(location["origin"]+location["pathname"])["replace"](/index\.html$/,''):'';
}catch(cF){return'';
}}()),bu-c6-0x2e,ch+0x9),cC=cB?bu-c6-cB-0xe:bu-c6;
ck["textAlig"+'n']="left",ck["fillStyl"+'e']="rgba(255"+",255,255"+",.42)",ck["font"]=bA(0x9,0x258),ck["fillText"]("来走一遍你自己的"+'生涯',
c6,ch+0x14),ck["fillStyl"+'e']="rgba(255"+",255,255"+",.28)",ck["font"]=bA(0x8,0x258),ck["fillText"]("bilibili"+" · 神来我也日",
c6,ch+0x21),ck["textAlig"+'n']="right";
var cD=(function(){var eG=ey;
if(a5)return a5;
try{var cF=location["hostname"];
return cF&&"localhos"+'t'!==cF&&"127.0.0."+'1'!==cF?cF:a3;
}catch(cG){return a3;
}}()),cE=cD!==a3;
ck["fillStyl"+'e']="#e6ece4",ck["font"]=bA(8.5,0x2ee),ck["fillText"](a3,cC,ch+(cE?0x18:0x1e)),cE&&(ck["fillStyl"+'e']="#4ade80",
ck["font"]=bA(7.5,0x2bc),ck["fillText"](cD,cC,ch+0x24)),bx=null,(function(){var eH=ey,cF=aw("share-ca"+"nvas"),cG=aw("share-im"+'g'),cH=aw("share-ti"+'p');
if(cF&&cG&&cF["toDataUR"+'L']&&bF()){try{cG["src"]=cF["toDataUR"+'L']("image/pn"+'g');
}catch(cI){return;
}cF["classLis"+'t']["add"]("hidden"),cG["classLis"+'t']["remove"]("hidden"),cH&&(cH["textCont"+"ent"]="长按上面这张图 "+"→ 发送给朋友 "+"/ 保存图片",
cH["classLis"+'t']["remove"]("hidden"));
}}());
});
}function bF(){var eI=d3;
try{return/micromessenger/i["test"](navigator["userAgen"+'t']);
}catch(bW){return!0x1;
}}var bG=null;
function bH(bW){var eJ=d3;
bG=bW||bC(),bw=!0x0;
var bX=aw("share-na"+'me');
bX&&(bX["checked"]=!0x0);
var bY=aw("share-ey"+"ebrow");
return bY&&(bY["textCont"+"ent"]=(bG["gen"]||0x1)>0x1?'第\x20'+bG["gen"]+(" 世 · 生涯结"+'束'):"生涯结束"),function(bZ){var eK=eJ,c0=aw("share-ne"+"xtlife");
if(c0){var c1=bZ&&bZ["legacy"];
if(c0["classLis"+'t']["toggle"]("hidden",!c1),c1){var c2=[];
c1["ovr"]&&c2["push"]("能力 +"+c1["ovr"]),c1["talent"]&&c2["push"]("天赋 +"+c1["talent"]["toFixed"](0x2)),c1["guanxi"]&&c2["push"]("关系 +"+c1["guanxi"]),
c1["money"]&&c2["push"]("家底 +"+al(c1["money"])),c0["innerHTM"+'L']="<button "+"class=\"b"+"tn btn-p"+"rimary\" "+"data-act"+"=\"card-n"+"ext-life"+"\">接着这一世，"+"投第 "+c1["gen"]+(" 世</butt"+"on><div "+"class=\"l"+"egacy-no"+"te\">")+(c2["length"]?"带走 "+ax(c2["join"](" · ")):"这一世没留下什么"+"，但可以接着往下"+'投')+"</div>";
}}}(bG),bI(bG),aw("share-mo"+"dal")["classLis"+'t']["remove"]("hidden"),bE(bG);
}function bI(bW){var eL=d3,bX=aw("share-en"+"ding-pic"+'k');
if(bX){var bY=bW&&bW["endingCh"+"oices"]||[];
bX["classLis"+'t']["toggle"]("hidden",bY["length"]<0x2),bY["length"]<0x2||(bX["innerHTM"+'L']="<div cla"+"ss=\"sep-"+"label\">卡"+"上印哪一条结局<"+"/div><di"+"v class="+"\"ep-list"+'\x22>'+bY["map"](function(bZ,c0){var eM=eL;
return "<button "+"class=\"e"+"p-btn"+(bZ["title"]===bW["endingTi"+"tle"]?" selecte"+'d':'')+("\" data-e"+"p=\"")+c0+'\x22>'+ax(bZ["title"])+("</button"+'>');
})["join"]('')+"</div>");
}}function bJ(bW){var eN=d3,bX=(bW["honours"]||[])["map"](function(bY){var eO=eN;
return bY["name"]+(bY["count"]>0x1?'×'+bY["count"]:'');
})["join"]('\x20');
return['【'+a3+'】'+bW["name"]+((bW["gen"]||0x1)>0x1?"（第 "+bW["gen"]+" 世）":'')+" · "+bW["posName"]+" · "+bW["originNa"+'me'],
"16 → "+bW["age"]+" 岁 · "+bW["seasons"]+" 个赛季 · "+bW["clubCoun"+'t']+" 家俱乐部","最高能力 "+bW["maxOvr"]+" · 身家 "+al(bW["money"]),
'gk'===bW["group"]?"出场 "+bW["apps"]+" · 零封 "+bW['cs']+" · 失球 "+bW['ga']:"出场 "+bW["apps"]+" · 进球 "+bW["goals"]+" · 助攻 "+bW["assists"],
"国家队 "+bW["caps"]+'\x20次'+((bW["nat"]||[])["length"]?'（'+bW["nat"]["map"](function(bY){var eP=eN;
return bY["comp"]+(aa[bY["stage"]]||bY["stage"]);
})["join"](" · ")+'）':''),bX?"荣誉："+bX:"荣誉：一个没有","结局："+bW["endingTi"+"tle"],a3+(" · bilib"+"ili 神来我也"+'日')]["join"]('\x0a');
}function bK(bW){var eQ=d3,bX=document["createEl"+"ement"]("div");
bX["classNam"+'e']="toast",bX["textCont"+"ent"]=bW,document["body"]["appendCh"+"ild"](bX),setTimeout(function(){var eR=eQ;
bX["remove"]();
},0x708);
}var bL=null;
function bM(bW){var eS=d3;
["view-int"+'ro',"view-ide"+"ntity","view-car"+"eer","view-sum"+"mary","view-arc"+"hive","view-cod"+'ex']["forEach"](function(bY){var eT=eS;
aw(bY)["classLis"+'t']["toggle"]("hidden",bY!==bW);
}),"view-int"+'ro'===bW&&(function(){var eU=eS,bY=aw("btn-arch"+"ive");
if(bY){var bZ=aG()["length"];
bY["classLis"+'t']["toggle"]("hidden",0x0===bZ),bY["textCont"+"ent"]=bZ?"生涯历史档案 ·"+'\x20'+bZ+'\x20段':"生涯历史档案";
var c0=aw("btn-code"+'x');
c0&&(c0["classLis"+'t']["toggle"]("hidden",0x0===bZ),c0["textCont"+"ent"]="结局图鉴 · "+aH(aG())["endings"]+" / "+a0["ENDINGS"]["length"]);
}}());
var bX="view-ide"+"ntity"===bW;
aw("actionba"+'r')["classLis"+'t']["toggle"]("hidden",!bX),document["body"]["classLis"+'t']["toggle"]("no-actio"+"nbar",!bX),
document["body"]["classLis"+'t']["toggle"]("in-caree"+'r',"view-car"+"eer"===bW),aw("app")["classLis"+'t']["toggle"]("wide",
"view-car"+"eer"===bW||"view-sum"+"mary"===bW||"view-cod"+'ex'===bW),aw("app")["classLis"+'t']["toggle"]("intro-wi"+'de',"view-int"+'ro'===bW),
bW!==bL&&(bL=bW,window["scrollTo"](0x0,0x0));
}function bN(){var eV=d3,bW=b2;
bM("view-car"+"eer"),aw("career-r"+"oot")["innerHTM"+'L']="<div cla"+"ss=\"care"+"er-grid\""+"><div cl"+"ass=\"col"+"-a\">"+(function(){var eW=eV,bX=ai(),bY=(aj(),ah(au["pos"])),bZ=bY["group"],c0="<div cla"+"ss=\"play"+"er-card\""+"><div cl"+"ass=\"her"+"o-row\"><"+"div clas"+"s=\"ovr-b"+"adge "+b5(au["ovr"])+("\"><div c"+"lass=\"ov"+"r-badge-"+"l\">OVR</"+"div><div"+" class=\""+"ovr-badg"+"e-v\">")+Math["round"](au["ovr"])+("</div></"+"div><div"+" class=\""+"club-bar"+"\"><div c"+"lass=\"cl"+"ub-bar-m"+"ain\"><di"+"v class="+"\"tag-row"+"\"><span "+"class=\"t"+"ag\">")+ax(ak(au["originId"])["name"])+("</span><"+"span cla"+"ss=\"tag "+"blue\">#")+au["number"]+'\x20'+bY['id']+("</span><"+"/div><di"+"v class="+"\"player-"+"name\">")+ax(au["name"])+("</div><d"+"iv class"+"=\"club-l"+"ine\">")+(bX?aT(bX):'')+("<span cl"+"ass=\"clu"+"b-name-b"+"ig\">")+ax(bX?bX["name"]:"自由身")+"</span>"+(au["loanFrom"]&&ag(au["loanFrom"])?"<span cl"+"ass=\"loa"+"n-tag\">租"+"借自 "+ax(ag(au["loanFrom"])["name"])+"</span>":'')+("</div></"+"div><div"+" class=\""+"club-rig"+"ht\"><div"+" class=\""+"cr-l\">年龄"+"</div><d"+"iv class"+"=\"cr-v\">")+au["age"]+("</div><d"+"iv class"+"=\"cr-l\" "+"style=\"m"+"argin-to"+"p:.25rem"+'\x22>')+("youth"===au["phase"]?'青训':'身价')+("</div><d"+"iv class"+"=\"cr-v")+("youth"===au["phase"]?" sm":'')+'\x22>'+("youth"===au["phase"]?ax(b7()||"还没定"):am(an(au["ovr"],au["age"])))+("</div></"+"div></di"+"v></div>");
if(c0+="<div cla"+"ss=\"stat"+"-row\">"+('gk'===bZ?[[b4["apps"]+'出场',au["totals"]["apps"]],[b4['cs']+'零封',au["totals"]['cs']],[b4['ga']+'失球',au["totals"]['ga']]]:[[b4["apps"]+'出场',au["totals"]["apps"]],[b4["goals"]+'进球',au["totals"]["goals"]],[b4["ast"]+'助攻',au["totals"]["assists"]]])["map"](function(c4){var eX=eW;
return "<div cla"+"ss=\"stat"+"-cell\"><"+"div clas"+"s=\"stat-"+"l\">"+c4[0x0]+("</div><d"+"iv class"+"=\"stat-v"+'\x22>')+c4[0x1]+("</div></"+"div>");
})["join"]('')+("</div></"+"div>"),bX){var c1=a0["ROLES"][au["role"]];
c0+="<div cla"+"ss=\"stat"+"us-row\">"+"<div cla"+"ss=\"st-c"+"ell\"><sp"+"an class"+"=\"st-l\">"+"队内地位</sp"+"an><span"+" class=\""+"st-v rol"+'e-'+au["role"]+("\" title="+"\"这一档一个赛季"+"大约 ")+c1["apps"][0x0]+'-'+c1["apps"][0x1]+" 场\">"+ax(c1["name"])+("</span><"+"/div><di"+"v class="+"\"st-cell"+"\"><span "+"class=\"s"+"t-l\">合同<"+"/span><s"+"pan clas"+"s=\"st-v\""+'>')+(au["contract"+"Left"]>0x0?"还剩 "+au["contract"+"Left"]+'\x20期':"本期到期")+("</span><"+"/div></d"+"iv>");
}c0+="<div cla"+"ss=\"mete"+"rs\"><div"+" class=\""+"bars\">"+b6('关系',au["guanxi"],"hsl(var("+"--info))")+b6('清白',au["clean"],
"hsl(var("+"--accent"+'))')+b6('名气',au["fame"],"hsl(var("+"--warnin"+"g))")+("</div><d"+"iv class"+"=\"money-"+"row\"><sp"+"an>")+(au["money"]<0x0?"家里的欠债":"个人财富")+("</span><"+'b')+(au["money"]<0x0?" class=\""+"neg\"":'')+'>'+al(au["money"])+("</b></di"+"v></div>");
var c2=a6["STAFF"]["filter"](function(c4){var eY=eW;
return au["staff"]&&au["staff"][c4['id']];
});
c2["length"]&&(c0+="<div cla"+"ss=\"staf"+"f-row\"><"+"div clas"+"s=\"staff"+"-row-l\">"+"团队<b>"+al(a6["staffFee"]())+(" / 赛季</b"+"></div><"+"div clas"+"s=\"staff"+"-chips\">")+c2["map"](function(c4){var eZ=eW;
return "<span cl"+"ass=\"chi"+"p\" title"+'=\x22'+ax(c4["desc"])+'\x22>'+ax(c4["name"])+"</span>";
})["join"]('')+("</div></"+"div>"));
var c3=(function(){var f0=eW,c4='',c5={},c6={};
return au["trophies"]["forEach"](function(c7){var f1=f0;
c5[c7["name"]]=(c5[c7["name"]]||0x0)+0x1;
}),au["awards"]["forEach"](function(c7){var f2=f0;
c6[c7["name"]]=(c6[c7["name"]]||0x0)+0x1;
}),Object["keys"](c5)["forEach"](function(c7){var f3=f0;
c4+="<span cl"+"ass=\"chi"+"p\">🏆 "+ax(c7)+(c5[c7]>0x1?'\x20×'+c5[c7]:'')+"</span>";
}),Object["keys"](c6)["forEach"](function(c7){var f4=f0;
c4+="<span cl"+"ass=\"chi"+"p award\""+">🥇 "+ax(c7)+(c6[c7]>0x1?'\x20×'+c6[c7]:'')+"</span>";
}),c4;
}());
return(c0+="<div cla"+"ss=\"trop"+"hy-block"+(c3?'':" empty")+'\x22>'+(c3?"<div cla"+"ss=\"shel"+"f\">"+c3+"</div>":"<div cla"+"ss=\"empt"+"y-case\">"+"<div cla"+"ss=\"empt"+"y-case-i"+"con\">🏆</"+"div><div"+" class=\""+"empty-ca"+"se-t\">奖杯"+"柜是空的</di"+"v></div>")+"</div>")+("<div cla"+"ss=\"rest"+"art-row\""+"><button"+" class=\""+"mini-btn"+"\" data-a"+"ct=\"rest"+"art\">从头来"+"过</butto"+"n><span>"+"重新选出身，开一"+"局新的</spa"+"n></div>");
}())+("</div><d"+"iv class"+"=\"col-b\""+'>')+b9()+("</div><d"+"iv class"+"=\"col-c\""+'>')+(function(){var f5=eV,bX,bY,bZ=au["pending"];
if(!bZ)return'';
if("random"===bZ["type"]){for(var c0=null,c1=0x0;
c1<a1["length"];
c1++)a1[c1]['id']===bZ["eventId"]&&(c0=a1[c1]);
if(!c0)return'';
var c2="<div cla"+"ss=\"even"+"t\">"+b1(c0["icon"]||'⚽',c0['cn']?"国内 · 事件":'事件',ao(c0["title"]))+("<div cla"+"ss=\"ev-d"+"esc\">")+ax(ao(c0["desc"]))+"</div>";
return bZ["roll"]&&(c2+=function(ce){var f6=f5,cf=Math["round"](0x64*ce['p']);
function cg(ch,ci,cj){var f7=f6,ck="slot";
return ce["spinning"]||(ck+='a'===ch==!!ce['ok']?" win":" lose"),"<div cla"+"ss=\""+ck+("\" data-s"+"ide=\"")+ch+("\"><span "+"class=\"s"+"lot-pct\""+'>')+ci+("%</span>"+"<span cl"+"ass=\"slo"+"t-lab\">")+ax(cj)+("</span><"+"/div>");
}return "<div cla"+"ss=\"roul"+"ette\" da"+"ta-state"+'=\x22'+(ce["spinning"]?"spin":"done")+'\x22>'+cg('a',cf,ce['a'])+("<span cl"+"ass=\"rou"+"lette-vs"+'\x22>')+(ce["spinning"]?"抽签中":'落定')+"</span>"+cg('b',
0x64-cf,ce['b'])+"</div>";
}(bZ["roll"])),bZ["result"]?(c2+="<div cla"+"ss=\"resu"+"lt\">"+ax(bZ["result"]["text"])+"</div>",bZ["result"]["deltas"]&&bZ["result"]["deltas"]["length"]&&(c2+="<div cla"+"ss=\"delt"+"as\">"+bZ["result"]["deltas"]["map"](function(ce){var f8=f5;
return "<span cl"+"ass=\"del"+"ta "+ce["cls"]+'\x22>'+ax(ce["text"])+"</span>";
})["join"]('')+"</div>"),c2+=bb(["<button "+"class=\"b"+"tn btn-p"+"rimary\" "+"data-act"+"=\"contin"+"ue\">继续</"+"button>"])):bZ["roll"]||(c2+=bb((bX=c0["options"]["map"](function(ce,
cf){var f9=f5;
return ba(cf,ao(ce["label"]),ao(a6["optHint"](c0,cf)));
}),bY="ev:"+c0['id']+':'+au["choices"]["length"],function(ce,cf){var fa=f5;
if(bc["key"]!==cf||!bc["ord"]||bc["ord"]["length"]!==ce){var cg,ch=[];
for(cg=0x0;
cg<ce;
cg++)ch["push"](cg);
for(cg=ce-0x1;
cg>0x0;
cg--){var ci=Math["floor"](Math["random"]()*(cg+0x1)),cj=ch[cg];
ch[cg]=ch[ci],ch[ci]=cj;
}bc={'key':cf,'ord':ch};
}return bc["ord"];
}(bX["length"],bY)["map"](function(ce){return bX[ce];
})))),c2+"</div>";
}if("report"===bZ["type"]){var c3=bZ["recs"][0x0]["age"],c4=bZ["recs"][bZ["recs"]["length"]-0x1]["age"];
return "<div cla"+"ss=\"even"+"t\">"+b1('📋','战报',c3===c4?c3+'\x20岁':c3+'–'+c4+'\x20岁')+("<div cla"+"ss=\"seas"+"on-mini\""+'>')+bZ["recs"]["map"](function(ce){var fb=f5,cf='gk'===ah(au["pos"])["group"]?ce["apps"]+" 场 / "+ce['cs']+" 零封":ce["apps"]+" 场 / "+ce["goals"]+" 球 / "+ce["assists"]+'\x20助',cg='';
ce["caps"]&&(cg="国家队 "+ce["caps"]+'\x20场','gk'===ah(au["pos"])["group"]?ce["natCs"]&&(cg+=" · "+ce["natCs"]+" 零封"):(ce["natGoals"]&&(cg+=" · "+ce["natGoals"]+'\x20球'),ce["natAssis"+'ts']&&(cg+=" · "+ce["natAssis"+'ts']+'\x20助')));
var ch=(cg?" <span c"+"lass=\"mi"+"ni-badge"+" nat\">"+ax(cg)+"</span>":'')+(ce["note"]?" <span c"+"lass=\"mi"+"ni-badge"+" bad\">"+ax(ce["note"])+"</span>":'')+(ce["nat"]?" <span c"+"lass=\"mi"+"ni-badge"+" nat\">"+ax(ce["nat"])+"</span>":'')+(ce["move"]?" <span c"+"lass=\"mi"+"ni-badge"+'\x20'+(0x0===ce["move"]["indexOf"]('升')?'up':"bad")+'\x22>'+ax(ce["move"])+"</span>":'')+(ce["staffGon"+'e']?" <span c"+"lass=\"mi"+"ni-badge"+" bad\">"+ax(ce["staffGon"+'e'])+("走了</span"+'>'):'')+(ce["trophies"]["length"]?" <span c"+"lass=\"mi"+"ni-badge"+'\x22>'+ax(ce["trophies"]["join"]('\x20'))+"</span>":'');
return "<div cla"+"ss=\"tl-r"+"ow done "+"tl-cols\""+" style=\""+"grid-tem"+"plate-co"+"lumns:2."+"4rem 1fr"+" auto\"><"+"span cla"+"ss=\"age-"+"chip\">"+ce["age"]+("</span><"+"span cla"+"ss=\"tl-c"+"lub\"><sp"+"an class"+"=\"tl-clu"+"b-name\">")+ax(ce["teamName"])+"</span>"+ch+("</span><"+"span cla"+"ss=\"tl-n"+'\x22>')+cf+" · "+ce["ovrEnd"]+("</span><"+"/div>");
})["join"]('')+"</div>"+bb(["<button "+"class=\"b"+"tn btn-p"+"rimary\" "+"data-act"+"=\"contin"+"ue\">继续</"+"button>"])+"</div>";
}if("youth_pa"+'th'===bZ["type"])return "<div cla"+"ss=\"even"+"t\">"+b1('🧒',"十二岁","去哪儿练",!0x0)+("<div cla"+"ss=\"ev-d"+"esc\">同龄的"+"孩子里，你踢得算"+"好的那几个。接下"+"来几年在哪家青训"+"营，家里得拿个主"+"意 —— 好队伍"+"练得快，也刷得狠"+"。</div>")+bb(bZ["offers"]["map"](function(ce,cf){var fc=f5,cg=ag(ce),ch=a6["leagueOf"+"Team"](cg),ci=cf===bZ["abroadId"+'x'];
return "<button "+"class=\"o"+"pt youth"+"-opt\" da"+"ta-opt=\""+cf+("\"><span "+"class=\"o"+"pt-lead\""+'>')+(ci?"送出国踢":ac(au["originId"],ce)?'本省':ab(au["originId"],ce)?'邻近':'外地')+"</span>"+aT(cg)+("<span cl"+"ass=\"opt"+"-label\">")+ax(a6["academyN"+"ame"](cg))+("</span><"+"span cla"+"ss=\"opt-"+"hint\">")+ax(ch["name"]+" · "+["保级队","中下游",'中游','争冠','豪门',"顶级豪门"][cg["rep"]])+("</span><"+"span cla"+"ss=\"yout"+"h-trade\""+'>')+["清静，练得慢",
'稳当',"练得不错","强度大","长得快，刷得狠","顶尖，也最挤"][cg["rep"]]+(ci?" · 家里出 "+al(a0["YOUTH_AB"+"ROAD_FEE"]):'')+("</span><"+"/button>");
}))+"</div>";
if("academy"===bZ["type"]){var c5=bZ["youthId"]?ag(bZ["youthId"]):null,c6="<div cla"+"ss=\"even"+"t\">"+b1('🎒','决策',"该上成年队了",!0x0)+("<div cla"+"ss=\"ev-d"+"esc\">")+au["age"]+(" 岁，能力够得着"+"成年队了。")+(bZ["homeId"]?"自家一线队也开了"+'口。':'')+(bZ["canStayY"+"outh"]?"也可以不急着走，"+"在梯队再练一年。":'')+"</div>"+bb(bZ["offers"]["map"](function(ce,cf){var fd=f5;
return bg(cf,ag(ce),ag(ce)["name"],ce===bZ["homeId"]?"自家一线队":'加盟',!0x0);
}));
return bZ["canStayY"+"outh"]&&(c6+=bb([ba("youth",'再在'+(c5?ax(a6["academyN"+"ame"](c5)):'梯队')+"练一年","不签成年队。梯队"+"练得更快，但明年"+"还要过一次选材那"+'一关')],"opts-alt")),
c6+"</div>";
}if("transfer"===bZ["type"]){var c7=bZ["backFrom"]?ag(bZ["backFrom"]):null,c8="<div cla"+"ss=\"even"+"t\">"+b1(c7?'🔙':bZ["fired"]?'🚪':bZ["mustLeav"+'e']?'🧳':'🔁',
'决策',c7?"租借期满，回到 "+c7["name"]:bZ["fired"]?"俱乐部不续约了":bZ["mustLeav"+'e']?"话已经说出口了":"转会窗",!0x0)+("<div cla"+"ss=\"ev-d"+"esc\">")+(c7?"你回来了。这一年"+"在外面踢的每一场"+"，这边都看在眼里"+" —— 接下来是"+"留是走，重新谈一"+'次。':bZ["fired"]?"你的出场时间已经"+"少到写不进总结。"+"合同到期，俱乐部"+"没有再谈的意思。":bZ["mustLeav"+'e']?"走这件事已经定了"+"，回头路没有了。"+"剩下的问题只是去"+"哪儿。":"合同到期了。这几"+"家来问过，也可以"+"留下。")+"</div>";
c8+=bb(bZ["offers"]["map"](function(ce,cf){var fe=f5;
return bg(cf,ag(ce),ag(ce)["name"],'加盟',!0x0);
})),bZ["loans"]&&bZ["loans"]["length"]&&(c8+="<div cla"+"ss=\"opts"+"-note\">在"+"这儿排不上号的话"+"，还可以租出去踢"+"一年</div>",c8+=bb(bZ["loans"]["map"](function(ce,
cf){var ff=f5;
return bg("loan"+cf,ag(ce),ag(ce)["name"],'租借',!0x0);
})));
var c9=[];
if(bZ["canStay"]){var ca=a6["offerBri"+'ef'](ai()['id']);
c9["push"](ba("stay","留在 "+ai()["name"],"续约，一切照旧 "+'·\x20'+al(ca["wage"])+"/赛季 · "+ca["roleName"],ai()));
}return bZ["canRetir"+'e']&&c9["push"](ba("retire",'挂靴',"就到这里")),bZ["offers"]["length"]||bZ["canStay"]||(c8+="<div cla"+"ss=\"empt"+"y\">没有任何报"+"价。</div>",
c9["length"]||c9["push"](ba("retire",'挂靴',"只剩这一个选项"))),c8+bb(c9,"opts-alt")+"</div>";
}if("bigmatch"===bZ["type"]){var cb="<div cla"+"ss=\"even"+"t bigmat"+'ch'+(bZ["result"]?bZ["result"]["won"]?" won":" lost":'')+'\x22>'+b1(bZ["icon"]||'🏆',
"关键战",bZ["comp"],!0x0)+("<div cla"+"ss=\"bm-v"+"s\"><span"+" class=\""+"bm-side\""+'>')+ax(bZ["side"])+("</span><"+"span cla"+"ss=\"bm-s"+"core\">")+(bZ["result"]?bZ["result"]["score"][0x0]+" : "+bZ["result"]["score"][0x1]:bZ["half"][0x0]+" : "+bZ["half"][0x1])+("</span><"+"span cla"+"ss=\"bm-s"+"ide\">")+ax(bZ["opp"])+("</span><"+"/div>")+(bZ["result"]&&bZ["result"]["pens"]?"<div cla"+"ss=\"bm-p"+"ens\">点球 "+bZ["result"]["pens"][0x0]+" : "+bZ["result"]["pens"][0x1]+"</div>":"<div cla"+"ss=\"bm-p"+"ens\">"+(bZ["result"]?'全场':'半场')+"</div>")+("<div cla"+"ss=\"bm-l"+"og\">")+bZ["log"]["map"](function(ce){var fg=f5;
return "<div cla"+"ss=\"bm-l"+"ine\">"+ax(ce)+"</div>";
})["join"]('')+"</div>";
return bZ["result"]?(cb+="<div cla"+"ss=\"bm-l"+"og secon"+"d\">"+bZ["result"]["log"]["map"](function(ce){var fh=f5;
return "<div cla"+"ss=\"bm-l"+"ine\">"+ax(ce)+"</div>";
})["join"]('')+"</div>",bZ["result"]["deltas"]&&bZ["result"]["deltas"]["length"]&&(cb+="<div cla"+"ss=\"delt"+"as\">"+bZ["result"]["deltas"]["map"](function(ce){var fi=f5;
return "<span cl"+"ass=\"del"+"ta "+ce["cls"]+'\x22>'+ax(ce["text"])+"</span>";
})["join"]('')+"</div>"),cb+=bb(["<button "+"class=\"b"+"tn btn-p"+"rimary\" "+"data-act"+"=\"contin"+"ue\">继续</"+"button>"])):cb+=bb(a6["BIG_OPTS"]["map"](function(ce){var fj=f5;
return ba(ce["key"],ce["label"],ce["hint"]);
})),cb+"</div>";
}if("staff"===bZ["type"]){var cc=a6["STAFF"]["filter"](function(ce){var fk=f5;
return au["staff"]&&au["staff"][ce['id']];
}),cd="<div cla"+"ss=\"even"+"t\">"+b1('🤝','决策',"有人想跟你签约",!0x0)+("<div cla"+"ss=\"ev-d"+"esc\">经纪公"+"司列了几个人。都"+"是按赛季付钱，签"+"了就每年从你账上"+"走。</div>")+(cc["length"]?"<div cla"+"ss=\"staf"+"f-cur\">现"+"在养着："+cc["map"](function(ce){var fl=f5;
return "<span cl"+"ass=\"chi"+"p\">"+ax(ce["name"])+"</span>";
})["join"]('')+"<b>"+al(a6["staffFee"]())+(" / 赛季</b"+"></div>"):'');
return cd+=bb(bZ["offers"]["map"](function(ce){var fm=f5,cf=a6["staffByI"+'d'](ce);
return "<button "+"class=\"o"+"pt staff"+"-opt\" da"+"ta-opt=\""+ax(ce)+("\"><span "+"class=\"o"+"pt-label"+'\x22>')+ax(cf["name"])+("</span><"+"span cla"+"ss=\"staf"+"f-fee\">")+al(a6["staffPri"+'ce'](cf))+(" / 赛季</s"+"pan><spa"+"n class="+"\"opt-hin"+"t\">")+ax(cf["desc"])+("</span><"+"span cla"+"ss=\"staf"+"f-note\">")+ax(cf["note"])+("</span><"+"/button>");
})),(cd+=bb([ba("skip","先不请","钱留着")],"opts-alt"))+"</div>";
}return "retire_f"+"orced"===bZ["type"]?"<div cla"+"ss=\"even"+"t\">"+b1('📵','决策',"没人来问了",!0x0)+("<div cla"+"ss=\"ev-d"+"esc\">经纪人"+"的电话半年没响过"+"。你自己也知道是"+"时候了。</di"+'v>')+bb([ba("retire",'挂靴',"结束职业生涯")])+"</div>":'';
}())+("</div></"+"div>"),function(bX){var fn=eV;
if(bX&&"function"==typeof requestAnimationFrame){var bY=b3(".ovr-bad"+'ge'),bZ=b3(".ovr-bad"+"ge-v"),c0=Math["round"](au["ovr"]);
bZ&&bY&&bX["ovr"]!==c0&&(function(c7,c8,c9){var ca=null;
requestAnimationFrame(function cb(cc){var fo=_gam_0b;
null===ca&&(ca=cc);
var cd=Math["min"](0x1,(cc-ca)/0x2ee);
cd=0x1-Math["pow"](0x1-cd,0x3),c7["textCont"+"ent"]=Math["round"](c8+(c9-c8)*cd),cd<0x1?requestAnimationFrame(cb):c7["textCont"+"ent"]=c9;
});
}(bZ,bX["ovr"],c0),bY["classLis"+'t']["add"](c0>bX["ovr"]?'up':"down"));
for(var c1,c2=[bX["guanxi"],bX["clean"],bX["fame"]],c3=(c1=aw("career-r"+"oot"))&&c1["querySel"+"ectorAll"]?c1["querySel"+"ectorAll"](".bar-f"):[],
c4=0x0;
c4<c3["length"]&&c4<c2["length"];
c4++){var c5=c3[c4],c6=c5["style"]["width"];
c5["style"]["transiti"+'on']="none",c5["style"]["width"]=ad(c2[c4],0x0,0x64)+'%',c5["offsetWi"+"dth"],c5["style"]["transiti"+'on']='',
c5["style"]["width"]=c6;
}}}(bW),(function(){var fp=eV,bX=b3(".tl-scro"+'ll');
if(bX&&bX["querySel"+"ector"]){var bY=bX["querySel"+"ector"](".tl-row."+"now");
if(bY&&bY["getBound"+"ingClien"+"tRect"]&&bX["getBound"+"ingClien"+"tRect"]){var bZ=bX["getBound"+"ingClien"+"tRect"](),c0=bY["getBound"+"ingClien"+"tRect"]();
bX["scrollTo"+'p']=Math["max"](0x0,bX["scrollTo"+'p']+(c0["bottom"]-bZ["bottom"])+0x4);
}}}()),(function(){var fq=eV;
if("function"==typeof requestAnimationFrame&&window["scrollBy"]){var bX=b3(".col-c ."+"event");
bX&&bX["getBound"+"ingClien"+"tRect"]&&requestAnimationFrame(function(){var fr=fq,bY=bX["getBound"+"ingClien"+"tRect"](),bZ=window["innerHei"+"ght"]||0x0,c0=bY["bottom"]-bZ+0xc;
c0<=0x4||bY["height"]>=bZ-0x28||("scrollBe"+"havior"in document["document"+"Element"]["style"]&&!be()?window["scrollBy"]({'top':c0,'left':0x0,'behavior':"smooth"}):window["scrollBy"](0x0,c0));
});
}}()),b2={'ovr':Math["round"](au["ovr"]),'guanxi':au["guanxi"],'clean':au["clean"],'fame':au["fame"]};
}function bO(){var fs=d3;
if(au)return "summary"===au["phase"]?(bM("view-sum"+"mary"),void(function(){var ft=fs,bW=aq(),bX=ah(au["pos"])["group"],bY=a0["endingVi"+'ew'](bm(au["ending"]),bX);
au["archived"]||((function(){var fu=ft,c9,ca,cb,cc=aG();
cc["unshift"]((c9=aq(),ca=bm(au["ending"]),cb=ah(au["pos"])["group"],{'at':Date["now"](),'legacy':a6["legacyFr"+'om'](c9),'gen':au["gen"]||0x1,'mode':au["mode"],'seed':au["seed"],'name':au["name"],'number':au["number"],'pos':au["pos"],'group':cb,'originId':au["originId"],'maxOvr':c9["maxOvr"],'age':au["age"],'seasons':au["seasons"]["length"],'apps':au["totals"]["apps"],'goals':au["totals"]["goals"],'assists':au["totals"]["assists"],'cs':au["totals"]['cs'],'ga':au["totals"]['ga'],'caps':au["caps"],'money':c9["money"],'earned':c9["careerEa"+"rnings"],'banned':!!au["banned"],'ending':ca['id'],'youthCut':au["youthCut"]||0x0,'youthPathName':b7(),'endings':au["endingsA"+'ll']&&au["endingsA"+'ll']["length"]?au["endingsA"+'ll']["slice"]():[ca['id']],'endingTitle':a0["endingVi"+'ew'](ca,cb)["title"],'clubs':bl()["map"](function(cd){var fv=fu;
return{'id':cd["teamId"],'name':cd["name"],'color':cd["color"],'apps':cd["apps"]};
}),'honours':bk(),'nat':ap()})),cc["length"]>aF&&(cc["length"]=aF),function(cd){var fw=fu;
az("archive",cd);
}(cc);
}()),au["archived"]=!0x0,aA());
var bZ,c0,c1,c2=au["trophies"]["filter"](function(c9){var fx=ft;
return/世界杯|亚洲杯/["test"](c9["name"]);
}),c3="<div cla"+"ss=\"sum-"+"top\"><di"+"v class="+"\"sum-mai"+"n\"><div "+"class=\"e"+"nding-ey"+"ebrow\">"+((au["gen"]||0x1)>0x1?"<span cl"+"ass=\"gen"+"-chip\">第"+'\x20'+au["gen"]+(" 世</span"+'>'):'')+("青训淘汰"===au["endReaso"+'n']?"没能走到那一步 "+'·\x20'+au["age"]+" 岁 · "+b7()+" 第 "+Math["max"](0x1,
au["age"]-0xb)+'\x20年':"生涯结束 · "+au["age"]+" 岁 · "+au["seasons"]["length"]+" 个赛季 · "+au["clubsPla"+"yed"]["length"]+" 家俱乐部")+("</div><d"+"iv class"+"=\"sum-he"+"ro\"><div"+" class=\""+"sum-hero"+"-main\"><"+"div clas"+"s=\"sum-n"+"ame\">")+ax(au["name"])+("</div><d"+"iv class"+"=\"tag-ro"+"w\"><span"+" class=\""+"tag\">")+ax(ak(au["originId"])["name"])+("</span><"+"span cla"+"ss=\"tag "+"blue\">#")+au["number"]+'\x20'+ah(au["pos"])['id']+("</span><"+"/div></d"+"iv><div "+"class=\"s"+"um-hero-"+"side\"><d"+"iv class"+"=\"cr-l\">"+"最高身价</di"+"v><div c"+"lass=\"cr"+"-v\">")+am(an(bW["maxOvr"],
0x1a))+("</div></"+"div><div"+" class=\""+"ovr-badg"+'e\x20')+b5(bW["maxOvr"])+("\"><div c"+"lass=\"ov"+"r-badge-"+"l\">OVR</"+"div><div"+" class=\""+"ovr-badg"+"e-v\">")+bW["maxOvr"]+("</div></"+"div></di"+'v>');
c3+="<div cla"+"ss=\"stat"+"-row\">"+('gk'===bX?[[b4["apps"]+'出场',au["totals"]["apps"]],[b4['cs']+'零封',au["totals"]['cs']],[b4['ga']+'失球',au["totals"]['ga']]]:[[b4["apps"]+'出场',au["totals"]["apps"]],[b4["goals"]+'进球',au["totals"]["goals"]],[b4["ast"]+'助攻',au["totals"]["assists"]]])["map"](function(c9){var fy=ft;
return "<div cla"+"ss=\"stat"+"-cell\"><"+"div clas"+"s=\"stat-"+"l\">"+c9[0x0]+("</div><d"+"iv class"+"=\"stat-v"+'\x22>')+c9[0x1]+("</div></"+"div>");
})["join"]('')+("</div></"+"div>"),c3+="<div cla"+"ss=\"sum-"+"card nat"+"\"><div c"+"lass=\"su"+"m-card-l"+"\">国家队</d"+"iv><div "+"class=\"s"+"um-card-"+"title\">中"+"国</div><"+"div clas"+"s=\"sum-c"+"ard-stat"+'\x22>'+au["caps"]+(" 次出场</di"+'v>')+(c0=au["natStats"]||{},
(c1='gk'===bX?c0['cs']?[c0['cs']+" 零封"]:[]:[c0["goals"]?c0["goals"]+'\x20球':'',c0["assists"]?c0["assists"]+" 助攻":'']["filter"](Boolean))["length"]?"<div cla"+"ss=\"nat-"+"prod\">"+c1["join"](" · ")+"</div>":'')+(ap()["length"]?"<div cla"+"ss=\"nat-"+"best\">"+ap()["map"](function(c9){var fz=ft;
return "<div><sp"+"an>"+ax(c9["comp"])+("</span><"+'b>')+ax(aa[c9["stage"]]||c9["stage"])+"</b><i>"+c9["age"]+(" 岁</i></"+"div>");
})["join"]('')+"</div>":"<div cla"+"ss=\"case"+"-empty\">"+(au["caps"]?"没赶上过大赛":"没进过国家队")+"</div>")+(c2["length"]?"<div cla"+"ss=\"trop"+"hy-row\">"+c2["map"](function(c9){var fA=ft;
return "<span cl"+"ass=\"tro"+"phy-item"+'\x22>'+bj(c9["name"])+"<span>"+ax(c9["name"])+("</span><"+"/span>");
})["join"]('')+"</div>":'')+("</div><d"+"iv class"+"=\"sum-ca"+"rd award"+"s\"><div "+"class=\"s"+"um-card-"+"l\">个人奖项<"+"/div>")+(au["awards"]["length"]?"<div cla"+"ss=\"trop"+"hy-row\">"+(bZ={},
au["awards"]["forEach"](function(c9){var fB=ft;
bZ[c9["name"]]=(bZ[c9["name"]]||0x0)+0x1;
}),Object["keys"](bZ)["map"](function(c9){var fC=ft;
return "<span cl"+"ass=\"tro"+"phy-item"+'\x22>'+bj(c9)+"<span>"+ax(c9)+(bZ[c9]>0x1?'\x20×'+bZ[c9]:'')+("</span><"+"/span>");
})["join"]('')+"</div>"):"<div cla"+"ss=\"case"+"-empty\">"+"奖杯柜是空的</"+"div>")+"</div>",c3+="</div>";
var c4=bk();
c3+="<div cla"+"ss=\"sect"+"ion-head"+"\"><h2>奖杯"+"陈列</h2><"+"span>"+(c4["length"]?au["trophies"]["length"]+au["awards"]["length"]+'\x20座':'')+("</span><"+"/div>"),
c3+=c4["length"]?"<div cla"+"ss=\"trop"+"hy-hall\""+'>'+c4["map"](function(c9){var fD=ft;
return "<div cla"+"ss=\"hall"+"-item\">"+bj(c9["name"])+(c9["count"]>0x1?"<span cl"+"ass=\"hal"+"l-count\""+'>×'+c9["count"]+"</span>":'')+("<span cl"+"ass=\"hal"+"l-name\">")+ax(c9["name"])+("</span><"+"/div>");
})["join"]('')+"</div>":"<div cla"+"ss=\"trop"+"hy-hall "+"empty-ha"+"ll\">一座也没"+"有</div>",c3+="<div cla"+"ss=\"club"+"-grid\">"+bl()["map"](function(c9){var fE=ft,
ca=ag(c9["teamId"]),cb=a6["leagueOf"+"Team"](ca),cc='gk'===bX?[['出场',c9["apps"]],['零封',c9['cs']],['失球',c9['ga']]]:[['出场',c9["apps"]],
['进球',c9["goals"]],['助攻',c9["assists"]]],cd={};
return c9["trophies"]["forEach"](function(ce){cd[ce]=(cd[ce]||0x0)+0x1;
}),"<div cla"+"ss=\"club"+"-card\" s"+"tyle=\"--"+"tc:"+(c9["color"]||"#666")+("\"><div c"+"lass=\"cl"+"ub-card-"+"wash\">")+aT(ca)+("</div><d"+"iv class"+"=\"club-c"+"ard-body"+"\"><div c"+"lass=\"cl"+"ub-card-"+"crest\">")+aT(ca)+("</div><d"+"iv class"+"=\"club-c"+"ard-name"+'\x22>')+ax(c9["name"])+("</div><d"+"iv class"+"=\"club-c"+"ard-lg\">")+ax(cb?cb["name"]:'')+("</div><d"+"iv class"+"=\"club-c"+"ard-year"+"s\">")+(c9["from"]===c9['to']?c9["from"]+'\x20岁':c9["from"]+'-'+c9['to']+'\x20岁')+" · "+c9["seasons"]+(" 个赛季</di"+"v><div c"+"lass=\"cl"+"ub-card-"+"stats\">")+cc["map"](function(ce){var fF=fE;
return "<span><b"+'>'+ce[0x1]+"</b>"+ce[0x0]+"</span>";
})["join"]('')+("</div><d"+"iv class"+"=\"club-c"+"ard-tro\""+'>')+Object["keys"](cd)["map"](function(ce){var fG=fE;
return "<span cl"+"ass=\"tro"+"phy-item"+'\x22>'+bj(ce)+"<span>"+ax(ce)+(cd[ce]>0x1?'\x20×'+cd[ce]:'')+("</span><"+"/span>");
})["join"]('')+("</div></"+"div></di"+'v>');
})["join"]('')+"</div>",c3+=(function(){var fK=ft,c9=(function(){var fH=_gam_0b,cU=[];
return(au["youthLog"]||[])["forEach"](function(cV){var fI=fH,cW=ag(cV["teamId"]);
cU["push"]({'age':cV["age"],'ovr':cV["ovr"],'youth':!0x0,'teamId':cV["teamId"],'club':cW?a6["academyN"+"ame"](cW):'青训','color':cW?cW["color"]:null});
}),(au["seasons"]||[])["forEach"](function(cV){var fJ=fH;
cU["push"]({'age':cV["age"],'youth':!0x1,'teamId':cV["teamId"],'ovr':Math["round"](null!=cV["ovrEnd"]?cV["ovrEnd"]:cV["ovr"]),'club':cV["teamName"],'color':cV["color"]});
}),cU;
}());
if(c9["length"]<0x2)return'';
var ca=window["innerWid"+'th']<0x2bc,cb=ca?0x17c:0x384,cc=ca?0x18:0x20,cd=ca?0xa:0x10,ce=ca?0x22:0x28,cf=ca?0x1a:0x20,cg={};
c9["forEach"](function(cU){var fL=fK;
cg[cU["age"]]=cU["ovr"];
});
var ch=function(){var fN=fK,cU={};
function cV(cW,cX){var fM=_gam_0b;
for(var cY=0x0;
cY<bn["length"];
cY++){var cZ=bn[cY];
if(cZ['re']["test"](cW))return void((!cU[cZ["label"]]||cX<cU[cZ["label"]]["age"])&&(cU[cZ["label"]]={'age':cX,'label':cZ["label"],'rank':cZ["rank"],'img':bi(cW)}));
}}return(au["trophies"]||[])["forEach"](function(cW){var fO=fN;
cV(cW["name"],cW["age"]);
}),(au["awards"]||[])["forEach"](function(cW){var fP=fN;
cV(cW["name"],cW["age"]);
}),Object["keys"](cU)["map"](function(cW){return cU[cW];
})["filter"](function(cW){var fQ=fN;
return!!cW["img"];
})["sort"](function(cW,cX){var fR=fN;
return cW["rank"]-cX["rank"];
});
}()["filter"](function(cU){var fS=fK;
return cU["age"]>=c9[0x0]["age"]&&cU["age"]<=c9[c9["length"]-0x1]["age"]&&null!=cg[cU["age"]];
});
ch["length"]&&ch[0x0]["rank"]<=0x3&&(ch=ch["filter"](function(cU){var fT=fK;
return cU["rank"]<=0x9;
}));
var ci=ch["length"]>0x0,cj=ci?ca?0x2a:0x30:ca?0x10:0x14,ck=(ca?0xcd:0xff)+(ci?ca?0x1a:0x1c:0x0),cl=cb-cc-cd,cm=ck-cj-ce,cn=0x64,
co=0x0,cp=c9[0x0];
c9["forEach"](function(cU){var fU=fK;
cU["ovr"]<cn&&(cn=cU["ovr"]),cU["ovr"]>co&&(co=cU["ovr"],cp=cU);
});
var cq=Math["max"](0x0,0xa*Math["floor"]((cn-0x5)/0xa)),cr=Math["min"](0x64,0xa*Math["ceil"]((co+0x5)/0xa));
cr-cq<0x1e&&(cr=Math["min"](0x64,cq+0x1e),cq=Math["max"](0x0,cr-0x1e));
var cs=c9[0x0]["age"],cu=c9[c9["length"]-0x1]["age"];
function cv(cU){return cu===cs?cc+cl/0x2:cc+(cU-cs)/(cu-cs)*cl;
}function cw(cU){return cj+(0x1-(cU-cq)/(cr-cq))*cm;
}var cx="<svg cla"+"ss=\"ovrc"+"-svg\" vi"+"ewBox=\"0"+" 0 "+cb+'\x20'+ck+("\" preser"+"veAspect"+"Ratio=\"x"+"MidYMid "+"meet\" ro"+"le=\"img\""+" aria-la"+"bel=\"能力值"+"曲线，")+cs+" 岁到 "+cu+" 岁，峰值 "+co+'\x22>';
cx+="<defs><l"+"inearGra"+"dient id"+"=\"ovrcFi"+"ll\" x1=\""+"0\" y1=\"0"+"\" x2=\"0\""+" y2=\"1\">"+"<stop cl"+"ass=\"ovr"+"c-s0\" of"+"fset=\"0\""+"/><stop "+"class=\"o"+"vrc-s1\" "+"offset=\""+"1\"/></li"+"nearGrad"+"ient></d"+"efs>";
for(var cy=cr-cq>0x32?0x14:0xa,cz=cq;
cz<=cr;
cz+=cy){var cA=cw(cz)["toFixed"](0x1);
cx+="<line cl"+"ass=\"ovr"+"c-grid\" "+"x1=\""+cc+"\" y1=\""+cA+"\" x2=\""+(cc+cl)+"\" y2=\""+cA+("\"/><text"+" class=\""+"ovrc-tic"+"k\" x=\"")+(cc-0x6)+"\" y=\""+cA+("\" text-a"+"nchor=\"e"+"nd\" domi"+"nant-bas"+"eline=\"m"+"iddle\">")+cz+"</text>";
}var cB=[],cC=null;
c9["forEach"](function(cU){var fV=fK;
cC&&cC["teamId"]===cU["teamId"]&&cC["youth"]===cU["youth"]?cC['to']=cU["age"]:(cC={'teamId':cU["teamId"],'youth':cU["youth"],'club':cU["club"],'color':cU["color"]||"#4a5a50",'from':cU["age"],'to':cU["age"]},
cB["push"](cC));
});
var cD=cj+cm+(ca?0xd:0x10),cE=ca?0x5:0x6;
cB["forEach"](function(cU,cV){var fW=fK,cW=cB[cV+0x1],cX=cv(cU["from"]),cY=cW?cv(cW["from"]):cv(cU['to'])+(cu===cs?0x0:cl/Math["max"](0x1,cu-cs));
(cY=Math["min"](cY,cc+cl)-(cW?1.2:0x0))-cX<0.8||(cx+="<rect cl"+"ass=\"ovr"+"c-band"+(cU["youth"]?" youth":'')+"\" x=\""+cX["toFixed"](0x1)+"\" y=\""+cD+("\" width="+'\x22')+(cY-cX)["toFixed"](0x1)+("\" height"+'=\x22')+cE+("\" rx=\"2\""+" fill=\"")+ax(cU["color"])+("\"><title"+'>')+ax(cU["club"])+" · "+cU["from"]+(cU['to']>cU["from"]?'-'+cU['to']:'')+(" 岁</titl"+"e></rect"+'>'));
});
for(var cF=[cs],cG=0x4*Math["ceil"]((cs+0x1)/0x4);
cG<cu;
cG+=0x4)cF["push"](cG);
cu!==cs&&cF["push"](cu),cF["forEach"](function(cU,cV){var fX=fK;
if(!(cV&&cU-cF[cV-0x1]<0x2)){var cW=0x0===cV?"start":cV===cF["length"]-0x1?"end":"middle";
cx+="<text cl"+"ass=\"ovr"+"c-tick\" "+"x=\""+cv(cU)["toFixed"](0x1)+"\" y=\""+(cj+cm+(ca?0xb:0xc))+("\" text-a"+"nchor=\"")+cW+'\x22>'+cU+"</text>";
}});
for(var cH=-0x1,cI=0x0;
cI<c9["length"];
cI++)if(!c9[cI]["youth"]){cH=cI;
break;
}var cJ=cH<0x0?c9:c9["slice"](0x0,cH+0x1),cK=cH<0x0?[]:c9["slice"](cH);
function cL(cU){var fY=fK;
return cU["map"](function(cV,cW){return(cW?'L':'M')+function(cX){var fZ=_gam_0b;
return cv(cX["age"])["toFixed"](0x1)+'\x20'+cw(cX["ovr"])["toFixed"](0x1);
}(cV);
})["join"]('\x20');
}if(cK["length"]>0x1){var cM=(cj+cm)["toFixed"](0x1);
cx+="<path cl"+"ass=\"ovr"+"c-area\" "+"d=\""+cL(cK)+'\x20L'+cv(cK[cK["length"]-0x1]["age"])["toFixed"](0x1)+'\x20'+cM+'\x20L'+cv(cK[0x0]["age"])["toFixed"](0x1)+'\x20'+cM+(" Z\" fill"+"=\"url(#o"+"vrcFill)"+"\"/>");
}if(cJ["length"]>0x1&&(cx+="<path cl"+"ass=\"ovr"+"c-line y"+"outh\" d="+'\x22'+cL(cJ)+"\"/>"),cK["length"]>0x1&&(cx+="<path cl"+"ass=\"ovr"+"c-line\" "+"d=\""+cL(cK)+"\"/>"),
cH>0x0){var cN=cv(c9[cH]["age"])["toFixed"](0x1),cO=cv(c9[cH]["age"])<cc+0.12*cl?"start":"middle";
cx+="<line cl"+"ass=\"ovr"+"c-mark\" "+"x1=\""+cN+"\" y1=\""+cj+"\" x2=\""+cN+"\" y2=\""+(cj+cm)+("\"/><text"+" class=\""+"ovrc-not"+"e\" x=\"")+cN+"\" y=\""+(cj+cm-0x5)+("\" text-a"+"nchor=\"")+cO+("\">签下第一份合"+"同</text>");
}c9["forEach"](function(cU){var g0=fK;
cx+="<circle "+"class=\"o"+"vrc-dot"+(cU["youth"]?" youth":'')+"\" cx=\""+cv(cU["age"])["toFixed"](0x1)+"\" cy=\""+cw(cU["ovr"])["toFixed"](0x1)+"\" r=\""+(ca?2.1:2.6)+("\"><title"+'>')+cU["age"]+" 岁 · "+ax(cU["club"]||'')+" · 能力 "+cU["ovr"]+("</title>"+"</circle"+'>');
});
var cP=0.82*cf,cQ=ch["slice"](0x0,0x4)["sort"](function(cU,cV){var g1=fK;
return cU["age"]-cV["age"];
}),cR=[];
cQ["forEach"](function(cU){var g2=fK,cV=cR[cR["length"]-0x1];
cV&&cU["age"]-cV["items"][cV["items"]["length"]-0x1]["age"]<=0x2?cV["items"]["push"](cU):cR["push"]({'items':[cU]});
}),cR["forEach"](function(cU){var g3=fK,cV=0x0,cW=0x63;
cU["items"]["forEach"](function(cX){var g4=g3;
cV+=cv(cX["age"]),cX["rank"]<cW&&(cW=cX["rank"]);
}),cU['cx']=cV/cU["items"]["length"],cU['w']=cU["items"]["length"]*cP,cU["rank"]=cW;
});
var cS=[];
cR["slice"]()["sort"](function(cU,cV){var g5=fK;
return cU["rank"]-cV["rank"];
})["forEach"](function(cU){var g6=fK;
for(var cV=0x0;
cV<cS["length"];
cV++)if(Math["abs"](cS[cV]['cx']-cU['cx'])<(cS[cV]['w']+cU['w'])/0x2)return;
cS["push"](cU);
});
var cT=cf+0x8;
return cS["forEach"](function(cU){var g7=fK,cV=cU["items"]["length"],cW=cU['cx']-(cV-0x1)*cP/0x2;
cW=Math["max"](cW,cc+cf/0x2),cW=Math["min"](cW,cc+cl-cf/0x2-(cV-0x1)*cP),cU["items"]["forEach"](function(cX,cY){var g8=g7,cZ=cW+cY*cP,d0=cv(cX["age"]),d1=cw(cg[cX["age"]]);
cx+="<line cl"+"ass=\"ovr"+"c-mile-l"+"\" x1=\""+cZ["toFixed"](0x1)+"\" y1=\""+(cT+0x3)+"\" x2=\""+d0["toFixed"](0x1)+"\" y2=\""+(d1-0x6)["toFixed"](0x1)+("\"/><circ"+"le class"+"=\"ovrc-m"+"ile-d\" c"+"x=\"")+d0["toFixed"](0x1)+"\" cy=\""+d1["toFixed"](0x1)+"\" r=\""+(ca?0x3:3.4)+("\"/><imag"+"e class="+"\"ovrc-mi"+"le-i\" hr"+"ef=\"")+ax(cX["img"])+"\" x=\""+(cZ-cf/0x2)["toFixed"](0x1)+"\" y=\""+(cT-cf)+("\" width="+'\x22')+cf+("\" height"+'=\x22')+cf+("\" preser"+"veAspect"+"Ratio=\"x"+"MidYMax "+"meet\"><t"+"itle>")+ax(cX["label"])+" · "+cX["age"]+(" 岁</titl"+"e></imag"+'e>');
});
}),cx+="<circle "+"class=\"o"+"vrc-peak"+"\" cx=\""+cv(cp["age"])["toFixed"](0x1)+"\" cy=\""+cw(cp["ovr"])["toFixed"](0x1)+"\" r=\""+(ca?3.8:4.4)+("\"><title"+">峰值 ")+cp["ovr"]+" · "+cp["age"]+(" 岁</titl"+"e></circ"+"le>"),
"<div cla"+"ss=\"ovr-"+"chart\"><"+"div clas"+"s=\"ovrc-"+"head\"><s"+"pan clas"+"s=\"ovrc-"+"title\">能"+"力值曲线</sp"+"an><span"+" class=\""+"ovrc-leg"+"end\"><i "+"class=\"l"+"g-youth\""+"></i>青训<"+"i class="+"\"lg-pro\""+"></i>职业<"+"i class="+"\"lg-peak"+"\"></i>峰值"+'\x20'+co+("</span><"+"/div>")+(cx+="</svg>")+"</div>";
}()),c3+=(function(){var g9=ft,c9=au["life"]||{},ca=[];
function cb(cc,cd,ce,cf){var ga=g9;
return "<div cla"+"ss=\"life"+"-item"+(cf||'')+("\"><span "+"class=\"l"+"ife-ico\""+'>')+cc+("</span><"+"span cla"+"ss=\"life"+"-text\"><"+"span cla"+"ss=\"life"+"-main\">")+cd+("</span><"+"span cla"+"ss=\"life"+"-sub\">")+ce+("</span><"+"/span></"+"div>");
}return c9["partner"]?ca["push"](['💞',ax(c9["partner"]["label"]),c9["partner"]["since"]+" 岁那年在一起"+(c9["married"]?'，'+c9["married"]+" 岁结婚":'')]):c9["married"]&&ca["push"](['💍',
"结过婚",c9["married"]+'\x20岁']),c9["kids"]&&c9["kids"]["length"]&&ca["push"](['👶',0x1===c9["kids"]["length"]?"一个孩子":c9["kids"]["length"]+" 个孩子",
c9["kids"]["map"](function(cc){var gb=g9;
return cc["born"]+'\x20岁';
})["join"](" / ")+"那年出生"]),c9["splits"]&&ca["push"](['🥀',0x1===c9["splits"]?"分开过一次":"分开过 "+c9["splits"]+'\x20次',c9["partner"]?"后来又遇上了人":"之后一个人"]),
"<div cla"+"ss=\"sect"+"ion-head"+"\"><h2>球场"+"以外</h2><"+"span></s"+"pan></di"+'v>'+(ca["length"]?"<div cla"+"ss=\"life"+"-row\">"+ca["map"](function(cc){return cb(cc[0x0],
cc[0x1],cc[0x2]);
})["join"]('')+"</div>":"<div cla"+"ss=\"life"+"-row\">"+cb('⚽',au["seasons"]["length"]?"这些年只有球":"这些还没轮到",au["seasons"]["length"]?"没成家，也没耽误"+"过一次训练":"十几岁就下来了，"+"那些都是后来的事",
" empty")+"</div>");
}()),c3+="<details"+" class=\""+"season-b"+"ook\"><su"+"mmary>逐赛"+"季表现<span"+" class=\""+"sb-note\""+'>'+au["seasons"]["length"]+(" 个赛季，点开看"+"每一年</spa"+"n></summ"+"ary>")+b9(!0x0)+("</detail"+'s>'),
c3+="<div cla"+"ss=\"endi"+"ng\" data"+"-ending="+'\x22'+ax(bY['id'])+("\"><div c"+"lass=\"en"+"ding-eye"+"brow\">")+((au["gen"]||0x1)>0x1?'第\x20'+au["gen"]+" 世 · ":'')+("结局</div>"+"<div cla"+"ss=\"endi"+"ng-title"+"-wrap\"><"+"div clas"+"s=\"endin"+"g-glow\" "+"aria-hid"+"den=\"tru"+"e\">")+ax(bY["title"])+("</div><d"+"iv class"+"=\"ending"+"-title\">")+ax(bY["title"])+("</div></"+"div><div"+" class=\""+"ending-d"+"esc\">")+ax(bY["desc"])+("</div></"+"div>");
var c5=(au["endingsA"+'ll']||[])["filter"](function(c9){return c9!==bY['id'];
});
if(c5["length"]){var c6=c5["map"](bm)["filter"](Boolean)["sort"](function(c9,ca){var gc=ft;
return(null==c9["tier"]?0x9:c9["tier"])-(null==ca["tier"]?0x9:ca["tier"]);
});
c3+="<details"+" class=\""+"also-end"+"ings\"><s"+"ummary>这"+"一世还够到了 "+c6["length"]+(" 条结局<spa"+"n class="+"\"ae-note"+"\">点开看是哪些"+"</span><"+"/summary"+"><div cl"+"ass=\"ae-"+"list\">")+c6["map"](function(c9){var gd=ft;
return "<div cla"+"ss=\"ae-i"+"tem\"><sp"+"an class"+"=\"ae-tit"+"le\">"+ax(a0["endingVi"+'ew'](c9,bX)["title"])+("</span><"+"span cla"+"ss=\"ae-h"+"int\">")+ax(c9["hint"]||'')+("</span><"+"/div>");
})["join"]('')+("</div><p"+" class=\""+"ae-foot\""+">结局只说一条，"+"是因为一段人生只"+"有一种定性 ——"+" 这些都写进了结"+"局图鉴。</p>"+"</detail"+'s>');
}var c7=a6["legacyFr"+'om'](bW),c8=[];
c7&&(c7["ovr"]&&c8["push"]("能力 +"+c7["ovr"]),c7["talent"]&&c8["push"]("天赋 +"+c7["talent"]["toFixed"](0x2)),c7["guanxi"]&&c8["push"]("关系 +"+c7["guanxi"]),
c7["money"]&&c8["push"]("家底 +"+al(c7["money"]))),c3+="<div cla"+"ss=\"btn-"+"stack\"><"+"button c"+"lass=\"bt"+"n btn-pr"+"imary\" d"+"ata-act="+"\"next-li"+"fe\">下一世<"+"/button>"+"<div cla"+"ss=\"lega"+"cy-note\""+'>'+(c8["length"]?"带走 "+ax(c8["join"](" · "))+"，重新投一次胎":bW["banned"]?"被抹掉的那一世什"+"么也留不下，但你"+"可以重新开始":"这一世没留下什么"+"，下一世从头再来")+("</div><b"+"utton cl"+"ass=\"btn"+"\" data-a"+"ct=\"repl"+"ay\">从头再来"+"</button"+"><button"+" class=\""+"btn\" dat"+"a-act=\"s"+"hare\">生成"+"总结卡</but"+"ton></di"+'v>'),
c3+="<div cla"+"ss=\"seed"+"-row\"><s"+"pan clas"+"s=\"seed-"+"l\">种子</s"+"pan><cod"+"e class="+"\"seed-v\""+'>'+ax(String(au["seed"]))+("</code><"+"button c"+"lass=\"mi"+"ni-btn\" "+"data-act"+"=\"copy-s"+"eed\" dat"+"a-seed=\"")+ax(String(au["seed"]))+("\">复制</bu"+"tton><sp"+"an class"+"=\"seed-n"+"ote\">首页填"+"上它，能再走一遍"+"同一段人生</s"+"pan></di"+'v>'),
aw("summary-"+"area")["innerHTM"+'L']=c3;
}())):void bN();
bM("view-int"+'ro');
}function bP(bW){var ge=d3;
a6["commitEv"+"ent"](bW),aA(),bN();
}function bQ(bW){var gf=d3,bX=au["pending"];
if(bX){if("random"===bX["type"]){if(bX["result"]||bf)return;
var bY=a6["resolveE"+"vent"](bW);
if(null===bY)return as();
if(!bY)return;
if(!(bY["roll"]&&bY["roll"]['p']>0.005&&bY["roll"]['p']<0.995&&"function"==typeof requestAnimationFrame)||be())return bP(bY["res"]);
var bZ=(c0=bY["opt"],c1=bY["roll"],(c2=String(c0["hint"]||'')["match"](bd))?{'p':c1['p'],'a':c2[0x2],'b':c2[0x4]}:c0["odds"]&&0x2===c0["odds"]["length"]?{'p':c1['p'],'a':c0["odds"][0x0],'b':c0["odds"][0x1]}:{'p':c1['p'],'a':'成了','b':'没成'});
return bX["roll"]={'p':bZ['p'],'ok':bY["roll"]['ok'],'a':bZ['a'],'b':bZ['b'],'spinning':!0x0},bf=!0x0,bN(),void function(c3){var gg=gf,
c4=b3(".roulett"+'e');
if(!c4||!c4["querySel"+"ectorAll"])return c3();
var c5=c4["querySel"+"ectorAll"](".slot");
if(c5["length"]<0x2)return c3();
var c6=au["pending"]["roll"]['ok']?0x0:0x1,c7=0xd;
(c7-0x1)%0x2!==c6&&c7++;
var c8=0x0;
!function c9(){var gh=gg;
if(c5[0x0]["classLis"+'t']["toggle"]('on',c8%0x2==0x0),c5[0x1]["classLis"+'t']["toggle"]('on',c8%0x2==0x1),c8>=c7-0x1)setTimeout(c3,
0x208);
else{var ca=c8/(c7-0x1);
c8++,setTimeout(c9,0x5a+0x1a4*Math["pow"](ca,2.2));
}}();
}(function(){var gi=gf;
bf=!0x1,au["pending"]&&au["pending"]["roll"]&&(au["pending"]["roll"]["spinning"]=!0x1),bP(bY["res"]);
});
}var c0,c1,c2;
a6["choose"](bW)&&(aA(),ar());
}}function bR(){var gj=d3;
a6["cont"](),aA(),ar();
}function bS(bW){var gk=d3;
b2=null;
var bX,bY=ay("mode","normal"),bZ=av;
(!(bX=ay("pid",null))||String(bX)["length"]<0x8)&&az("pid",bX='p'+Date["now"]()["toString"](0x24)+Math["random"]()["toString"](0x24)["slice"](0x2,0xc));
var c0=aB();
aD();
var c1,c2,c3,c4=(c2=(c1=aw("in-seed"))&&null!=c1["value"]?String(c1["value"])["trim"]():'')?/^\d{1,15}$/["test"](c2)?Number(c2):c2["slice"](0x0,
0x20):null,c5=null!=c4?c4:Math["floor"](0x3b9aca00*Math["random"]());
if(null!=c4){var c6=aw("in-seed");
c6&&(c6["value"]=''),bK("按种子 "+c4+" 开局");
}(au=a6["newState"](bY,bZ,c5,c0))["cheat"]||(c3=bZ)&&az("ident",{'name':c3["name"],'number':c3["number"],'foot':c3["foot"],
'pos':c3["pos"]}),au["maxOvr"]=au["ovr"],au["rid"]=null,aA(),as(),bW&&bW(au);
}function bT(){var gl=d3;
bf=!0x1,aJ(),au=null,a6["attach"](null),bU();
}function bU2(bW,bX){if("郝海东"===bW&&9===bX)return'郝海东';
if("范志毅"===bW&&5===bX)return'范志毅';
if("孙继海"===bW&&12===bX)return'孙继海';
if("郑智"===bW&&10===bX)return'郑智';
if("武磊"===bW&&7===bX)return'武磊';
return'';
}
function bU1(){var gm=d3,rr,lt,bs,pp;
pp=a0["POSITIONS"][Math["floor"](Math["random"]()*a0["POSITIONS"]["length"])]["id"];
rr=Math["floor"](Math["random"]()*0x64);
if(rr<0x5){lt=[['郝海东',0x9],['范志毅',0x5],['孙继海',0xc],['郑智',0xa],['武磊',0x7]];
bs=lt[rr];
return{'name':bs[0x0],'number':bs[0x1],'pos':pp,'foot':0x0===rr%0x2?'left':'right'};
}var sn=['李','王','张','刘','陈','杨','赵','黄','周','吴','徐','孙','胡','朱','高','林','何','郭','马','罗'],gn=['伟','磊','军','洋','勇','明','强',
'杰','涛','超','斌','刚','平','辉','鹏','华','飞','凯','健','龙'];
return{'name':sn[Math["floor"](Math["random"]()*sn["length"])]+gn[Math["floor"](Math["random"]()*gn["length"])],'number':0x1+Math["floor"](Math["random"]()*0x63),
'pos':pp,'foot':Math["random"]()<0.5?'left':'right'};
}
function bU(){var gm=d3,bW,bs;
(bW=ay("ident",null))&&"object"==typeof bW||(bW={});
if("string"==typeof bW["name"]&&bW["name"]["trim"]())bs={'name':bW["name"]["trim"]()["slice"](0x0,0x6),'number':ad(parseInt(bW["number"],0xa)||0xa,0x1,0x63),
'foot':"left"===bW["foot"]?"left":"right",'pos':bW["pos"]||'ST'};
else{bs=bU1();
bW["foot"]=bs["foot"];
}av={'name':bs["name"],'number':bs["number"],'foot':"left"===bW["foot"]?"left":"right",'dreamId':null,'pos':bs["pos"],'origin':a9[0x0]},
aX=0x0,bM("view-ide"+"ntity"),b0();
}var bV={'long':"每个赛季一次决策"+"，最细最长",'normal':"每两个赛季一次决"+"策，节奏适中",'express':"每三个赛季一次决"+"策，一局二十分钟"};
document["addEvent"+"Listener"]("DOMConte"+"ntLoaded",function(){var gR=d3;
!(function(){var gn=_gam_0b,bX;
aw("hero-art")["innerHTM"+'L']=(function(){var go=gn;
for(var c8=aV('hg'),c9=aV('hl'),ca='',cb=0x0;
cb<0x8;
cb++)cb%0x2!=0x0&&(ca+="<rect x="+'\x22'+0x32*cb+("\" y=\"0\" "+"width=\"5"+"0\" heigh"+"t=\"300\" "+"fill=\"#f"+"ff\" fill"+"-opacity"+"=\".05\"/>"));
return "<svg vie"+"wBox=\"0 "+"0 400 30"+"0\" prese"+"rveAspec"+"tRatio=\""+"xMidYMid"+" slice\" "+"aria-hid"+"den=\"tru"+"e\"><defs"+"><linear"+"Gradient"+" id=\""+c8+("\" x1=\"0\""+" y1=\"0\" "+"x2=\".5\" "+"y2=\"1\"><"+"stop off"+"set=\"0\" "+"stop-col"+"or=\"#155"+"63e\"/><s"+"top offs"+"et=\".55\""+" stop-co"+"lor=\"#0e"+"3b2b\"/><"+"stop off"+"set=\"1\" "+"stop-col"+"or=\"#082"+"51b\"/></"+"linearGr"+"adient><"+"radialGr"+"adient i"+"d=\"")+c9+("\" cx=\".5"+"\" cy=\".4"+"2\" r=\".6"+"2\"><stop"+" offset="+"\"0\" stop"+"-color=\""+"#34d399\""+" stop-op"+"acity=\"."+"2\"/><sto"+"p offset"+"=\"1\" sto"+"p-color="+"\"#34d399"+"\" stop-o"+"pacity=\""+"0\"/></ra"+"dialGrad"+"ient></d"+"efs><rec"+"t width="+"\"400\" he"+"ight=\"30"+"0\" fill="+"\"url(#")+c8+")\"/>"+ca+("<rect wi"+"dth=\"400"+"\" height"+"=\"300\" f"+"ill=\"url"+'(#')+c9+(")\"/><g f"+"ill=\"non"+"e\" strok"+"e=\"#eaff"+"f6\" stro"+"ke-opaci"+"ty=\".3\" "+"stroke-w"+"idth=\"2\""+"><rect x"+"=\"26\" y="+"\"26\" wid"+"th=\"348\""+" height="+"\"248\" rx"+"=\"2\"/><l"+"ine x1=\""+"200\" y1="+"\"26\" x2="+"\"200\" y2"+"=\"274\"/>"+"<circle "+"cx=\"200\""+" cy=\"150"+"\" r=\"46\""+"/><rect "+"x=\"26\" y"+"=\"92\" wi"+"dth=\"52\""+" height="+"\"116\"/><"+"rect x=\""+"322\" y=\""+"92\" widt"+"h=\"52\" h"+"eight=\"1"+"16\"/><re"+"ct x=\"26"+"\" y=\"126"+"\" width="+"\"18\" hei"+"ght=\"48\""+"/><rect "+"x=\"356\" "+"y=\"126\" "+"width=\"1"+"8\" heigh"+"t=\"48\"/>"+"</g><cir"+"cle cx=\""+"200\" cy="+"\"150\" r="+"\"5\" fill"+"=\"#eafff"+"6\" fill-"+"opacity="+"\".8\"/></"+"svg>");
}()),aw("intro-fa"+"cts")["innerHTM"+'L']=[[a1["length"],"个事件"],[a0["ENDINGS"]["length"],"条结局"],[a0["TEAMS"]["length"],"支球队"],
['12',"岁开踢"]]["map"](function(c8){var gp=gn;
return "<li><b>"+c8[0x0]+("</b><spa"+'n>')+ax(c8[0x1])+("</span><"+"/li>");
})["join"](''),(bX=window["SUPPORTE"+'RS']||[])["length"]&&(aw("thanks-r"+'ow')["innerHTM"+'L']=bX["map"](function(c8){var gq=gn;
return "<span cl"+"ass=\"thx"+'\x22>'+ax(c8["name"])+"</span>";
})["join"](" · "),aw("thanks")["classLis"+'t']["remove"]("hidden")),aw("btn-help")["addEvent"+"Listener"]("click",function(){var gr=gn;
aw("help-mod"+'al')["classLis"+'t']["remove"]("hidden");
}),aw("btn-open"+"-news")["addEvent"+"Listener"]("click",function(){var gs=gn;
aw("help-mod"+'al')["classLis"+'t']["add"]("hidden"),aw("news-mod"+'al')["classLis"+'t']["remove"]("hidden");
}),document["addEvent"+"Listener"]("click",function(c8){var gt=gn,c9=c8["target"]["getAttri"+"bute"]&&c8["target"]["getAttri"+"bute"]("data-clo"+'se');
c9&&aw(c9)["classLis"+'t']["add"]("hidden"),c8["target"]["classLis"+'t']&&c8["target"]["classLis"+'t']["contains"]("overlay")&&c8["target"]["classLis"+'t']["add"]("hidden");
});
var bY=aw("share-na"+'me');
bY&&bY["addEvent"+"Listener"]("change",function(){var gu=gn;
bw=bY["checked"],bE();
});
var bZ=!(!navigator["canShare"]||!navigator["share"]),c0=!!(window["Clipboar"+"dItem"]&&navigator["clipboar"+'d']&&navigator["clipboar"+'d']["write"]),
c1=function(c8){var gv=gn;
if(document["querySel"+"ector"]){var c9=document["querySel"+"ector"]("[data-sh"+"are=\""+c8+'\x22]');
c9&&c9["classLis"+'t']["add"]("hidden");
}};
bZ||c1("share"),c0||c1("copy"),bF()&&c1("save");
var c2=aw("share-ti"+'p');
if(c2&&!bF()&&(!bZ||!c0)){var c3=!0x0;
try{c3=!0x1!==window["isSecure"+"Context"];
}catch(c8){}c2["textCont"+"ent"]=c3?"这个浏览器只支持"+"「保存图片」，另"+"外两个键用不了":"浏览器只在 ht"+"tps 和 lo"+"calhost "+"下开放分享 / "+"复制图片。现在是"+"用 IP 访问的"+"，先用「保存图片"+'」。',
c2["classLis"+'t']["remove"]("hidden");
}var c4=aw("share-mo"+"dal");
c4&&c4["addEvent"+"Listener"]("click",function(c9){var gw=gn,ca=c9["target"]["closest"]&&c9["target"]["closest"]("[data-ep"+']');
if(ca){var cb=bG&&bG["endingCh"+"oices"]&&bG["endingCh"+"oices"][Number(ca["getAttri"+"bute"]("data-ep"))];
if(!cb||!bG)return;
return bG["endingTi"+"tle"]=cb["title"],bx=null,bI(bG),bE(bG);
}if(c9["target"]["closest"]&&c9["target"]["closest"]("[data-ac"+"t=\"card-"+"next-lif"+"e\"]")){var cc=bG&&bG["legacy"];
if(!cc)return;
return aC(cc),aw("share-mo"+"dal")["classLis"+'t']["add"]("hidden"),bK('第\x20'+cc["gen"]+(" 世，重新投一次"+'胎')),bT();
}var cd=c9["target"]["closest"]&&c9["target"]["closest"]("[data-sh"+"are]");
if(cd){var ce=cd["getAttri"+"bute"]("data-sha"+'re');
(bx?Promise["resolve"](bx):new Promise(function(cf){var gx=gw,cg=aw("share-ca"+"nvas");
if(!cg||!cg["toBlob"])return cf(null);
cg["toBlob"](function(ch){bx=ch,cf(ch);
},"image/pn"+'g');
}))["then"](function(cf){var gy=gw;
if(!cf)return bK("这个浏览器存不了"+"图，试试截屏");
var cg=bG||bC(),ch=a3+'-'+cg["name"]+".png";
if("save"===ce){var ci=URL["createOb"+"jectURL"](cf),cj=document["createEl"+"ement"]('a');
return cj["href"]=ci,cj["download"]=ch,document["body"]["appendCh"+"ild"](cj),cj["click"](),cj["remove"](),setTimeout(function(){var gz=gy;
URL["revokeOb"+"jectURL"](ci);
},0xfa0),bK("图片已保存");
}if("copy"!==ce){if("share"===ce){var ck=new File([cf],ch,{'type':"image/pn"+'g'});
if(!navigator["canShare"]({'files':[ck]}))return bK("这个设备不支持分"+"享图片");
navigator["share"]({'files':[ck],'text':bJ(cg)})["catch"](function(){});
}}else{var cl=new ClipboardItem({'image/png':cf});
navigator["clipboar"+'d']["write"]([cl])["then"](function(){var gA=gy;
bK("图片已复制，可以"+"直接粘贴");
},function(){var gB=gy;
bK("复制失败，用保存"+"那个键吧");
});
}});
}}),aw("brand-ho"+'me')["addEvent"+"Listener"]("click",function(){var gC=gn;
au&&"summary"!==au["phase"]&&!confirm("回首页会保留当前"+"存档，确定？")||bM("view-int"+'ro');
}),aw("mode-seg")["addEvent"+"Listener"]("click",function(c9){var gD=gn,ca=c9["target"]["closest"]("[data-mo"+"de]");
if(ca){var cb=ca["getAttri"+"bute"]("data-mod"+'e');
Array["prototyp"+'e']["forEach"]["call"](this["children"],function(cc){var gE=gD;
cc["classLis"+'t']["toggle"]("selected",cc===ca);
}),az("mode",cb),aw("seg-note")["textCont"+"ent"]=bV[cb];
}});
var c5=ay("mode","normal");
Array["prototyp"+'e']["forEach"]["call"](aw("mode-seg")["children"],function(c9){var gF=gn;
c9["classLis"+'t']["toggle"]("selected",c9["getAttri"+"bute"]("data-mod"+'e')===c5);
}),aw("seg-note")["textCont"+"ent"]=bV[c5]||bV["normal"],aw("btn-star"+'t')["addEvent"+"Listener"]("click",function(){var gG=gn,
c9=aI();
if(c9&&"summary"!==c9["phase"]&&c9["seasons"]&&c9["seasons"]["length"]){if(confirm("检测到上次的存档"+'（'+c9["name"]+'，'+c9["age"]+(" 岁），继续吗？"+"\n取消则重新开始"+'。')))return au=c9,
a6["attach"](au),void bO();
aJ();
}bU();
}),aw("btn-back")["addEvent"+"Listener"]("click",function(){var gH=gn;
if(0x0===aX)return bM("view-int"+'ro');
aX--,b0();
}),aw("btn-next")["addEvent"+"Listener"]("click",function(){var gI=gn;
if(aX<aY["length"]-0x1)return aX++,void b0();
bS();
}),aw("step-bod"+'y')["addEvent"+"Listener"]("click",function(c9){var gJ=gn,ca=c9["target"]["closest"]("[data-or"+"igin]");
if(ca)return av["origin"]=ak(ca["getAttri"+"bute"]("data-ori"+"gin")),b0();
var cY=c9["target"]["closest"]("[data-r"+"eroll]");
if(cY){var cZ=bU1();
av["name"]=cZ["name"],av["number"]=cZ["number"],av["foot"]=cZ["foot"];
return b0();
}var cb=c9["target"]["closest"]("[data-po"+'s]');
if(cb)return av["pos"]=cb["getAttri"+"bute"]("data-pos"),b0();
var cc=c9["target"]["closest"]("[data-fo"+"ot]");
if(cc)return av["foot"]=cc["getAttri"+"bute"]("data-foo"+'t'),b0();
var cd=c9["target"]["closest"]("[data-dr"+"eamlg]");
if(cd)return aZ=cd["getAttri"+"bute"]("data-dre"+"amlg"),b0();
var ce=c9["target"]["closest"]("[data-dr"+"eam]");
if(ce){var cf=ce["getAttri"+"bute"]("data-dre"+'am');
return av["dreamId"]=cf&&cf!==av["dreamId"]?cf:null,b0();
}}),aw("step-bod"+'y')["addEvent"+"Listener"]("input",function(c9){var gK=gn;
if("in-name"===c9["target"]['id']||"in-numbe"+'r'===c9["target"]['id']){"in-name"===c9["target"]['id']?av["name"]=c9["target"]["value"]["trim"]()||'无名':av["number"]=ad(parseInt(c9["target"]["value"],0xa)||0xa,0x1,0x63);
var ca=aw("step-bod"+'y')["querySel"+"ector"](".jersey-"+"wrap");
ca&&(ca["innerHTM"+'L']=aM(av["origin"],av["name"],av["number"]));
}}),aw("app")["addEvent"+"Listener"]("click",function(c9){var gL=gn,ca=c9["target"]["closest"]("[data-op"+'t]');
if(ca)return bQ(ca["getAttri"+"bute"]("data-opt"));
var cb=c9["target"]["closest"]("[data-ac"+'t]');
if(cb){var cc=cb["getAttri"+"bute"]("data-act");
if("continue"===cc)return bR();
if("restart"===cc){if(bf)return;
if(!confirm("这一局就不要了？"+"当前存档会清掉，"+"重新选出身开一局"+"新的。"))return;
return bT();
}if("next-lif"+'e'===cc)return aC(a6["legacyFr"+'om'](aq())),bT();
if("replay"===cc)return aD(),bT();
if("share"===cc)return bH();
if("copy-see"+'d'===cc)return function(cd){var gM=gL;
if(cd){var ce=aw("in-seed");
ce&&(ce["value"]=cd);
var cf=null;
try{cf=navigator["clipboar"+'d'];
}catch(cg){}cf&&cf["writeTex"+'t']?cf["writeTex"+'t'](String(cd))["then"](function(){var gN=gM;
bK("种子已复制，也填"+"进首页了");
},function(){var gO=gM;
bK("复制不了，已经填"+"进首页的种子框");
}):bK("这个浏览器不给复"+"制，已经填进首页"+"的种子框");
}}(cb["getAttri"+"bute"]("data-see"+'d'));
if("arc-back"===cc)return bM("view-int"+'ro');
if("codex-ba"+'ck'===cc)return bM("view-int"+'ro');
if("arc-clea"+'r'===cc){if(!confirm("清空生涯历史档案"+"？这些记录只存在"+"这台设备上，清了"+"找不回来。"))return;
return(function(){var gP=gL;
try{localStorage["removeIt"+'em'](a2+"archive");
}catch(cd){}}()),bK("档案已清空"),bs();
}}}),aw("view-arc"+"hive")["addEvent"+"Listener"]("click",function(c9){var gQ=gn;
if(c9["target"]["closest"]&&!c9["target"]["closest"]("[data-ac"+'t]')){var ca=c9["target"]["closest"]("[data-ar"+'c]');
ca&&br(parseInt(ca["getAttri"+"bute"]("data-arc"),0xa));
}});
var c6=aw("btn-arch"+"ive");
c6&&c6["addEvent"+"Listener"]("click",bs);
var c7=aw("btn-code"+'x');
c7&&c7["addEvent"+"Listener"]("click",bq);
}());
var bW=aI();
bW&&bW["seasons"]&&bW["seasons"]["length"]?(au=bW,a6["attach"](au),bO()):bM("view-int"+'ro'),ay("pid",null)||ay("save",null)||ay("mode",
null)?ay("news_ver",'')!==a4&&(aw("news-mod"+'al')["classLis"+'t']["remove"]("hidden"),az("news_ver",a4)):az("news_ver",a4);
}),window["__SIMTES"+'T']={'origins':a9,'modes':a8,'start':function(bW,bX,bY){var gS=d3;
return b2=null,(au=a6["newState"](bW,bX,bY))["maxOvr"]=au["ovr"],as(),au;
},'state':function(){return au;
},'startOnline':function(bW,bX,bY){var gT=d3;
av=bX,az("mode",bW),bS(bY);
},'option':function(bW){return bQ(bW),au;
},'cont':function(){return bR(),au;
},'render':function(){return bO(),au;
},'showIntro':function(){var gU=d3;
bM("view-int"+'ro');
},'openArchive':function(){bs();
},'openCodex':function(){bq();
},'showIdentity':function(bW,bX){var gV=d3;
return bX&&(av=bX),aX=bW||0x0,bM("view-ide"+"ntity"),b0(),av;
},'arcCard':function(bW){return br(bW),bE();
},'openShare':function(){return bH();
},'cardData':function(){return bG;
},'drawShare':function(){return bE();
}};
}()));
function _gam_0b(a,b){a=a-0x1cc;
var c=_gam_0a();
var d=c[a];
if(_gam_0b['vjeLnG']===undefined){var e=function(i){var j='abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789+/=';
var l='',m='';
for(var n=0x0,o,p,q=0x0;
p=i['charAt'](q++);
~p&&(o=n%0x4?o*0x40+p:p,n++%0x4)?l+=String['fromCharCode'](0xff&o>>(-0x2*n&0x6)):0x0){p=j['indexOf'](p);
}for(var r=0x0,s=l['length'];
r<s;
r++){m+='%'+('00'+l['charCodeAt'](r)['toString'](0x10))['slice'](-0x2);
}return decodeURIComponent(m);
};
_gam_0b['DZcQTh']=e,_gam_0b['IxkfQv']={},_gam_0b['vjeLnG']=!![];
}var f=c[0x0],g=a+f,h=_gam_0b['IxkfQv'][g];
return!h?(d=_gam_0b['DZcQTh'](d),_gam_0b['IxkfQv'][g]=d):d=h,d;
}function _gam_0a(){var gW=['zd0IttiUnsa','mtyG4OAsia','n2eIihn0CM8','Cg93','5OQ55lIn5O6j55Qe','Ec1OAw50iJ4','zxH0lwXPzMu',
'BNzHCW','pUwZSowaVca','r3jHzgLLBNq','igrLy2LZAw8','ldaSmcWUmZi','ios4LJWVC3bHBG','A2u9iImWmda','y2fUu3rHEvK','zgL2','DhK9iI4ZiIa',
'BMKTyNrUiIa','yZaSlteZidi','BwvLDciGCM8','zgLLBNqGAwq','ign5psi0mci','CM9Rzt0IDxi','Cg9Zswq','ywrNzsa','C2H1zMzSzq','mI41iIb5psi',
'DhjVA2u9iIm','CgfYC2u','phnWyw4Gy2W','y3iTBci+5BM06B6e','C3rHDhmIpG','yxjKlxn0yxq','Bs1JyxjKlwW','i2q5owiXyW','yxnZAxn0CW',
'ChjVzci+','zMLSBfrLEhq','yxjJlw1HAw4','CJ0IDgHPCY4','lw1HAw4IpJW','AxrSzt4','iJ7NRB7KUiVNRkZKUidKU73LKiG','C3bSAxq','y2XLyw4',
'iJ48C3bHBIa','y3q9iNjLCgW','ios4LU+8Iq','C3rLCc1IB2q','mI44idaTnc4','iIb0ExbLpsi','Bd0I','pc9JAxjJBgu','iowYGEE7K+wPMG','yxnZpsjHz2u',
'iImZngqZotK','lwHPBNqIpG','B2XVCJ0Ii2q','CMfUzg9T','yY1Uyw1LiJ4','C3m9iMjTlxa','AgfSDMvZ','y2HHCNqIpJW','5zoQ5ys/44cc','DgG9iJm0oci',
'DgvZDa','iM5LEhqTBgK','D2LKDgG9iJm','igLKpsjPBI0','C2HHCMuTy2e','qKLhx09qvfm','z2v0q29UDgu','zMfJDxa','yNjHBMqTAg8','57Ut5P2F6igm5lIA55sF5RAV',
'CMvJCW','DgL0Bgu','lxnLzYi+pgi','BgvHz3vLt2y','CM9SBa','iMXLzNq6','BgfIzwWIpUAdRW','mJySndqGBc0','lw9YzgvYpsi','DwiTy2fYzc0',
'zM9YBq','mciGC3rYB2S','zwn0B3jbBgW','C2TPCa','iI40nsiVpJW','ltn2nMGZiIa','CMLUDa','BxvZDeXLyxy','Agf2Aw9Y','BMfUDc1Iyxm',
'y2vUDgvY','5Q+p5lIj5lIQ6lwB5A2J5lIa5QYH5yAZ','idi3lc05idi','phn0B3aGy2W','mtHbuuPfteG','mJGGmJqSnti','55sF5RAV57Ut5P2F','lJCGms41ttC',
'A2u9iIm0ywq','y2HPCci+','6l+z5lIa5Bga5BcX5lIn6kAb5lQg77YF','mcbdntGSmtC','iemXntGSmte','BgLUzwfYr3i','phn2zYb2Awu','CZ0Izw5KAw4',
'C3rYB2TLu3q','EMu9iJmXiIa','Ed0ImYiGzMK','yMfKz2vZiJ4','Aci+','C3rHzMzcEuK','re9nq29UDgu','C3vTlwHLCM8','y3KTBM90zsi','BM9Uzq',
'zci+','phbHDgGGzd0','Dw1Tyxj5pUI/Mq','zwfMzMy2iIa','utuWidiXidu','idqSmJyGAc0','Dd0ImZaWiIa','y29UiJ7WN4+gpc8','psjVDNjJrMK',
'iJeWmciGEt0','BMqIigrVBwK','C3zNpG','6Ag257QN6lgQ6zEO','iIbHBhq9iIi','y3qGEd0Iotq','B3nVzNqGwwe','nsaTmtGSlte','lZ48CMvJDca',
'idqYldG0ieW','yxnZpsjYB3u','ldu4idC4ldC','5lIl5BYa5Ps+5yIg5lQRic8G','pJWVzgL2pG','yxjJAgL2zs0','ihKYpsiXiJ4','BMDL','ldq4ide1ncW',
'CM91BMqIigy','jMX0oW','psjJCI12','zwn0ihG9iJe','lwvTChr5iJ4','C3m9iNnOzwW','mciGEt0Imtq','BMvHCKDYywq','zwfYiIbKyxq','zw1LBNq',
'iJi2iIb4mJ0','lNjVDwXLDhq','C3rVCcbVzMy','DNjJrMLSBcK','BwLKzgXL','ngy2zJiIlZ4','mJGIlZ48l2W','BgLUzvrV','B2TLlxDPzhq','C3bHBIbJBge',
'6iEQ5A625lIa57Q/6zIF5lMF5BYa5lQg','6yEm55Qe5BQv5A2q77Ym5zkm5l2G6k6K','Dgv4DefSAwC','BgfIzwW','iIbKyxrHlwq','AwDODd0Inti',
'ios4QUI1M+wTOW','5PsV55cd6zIF','ywXS','iZrHzgu4ma','6zw/5OYj5lIk6z2I6l+z5BYG5zU+ia','lwnOAxbZiJ4','C2HHCMuTAw0','z2H0psiXmda',
'mIiGzMLSBd0','Aw50CM8TzMe','CgfJAxr5psi','lwnVBg9Ypsi','lwnOAxaI','ldm0iem0ocW','u0Ln','zMLSBfjLy3q','iImXyZnHmJG','BMKTyMfKz2u',
'77Ym5zUE5As06lEV5RkH5PYj5lQg44cc','y2XHC3noyw0','zwvY','iJeIihn0B3a','5PIV5QYG5yc65y6755Qe','Aw5Uzxjive0','mcbdnJGSmtq','iJ48l3bPy3q',
'44ccpgjYpUI1SowUJos4Ga','yxnZpsjKCMu','BgfZCZ0Iyxi','5Q+p5lIa5BM0pc9ZCge','CMvHBwXNpsi','C2HHCMuTBw8','AcbKpsjnmta','77YF77YF77YF',
'lw5LD3m','lJe4kq','psjJyxjKlw4','5OQv56YSia','zsi+','BguGy2XHC3m','nsWTmtCGmJC','yw4+phnWyw4','iJaIihK9iJa','B3vUzdO','6igu6lwB5yAG5yAB',
'y2f0y2G','iJ48Adi+57Ut5Bga','y3jLyxrLrwW','yMvNAw5qyxq','iJaGmcaYnca','Axy+','pc9KAxy+phm','Et0IociGzMK','nIaXnsbmntq','DMvbC3bLy3q',
'5lU944ccpc9KAxy+','pJXZCgfUigm','B3b0sgLUDa','B3bHy2L0Et0','C2nVCMu','Edi9iI41iIa','5RIf6z2z77Ym57Ud5B6x5OwI','psjJB3b5lxm',
't1jjr0LouW','yxr0ywnO','5ywZ57o7icS','y3DJ','ihi9iJmUmsi','zNm+phbHDgG','Ag9TzuLK','zw50','iNrSlxnJCM8','C3m9iMfNzs0','CM8IpJXKAxy',
'ihG9iJeXnIi','CZ0IC3rHDc0','Ed0ImJyIihK','BMv4Dc1SAwy','iJe1mciGCJ0','zg9Uzq','ldK2ide0mIW','C3bLBgXZ','mMG1lJzJlJC','ChqIigrHDge',
'y29SB3i','mcW2mIb2nJa','AgvYBY1HCNq','iIb4psi','D2nNB2XK','lc45nsK','pc9KAxy+pgq','55sO6isApc9ZCgfU','CMLUzW','tde3ocWZnca',
'ztGWiIbZDhi','u1rbrKy','iowKQEwjJq','zxnJiJ7LKiZPVOtNMOq','iIbJEt0IlJq','BJ48C3bHBIa','nsW2igWTnsW','ignSyxnZpsi','6ykJ5BM05yE655sF',
'zciVpJXYzwm','nwrLztqIlZ4','BgvNywn5','y3vYtgvHz3u','CdOUmJvYzw0','6l+h44cn55Qe77Ym5PIV6ykJ5lIa5Bga','yY0YmsWWic0','zMzMiG',
'5l2G55Qe5yE65zY65PE26zE05BEY57Up','uMf0Aw89iNG','nIaXmdaSmtK','yxnPyw5JDxa','lxjVDYi+phm','ic0YocW2mIa','CZ0IB3zYyY0','EIiGzMLSBd0',
'Aw5WDxq','CNzLqxnWzwm','l+I1M+wTOYdcTYa','zI1JDxiIpUEoSa','BdGSmtyGmty','6l+z5lQB6l+y5RkH6l2U5yIW','CgfUpG','iIb5mJ0I','mtiIihn0CM8',
'Dcb4psiXmti','t25Lu2vHC28','nZyIigHLAwC','B3bLCNr5','5zU95A626zIFia','ign4psiXmIi','ksiVpJWVC3y','Aw5UzxjizwK','DgeG','Dgm6',
'zgvMCZ48CMu','Ed0ImtuWiIa','iNjVDw5KiIa','ttiXlJuGowG','BMf0CW','B3zY','ncWYnsaTmJi','Et0Imti2iIa','Ag9UB3vYCW','BgW9iNvYBcG',
'AwDODd0ImJa','iNn1Bs1TywK','mYa0Ac00lJi','zNnLDd0Imci','AwDODd0Imta','mtuIihn0CM8','DcbIAwDTyxq','DfjHDgLVpsi','nsiGDgv4Dc0',
'Dg9cBg9I','5Aw955Qe6ykJ5yEG5lIQ44cc5O6L5lIl','5lQA5RsY5P2V5yAG5yAB','igf1Dg8IpJW','Ew91DgHqyxq','idOG','nIiGzMLSBc0','iIbYEd0ImIi',
'Dgv4Da','z3jVDxa','iJe4iIbOzwK','igHLAwDODd0','zI1UB3rLiJ4','zt0Ii2vHzMy','Bg9HBNm','yY1TyxjRiIa','C2XPy2u','l3n2zZ4','BwfRzq',
'ndiSndaGEIi','AgL2zq','C2iTBM90zsi','ns0Ztte2lJy','AwXK','Bw92zxm','nYiGB3bHy2K','ihrVDwnO','CYi+','55sOieLqioIUV+MxRUEAHa',
'yxnZpsjNzw4','Cg9ZtMfTzq','DMLLDY1HCMm','zgLLBNq+pc8','zwDHy3KTBM8','psj1CMWOiW','y2X1yI1Uyw0','zxH0ihG9iJu','DIbJBgfZCZ0',
'5PE25ycz5lQg44ccpc9KAq','mdaSmti0iem','5yIg5BYa6l+h5lIa5QYH','yMfKiJ7OOQVLIlFKUiS','CZ0Iy29KzxG','zIiGC3rVCc0','lcbZyw5Zlxm',
'yxbWCW','iZm5ndiZyq','Ed0ImJGIihi','C3rYB2TLiIa','6l+y5RkH5A6A','CNbPy2S','iowYGEwiSca','Cgf0AcbKpsi','yxbW','C29SAwq','y29Uy2f0',
'5ycF6iEQia','AcbKpsjnnZa','y3KTyMfUBMu','55wz5zYOia','iK0XmdaSmJa','CMvTiJ48C3a','psiXmtaIihi','ieWXntGSodq','idq2ldyYiem',
'iNn0lwnLBgW','zMLSDgvY','6l+hpc9IDxr0BW','Aw9U','5P2o5lQM6z2E','CI1MiIbZDhK','5Bcr5yIW5yAz5lIn6l+B5Oc757Ut44cc','y3jLC3qTCgK',
'Ed0ImZaIihi','nIaXmZiSmtq','lNnSB3q','i2u4ywmZyW','ios4QUI1M+wTO++8JoEcUEw8GoECIW','y24TzMfJDxa','nZiIihK9iJe','tu9ervm',
'y2HHBMDL','uK9mrvm','B3b0Aw9UCW','Cg9Z','i2m5oge0nq','5l+D57QN6zIF','5Oco5lMi5Ps25zY6','C2HHCMuTzxK','Bgu+','CMLMiIbMB24',
'BdGSmtqGtte','ltC2ihOIigy','owGZDJzOltm','5RkH5PYj6yca5B256l+h55Qe55sF5RAV','6l+z5lIQ5Rwp6kEi5zMO5y+Q5PsV5OYb','Ad0IDxjSkcm',
'psjZzwvKlw4','y3KTyML0CYi','tgLZDgvUzxi','iJm0nYiGD2K','mciVpJWVCMe','yxa9iNjVDw4','zwfYr3jHzgK','mIi+phn0B3a','AxrLBxm',
'5Rwp6kEi5zMO5y+Q5zYOigH0','BM90zxm','BI10ywCIpUENNW','mca0mIWXmty','lc4WocK','BMv4Dfn0zxa','Bgv0DguTDNm','BwvHC3vYzvq','lw9WDd0I',
'ldiZidi1ldm','BMv3CY1TB2q','BgWIpUs4Gow6P+s5N+AYOq','C3m9iMvUzgK','lwL0zw0IpG','ihnSAwnLiIa','ltmWlc01mca','5y+Q5yMP6l+z5lIa5lIQ6ycj6Ag5',
'iIb0zxH0lwe','mJ0Imsi+phm','zxKIpG','zMuIpUs4I+s4Gos4LJW','iNnLzwqTDIi','ioASOEwhUUwCUJWVzgK','Dcb4psi2nci','DgeTB3b0psi',
'ndfKiIbZDhi','pgrPDJ48C3a','ldeXmcbOntG','iIbWCMvZzxi','zs13Awr0Ad0','zxjPzIiGzM8','phjLy3qGy2W','6kEb6l+h55Qe5y+Q5yAz6l6+5OIq5P2H',
'ncaZlJHimI4','w2rHDgeTBw8','CgXHDguTy28','idu2ide1ieW','zsjD','nIaYmIWXndy','ywqTBwfPBIi','lYdKV53LRzJLM77NIyC','5zYO5QkV6zIF5yAn57Ud5lIa5BM044cc',
'yNjVBNPL','y2XHC3m9iMu','psjTyxjNAw4','C2LKzsi+pgq','DhK9iI4Wntu','yY1ZmciGB2y','yM9YBG','yw5Nifndlca','CI1MAwXSiIa','C21VB3rO',
'i2y0zJzMmIi','CNG9iJqIigy','y2XHC3m9iM8','CMnZzxq9iG','psjZDgf0lxy','CIiGDhLWzt0','Bw92zvrV','zg93BMXVywq','AwDODd0ImZK',
'A2uTBgLUzwm','C3rYAw5N','twLKwu1Pzca','AgLKzgvUpsi','iMXNlxbLywS','ihi9iJmYiIa','zxiTz3jPzci','Et0InYiGzMK','BgLUzq','6zIF5yAf5zYW5l2npc9ZCa',
'iJiWmciGAgu','ie1Py3jVC28','mJaYnI0Woc0','nJq5otu2t3HIqLne','77Yi56YSia','mciGEte9iJa','nJqIigHLAwC','AI1WAwmIpJW','mZKSotyGntG',
'C2nYB2XSqMu','rMfUzYbtqYW','AhqIpJXKAxy','AgvPz2H0psi','lc4WnduP','zhrOpsiXnty','C3m9iMjHCIi','z0zHBMCGu0m','phnWyw4+','Ed0ImtiIigm',
'Ad0ImtaWiIa','CYi+pgrPDIa','5PYS5PYF5yIW5PYF','Axntzwn1CMu','A2LKCW','zgf0ys1MB28','Aw5UzxjxAwq','ttCGnY41yZa','yMfSBg9U',
'yxnZpsjOywW','5y+V5lUL6yEn5PAW5BYa5AEl','ihn0B3aTB3a','Bci+5AEt5zcnpc9Z','5P2L6lwW5lIa6ygn5l2G6iEQ5BEX55Qe','l2rPDJ48l2q',
'iIb5mt0Imci','Dg9gAxHLza','Ad0Ims4ZiI8','pUw5ToM+HdWVC3bH','yMfKz2uGC20','mcaXmdaSmtu','5B+d5lUQ55cd6zIF','zxiTy2fYzci','iJ7MUixNQBRMOApMOyG8lW',
'zwHVBgrLCJ0','C3m9iM1LDgu','zg9uCMfUC2y','lJyGmYaWDJi','yxjPys1OAwq','Chq9iG','iowYGtWVAt48lW','pJWVzgL2pJW','lNbUzW','BM9UzsiGC3q',
'BM8Tywn0Aw8','ios4LJWVzgL2pG','iUADJUs6PUMDNIiGyxu','5AsP6lwlicS','BgfZCZ0IyMe','CMvZB2X2zuu','psiYnIiGAgu','imk3ideYiowYGEMaGq',
'B2zMzxjcCMK','AhKTAgfSBci','iowUTUs/SEs5KoMdQa','zt48l2nPCMm','C3rLCc1ZDwi','ioACIca','lwnHCMqIihm','lwHPzgrLBJ0','AweTAgLKzgu',
'iJaIihn0B3a','q2XPCgjVyxi','zxy6','Ew91DgG','pc9ZCgfUpG','zwfY','5lQA5RsY6lAZ55cd5ywi55sF','lxbPBgWG','DgvHBu5HBwu','C3m9iM92CI0',
'zM10vMfSDwu','iowCUIaVia','5Oc757Ut5y2Hpc9IDxq','C3rYB2TLlwW','yxnZpsjOzxi','lc0YmcaXnIW','zs12iJ4','ocaTmtaSndq','AwX5psjjBNq',
'DMLLDY1ZDw0','y2XVC2vZDa','y2fYzcbUyxq','iIb5psi4oci','y2LYy2XLigm','mte4ldKWiem','C3r5Bgu9iM0','ic0ZmcW1mca','iImWmdaYiIa',
'ifOIigzPBgW','lxjVDY1SiJ4','ihGXpsiXmci','pgXPBMuGy2W','iM51BwjLCIi','Bg9HBG','Aw5LAM9PBJ0','z3jPzc10zw0','Ew91DgHdDxq','y2X1yNnqBge',
'CgLK','zgvD','nIiGAgvPz2G','ywn0Aw9UyMe','odaGBc02lc0','zwfTBgDD','DwuIpUE7P+E7RtWV','BgWTB3bHy2K','pc90zxH0pG','CZ0IC2vLzc0',
'lZ48Cgf0Aca','DNn0CMLWzxm','jMD0oW','mtiGmYWXnYa','C3m9iMjTlxm','DYi+phnWyw4','5Asn5yI25zU+54Mh44cc546W5zYO5PIV','psjZDc1SiJ4',
'iJ48Cgf0Aca','nJqSmJGGmtC','CIi+pgrPDIa','Dg9Nz2XL','5Bga5zU+6yM044ccpc9WpG','yxjJAgL2zq','y3qGEd0Ioti','iIb5mJ0InJa','C3rHDhmIpJW',
'zY15B3v0Aci','ywrKq29SB3i','Dcb4psi5nci','ieWZmsa0mca','iI8+pc9JBgK','mtaWiIbOzwK','iMXNlxbYBYi','zMzMnIiGzMK','jsi+','iJ7NGRNLVidNNiVMMk/LK6RKUPS',
'BgfZCZ0IC3u','mJaWidiWmci','iK03mIWYnIa','Cc1IDg4','iJiWmciGzMK','zMLYzwq','zgLUzYi+','ig5HDci+','mcbwndaGqti','psjHCMmTBwu',
'Ed0ImZu2iIa','iZHHnMe0na','y3G9iJiWmci','lxrPDgXLiJ4','CgfYDg5LCG','iIbYpsi2nIi','C2v0psiXiIa','tte5idz2mti','ywnHzgvTEu4',
'6yEr5OMl5Awx','BMfTzsi+','pJWVAt7PNzlORQ08','pJXZCgfUpG','yxnZpsjYiJ4','yY42ltiUmIa','5zUE6AAw6Ag15lYA5l+D55wz5B2t5yMn','iI8+pc9ZDMC',
'zgzI','D2LKDgG9iJu','w29IAMvJDca','y2fUu3rHEq','ntaIihK9iJy','zw5KAw5NCW','iJi4mciGAgu','EhqGEd0Imta','yxnZpsjSB2e','AM9PBG',
'C3m9iMOTChi','Edi9iJaIihK','BNmTC2vYAwy','zsi+pgrLzNm','yM9VDa','Dg9eyxrHvvi','cUwpLUA2IowiMEMhJEAwSow8GowNIW','5Bga5PAW55Qepc9ZCge',
'ldm1igW2ldu','l3n2zZ48l2q','BMqIlZ48Cge','yw0TBM9Uzs0','ncaTmtaSntq','B2XVCJ0IiZy','BJ7OP4hOV4FNMOtNU5pLSya8','DhjHBNnWyxi',
'ic8G6lwB5A2Jpc9I','ys1ZzwvKpsi','i0veqKiWma','psi1ociVpJW','DJ48zgL2igm','zwjYB3CIpG','C3rHzMzqCMK','5PYa6AUy6lQR5lU3pc9KAq',
'C3r5Bgu','zgu4mciGC3q','pgrLzNm+pgW','zwjYB3C','yxnZpsjQzxi','lNrSlxnJCM8','nxOIigzPBgW','Aw50iJ4','iZaWnJzcmG','Cg9ZqNLjza',
'iIbMAwXSlw8','yxnZpsjJBhu','iJeUnIiVpJW','C3m9iMnHC2u','CgLJA0v2zw4','CgfUignSyxm','CZ0IC3vTlw4','psjJBhvIlwW','iMfLlw5VDgu',
'DhK9iI40nsi','AhKTCM93iJ4','nIi+','D2LKDgG','DhjVA2u9iNu','CJ0IiZaWmci','Ac10CMfKzsi','BhvIiJ48C3a','zd0I','6lwW6l+h55Qe6lEV',
'zgf0ys1JBg8','msaXnJaSmtC','psjJBhvIlwm','AxniB21L','iUI/MEs4GoAHO+s4Gos4QUI1M+wTOW','C2XVDa','Dg9HC3q','zhrO','l2rPDJ48zgK',
'psjSyxP5iJ4','lJiYkq','z2v0qM91BMq','5y+45yIx5lQg5yEG5lIQ5lQ644cc6yo9','5PIV6kkR5PU06yEn55Qe6ykJ5P2H5y6l','Dg9WlwnVBg8','pc9ZCgfUpJW',
'iJ7LPj/LIldOV4C8l3m','t2rKCW','ldeYidqSlti','5Bkb5BYa6lII','pJWVC3zNpG','CIWGugLUz0y','y29WEs1Zzwu','CgfUpJWVzgK','C2vHC29UCW',
'5A625P2L6zEU6l+h77Ym5lMF5y+V5lUL','iIbJEd0IlJu','iZiWmJqXzci','psjHCMmTzw4','iJ7LM73LRRBPMj88l2q','6jcL77Ym5A626yEm5B6x5OU/5lIQ5lI7',
'iZvMndqYnG','yNrUiIbKyxq','w2rHDgeTyxi','tteYidqUnxy','Eti9iJeIpJW','DgG9iJeUnci','i0y2rui2mq','5AsN57QMia','mcW1ocb6iIa',
'Ede9iG','yNrUlw5LEhq','BMf0qMvZDa','CgXHDa','zg9Uzsb5B3u','B29RiJ48C3u','z2LU','idKUnMWZlJC','Awr0Ad0ImJy','iIb0AxrSzt0',
'imk3iowKSEEqGYa','C3vTBwfYEs0','Dg9tDhjPBMC','77Yjpc9IpJWVCa','ChqGC3rHzMy','6zw/5B6x5B+R77Ym5yI35B6x54UG','AgvHzci+phm',
'yxnZpsjPBNa','CZ0IC3rHzMy','CMDIysG3ncW','ios4H++8JowUTUw6LEs4JEwKN+wWSq','ndaGyY0Ylde','C2fZAa','yxnZpsjVChq','pc9IpG','B3jPz2LUtMe',
'nsiGChjLC2u','iZaWnti5rG','yw1SzW','ihiIpG','pc9IpJXPpG','B2zMzxjpChq','yxnZpsjJAgK','Dhj1zsi+phq','y3G9iJeWmci','zM9YrwfJAa',
'lxn1yIi+','zgf0ys1HCMm','pc9KAxy+','D3jPDgu','CMvW','Ed0InciGzMK','zwXKlwXHyMu','zxzLBNq','AwXSpsj1CMW','zxnJiJ7NU4/NUQRKURO',
'C3bPBM5PBMC','pc9IpJXZCge','igzPBgW9iG','psjUB25LiIa','B3zYyY1SzwC','yNv0Dg9UpG','5y2b5yEG5Bkb5BcX5lIl5P2L5lQg77Ym','B3rLiJ7PPPBPOBxLOAS',
'zwLNAhq9iJe','BMvNiG','yxjKlwjVzhK','lc42kq','5RkH5OIq5A6277Ym5lMF5RkH6ic96k+V','C3m9iNrSlwm','Ew91DgHjza','C3m9iNn0ywy',
'Ad0Ims40iI8','D2LKDgG9iJe','Dd0Iy2fYzc0','y3qGEd0ImJy','igzPBgW9iNu','mtGXodiYExr3B0PM','AgLUDa','ic0XocWXoca','ihGXpsi2oci',
'vJe1mcbbmJq','iI8+','mIa2lJzSmY4','BgfZCZ0IzMK','ideWmciGyxi','y2fWCW','lw5VDguIpUwCQa','55wz5PIV6lwW77Ym6yEn5PAW6lci5lIa',
'zgLUzY1WAwm','iIbYpsi','BgfZCZ0Iy2W','iJeWmciGAgu','iowWJ+AxTUwjJq','zdDHodqIlZ4','BMrPBMCTzxK','q29UDgv4Da','AwXPioELNUADPEAiKEs5NW',
'ihKXpsi2mci','Dxr0B24+pgi','Dg9WiJ48zgK','B2CIpG','C3rYB2TLlxC','55sF5RAV5y6g5y+Y5QgJ5Qgiimk3','imk3ioIdVEwkMYa','lJKGnsaZlJm',
'yY0YmcWTnIa','mJqGmcaWide','yxj0lxjVDYi','psiXntyIigG','psiYmdaIigG','A2uTD2LKDgG','6yEr55cd5Aww','pJXSAw5Lyxi','DgL0BguIpUs4Rq',
'5zU+77Ym6k+v6k+v5OIQ5Bgp','DwX0','q1jfu1rFueW','CMvHBt0I','zw1WDhKIpUI/Ma','y29UDgfPBNm','mZmSltmXihO','iZaWqJjboq','y2XHC3nmAxm',
'pUAmIEs9JEE9RUs5N+wpQ++8MG','AY1TywLUiJ4','5lIn5OYh5A6A77Ym5OQL5lU35y2v54wN','yY1SAw5LihK','ihi9iJe1iIa','zgL2ignSyxm','igq9iK0Xmda',
'AxrLBsiGzge','BIbHCMmTy2W','iJe1mciGy3K','B3jPz2LU','C3m9iMnSDwi','iJuIigzPBgW','zI1YB3CIpJW','ihKXpsiWiIa','wu9vveHFqui',
'z2H0psi0oci','5PYjpc9KAxy+','iJeWiIbZDhi','5P2V6lwB5yAG5yAB','y3jLyxrLtgK','psiZmIiGzMK','ngjPEgnUsa','iJ48Adi+55cd5zY6',
'A2v5CW','6l+z5ys/5O6s5lIn5lIk5y+355Qe6k+D','Ec1PDgvT','5Asn5yI25AsX6lsL77Ym55sO5l+D5A2y','5Q+p5lIK5lIQ6lwB5A2J5lIa5QYH5yAZ',
'zsiGEd0I','ihK9iJaIihC','mYWTmtmGmJm','5lMi5lMF55wz5lIn5lIl77Ym5l2g5l2G','C3m9iM5HDc0','6ycj6z2s6k6T6jcL4OcM','ic0YldCGAc0',
'Awq9iMzVB3q','nteSnZmGnJi','igzVBNqTC2K','lwL0zw0','pgLTzYbJBge','Edi9iI4Znsi','5A2J6kgO546WphnWyw4','iInLywzMzJy','uK9brf9gruu',
'C2LKzq','Bg9N','57Ud5B6x5lIn6zsz','DgfN','iIbYpsi0nIi','zhrOpsi1mIi','lZ48l3n2zZ4','ihzHBhvLpsi','Dw0Ty2fYzc0','psiZmdaIigG',
'BhvTBNm6mI4','CM91BMq','iJmIlZ48CMu','Ahq9iJe0iIa','Aw9UlwHLywq','Aw1Hz2uVCg4','mtqSntGGmta','CgfUpJXJB2q','BgfZCZ0Izw4',
'5RIf56M655sF5RAV5y6g5y+Y5QgJ5Qgi','77Ym5lIl5lIa5lIw5lUo5As05yAn5P2L','DgeTC3rHDgu','CI1IywrNzs0','iJaIihDPzhq','zMLSBfn0EwW',
'CMv0AxjLx2y','55U05O6L57ky6ls0','iJ48l2K+5BoW5yc8','CNDLAwDODa','ociVpJXYzwm','BY1YB3CIpJW','mtCGlteYlc0','BgfZCZ0IBwK','Aw5LiJ4',
'iowYGE+8IE+8JoE7P+E7REwqL++8NW','BMuIihn0CM8','A2u9iImYmdi','CM5PBMDZ','iIb5psiXmci','iNvYBcGJ','ru5esu5huW','CM9Szu5HBwu',
'zgvUpsj0CNu','i2zMzIiGzMK','iK03nIW0mIa','5A626yEm55Qe5QYG5yc6','C2nYB2XSvg8','Dxr0B24Gy2W','mtiUngG0lJi','CNvUugvYAw8','psiXiIbZDg8',
'EwvK','5l+X5lMq6yoO5lIn57UT57QM5lQg','mYiGC3rYB2S','Dcb3Awr0Ad0','nciVpG','77YB5lMl5zco5AsF5B6x5lIk5A6d55Qe','B25LlwqIpG',
'mtuIigzPBgW','BNrPDhK','tdiWmcW2nIa','CMuIpUAaU+E7K+wnOsa','BJ48l2rPDJ4','ihLVDxrO','Dgv4DejHC2u','DgvTiJ48C3a','i0m5qtiYnW',
'zsiGC3rYB2S','C2HHCMuTzw4','C2HHCMuTBMu','x19tsu1urvm','BgW9iM5VBMu','ign5psiXmda','mIaYmcWYmIa','lw1HAw4IpG','zxH0','zgf0ys1VCMK',
'ihOIigzPBgW','iow8GowXGa','iI44iI8+pc8','C3m9iNn0yxq','mIbJnYWTmtm','ns0XlJCTnc4','pJXYzwn0ihG','5lIk5A6d77Ym6io95yAn6lwW5lIa6ygn',
'yxnZpsj0CM8','5lIT6lAf5PYa5l2Z55cd5zgy','zgf0ys1Zzwu','khbYzwzLCNm','iJ48DgL0Bgu','57Ut5Bga77YA','C2uTDci+5Aww5P2V','57Ut5Bga5zU+6yM0imk3ia',
'yNvPBgrqCM8','B3v0AciGzd0','Dg9Wig9MzNm','BMf0qxnZAxm','pc9NpJXJAxi','D3jPDgvuzxG','zMLSzq','5lQR5zU+54Mh','zxGTDg91y2G',
'DJ48zgL2pJW','CMWOiW','CMvWBgfJzq','C3m9iNn1Bs0','C3rYB2TLlw8','iIbWywLUDc0','zgfS','Dd0IndGIlZ4','yM0TC2LKzsi','Et0IotiIihC',
'lde4mcW4nIW','y29TChv0zvi','DxnLCKfNzw4','ideGndaSmty','iK03idz2mti','CMqGyxDHCMq','5PUY57Q/77Ym','yxjJvg8','zxq9iJeIihm',
'psjLDI10Axq','mti3lJaUmc4','C3m9iMnYzxm','BgfIzwWIpUwnOq','mta2ldeXnIa','yMvSpslOG73LIPVLGlW','zg9JDw1LBNq','zMXVB3i','Bw9Kzs1ZzwC',
'zt48l2LTywC','BwvUDgvKiIa','6l+z5lIQ5Rwp6kEi5zMO5lIn57Uz5Asn','i2u4yZq3nW','y2HHBgXLBMC','Dd0ImJqIigy','lwnLBgWIpJW','ldK2igG5nci',
'yxjLyq','B3bW','5PEN5ywO5yET6l+q5Rcu','ocaYnsWTmYa','BM9YBwfS','yxjJAgL2zwq','y29TBwL0rxy','Awr0AdO','iK02ncW1nIa','lJa1kq',
'yNrUlxn0yxi','mciGAgvPz2G','zxGTEci+W5C','ywrK','5yQB5yc85PUY57Q/pc9ZCa','B2XL','BgLMzq','iIbKyxrHlwy','psi4mdaI','lJKIigXLDhq',
'CMvZDgfYDa','zMzMzIiVpJW','Bc1JB3vUDci','DxmTCM93iJ4','5lIw5l+X5P2V5yAG5yAB','mJaWiIbJEt0','zxiSifbPBMC','nciVpJWVC3y','ideGmsa5os4',
'B3m9iG','yxjNAw4TDg8','mZaXotu0nen6r0rXBa','AhKG','AcbKpsjnntq','lc4YocK','BgvMDa','5zci5zcm5yIW5PYF5lQg44cc6l+z5yEG','zgv4lxrPDgW',
'jNf1B3q7','5yAZ5A6A6lw35Q2L6io95yQB44cb5A62','Ec1KzxnJiJ4','BuLK','ywnHzgvTEq','AdyWigmWldi','zwy9iG','lxDPzhrOpsi','yxjJlwnSzwe',
'DciGDMLLD0i','Ahr0Chm6lY8','Awr0Ad0Imti','qvrf','i2m2ogyZmW','CM9Szq','C3rHEq','Aw5LyxjhCMe','BIi+','mZiYiIb5psi','Aw5KzxHpzG',
'igfYAweTBge','nIW1mIaXnJG','B25LiIbZDhi','DhLSzt0Ils0','yxjLxq','mJaWiIb5mt0','C3m9iMjTlxy','iMnVzgv4lwi','ls1Hy2nLBNq','ndqSmZyGyZe',
'5QgJ5Qgi5BEY5RIf56M6','Ahq9iJi0iI8','iI8+phjLy3q','BIbIDg4TChi','iM9WDc1OAw4','CciGDgL0Bgu','D0jVEd0Imca','BMf0Aw9UywW','AwXSpsiJmda',
'ndGZodm3qvnJze5Y','DgG9iJiWmci','jMfTCdS','u3rVCa','ldeWmca3mIW','psiYmdCIihi','pc90AxrSzt4','ksiVpJXLBgW','yYi+phnVDxi',
'A2uTB3bHy2K','ihGYpsiXmZi','mIiGC3rVCc0','yM9KEq','Axy+pgrPDIa','ywzJCg95','iZm0zdm5osi','BMnOB3i9iG','ioMBTUwWGq','Ahq9iJiWiIa',
'yxmIpG','pgj1DhrVBIa','BJ0IDhj1zsi','Ad0IntiIigG','C3m9iM9WDhm','lZ48C3rVCca','zxKTD3jHCci','pc9KAxy+pgi','zgXLiIbMAwW','pgnPCMnSzsa',
'CM5K','C3m9iNnLy3q','iIbZDhLSzt0','iIb4mJ0I','5Asn5yI25lIn5lQg77Ym5BEY57Up5AgR','pJXIDxr0B24','yxnZpsj0Bc0','6kkR5OQ55O6j55Qe6ykJ5lIa5lIw5lUa',
'zMyIigzPBgW','5y675zoQ5ys/57Ud','mdaGmtyWiIa','BgW9iG','mJqIlZ48l2W','5ywZ6zsU5OIy','BgWIpG','AgfYzsi+55sF5OIq','iIb2ywX1zt0',
'B3i9iImXntu','CM91BMqIlZ4','BMv3C192zxi','y2X1yI1YAwC','C3vI','ChqTBgvHzci','zY1NBg93iIa','Ec1NCMLKiJ4','Bci+','B2zMzxjZ',
'iIb4mJ0ImJK','iI45iIbVCge','ys1Hy3q9iMe','CcbHD2fYzci','AhjLzG','lxDYyxaIpJW','Dgv4Dc1HBMm','C2LUy2u','CMvZB2X2zq','5BYa5AEl55sF5RAV',
'BgfZCZ0Iy28','pgrPDIbJBge','DNjJlxmXiIa','pc9IDxr0B24','iJaIihK9iJC','ywrKqxDHCMq','iIb2Awv3qM8','DgHHBMTZ','6k+D5BEY57Up6k+05yE65y+J5lQg',
'C3m9iMjTlwW','pc90zxH0pJW','idXZCgfUigm','ihDPBG','iIbOzwLNAhq','5PAW55Qe44cc','ncW0idiWlde','CM9Rzs13Awq','B3mTyNrU','yMfSBa',
'tYbwifi','iI8+phrLEhq','CgvUCW','As1IywrNzsi','mciGEt0Imta','z29Sza','zwfKiJ48C3a','6lwW44ccpc9KAxy+','ioESRca','tteWmcWXmIa',
'zgf0ys1LCa','C3m9iM9WDc0','ywLUiJ48zgK','5zU9pc9KAxy+pa','Dg4GyNrUlxa','ihG9iJK0iIa','DgfSzw50','ideUnu0Xos4','iJaGmcaXmda',
'zt0Ii2uWoge','C3m9iNjLC3u','4Oc6pc9KAxy+pa','psjJCI1SiIa','CMmTy2XLyxi','psiJmdaWmIi','A2LUz2n1Ca','yxnZpsjZBg8','Bci+56En5A2qpc9Z',
'zgrHntfMiI8','6yEn5PAW6ycj5yE66lQR5BYa5lIa5Bga','6l+z5lQB5BM05y+Q5PYj55cd','zxiTC3bHy2K','Adu2igmWldm','ywnJzw50','mtiYnJi5ntLQt0D1ufy',
'lc4WnIK','BgLUzvDPzhq','y2fYzwvYlxi','Bg9YpsiJmgu','yMvZDa','77Ym5l2g5y+V5lUL5O6L552a5B6a5lIl','l2rPDJ4','Dci+pgrPDIa','BMf0iJ4',
'zw5KAw5N','Dc13zwLNAhq','lwrYzwfTiJ4','ttmGmtbOmtG','BIbJBgfZCZ0','zxq9iJaIihm','AgvSCc1TB2q','Ed0ImJyIihi','zw5KuMvHC28',
'Bci+t1zspc8','iIbZDhjVA2u','Cci+8j+pHIa','iIb5psi','w2rHDgeTCg8','lxjVDYi+','AwrKBguIpG','iZHMnweYmG','igXVC3q','z2XVDMu',
'iNbSyxLLCI0','5PYj5lIa56En5A6A5OcNiokaLokaLa','CMvHBt0IiJ4','y2XHC3m9iMe','Awr0Ad0ImY4','idiYldeWmca','Et0ImtiIihi','C3m9iNbSyxK',
'ChvZAa','z2v0rwXLBwu','BgvHz3vL','B2XVCJ0Ii2y','mJaGtdq0ide','y2XHC3m9iNm','iJmIigzPBgW','Ec10AwvYiJ4','AxnozwfY','AwvUDd48l2q',
'C3m9iMnVzgu','ywnPDhK9iI4','B3jJzwq','D2LKzq','AwrLBNq','AhKTyMXVy2S','mJqIigfYAwe','iJKIihn0CM8','CMvTB3zLsxq','C3m9iNbPDgm',
'EgXLBMD0Ad0','Dcb4psi2nIi','AwvUDa','iJ48l2rPDJ4','lxrVCdOXlJe','tKfux1nit1i','Ew91DgHmB2C','yxn0','Awr0Ad0Inti','57Ud5lIa5BM0',
'Cg9ZuMf0zxm','Dg90ywXZ','ihjLzhvJzsK','mtiIlZ48Dgu','CJ0I','jtWVC3bHBJ4','AwrLiJ4','y2HPBgrYzw4','yxDHCMrZ','56EF5ycF5PYF5RUH77Ym5zUE5yIWia',
'refuqq','tdiYldm0ifO','5Aww5P2V5P+C5PIV56M655Qepc8','5Bga77Ym6l+z6yEm5lYA55wz5lIl5lIa','C3qTDIbYB2W','DhjHBNnPDgK','psj1CMWOi28',
'igXVC2u','A2v5','iM92CMmTBwK','pc9JB2rLpJW','yxjJlxjPz2G','ios4LU+8JoMhJEAwSoAkLEs4GoASOq','BgvNywn5rNi','CMDIysGYntu','ig9MzNnLDd0',
'lc0XmYaYnsW','DgvHBuj5swq','psiYmdaIigy','44cc5l2G6iEQ5BEX5lMF55+L6ygt5PIV','iZaWneq5oa','5RkH6l+B6l+h5zU95A626zIF','B2LJzxm',
'Bw1HCNK+6ycq6lwB','DxjSkcm','BhqIpG','Bg9YpsiJzMy','5l+X5lMq6yoOpc9ZCge','mtyGmJKGwIi','57UtiowXGa','iI8+phbHDgG','zhjHD0LTywC',
'iZHKowfHnG','D3jHCa','AwrLpsi','DwvS','CM91BMrLza','C3bHBJ48l3m','AwXLlwqIigm','CM9SzsbYB2W','y3vW','zxnJiJ4','B3i9iImXmdq',
'yNrUlw9Wzw4','AxyGC3r5Bgu','mIiVpJXZDg8','5Pwi5yQB6l+h','y2HLyxq','zxq9iI41iIa','5lUL5Aswpc9OmJ48','DhLWzq','iJi0ociGCNG',
'ihDPzhrOpsi','zxjZzxKV','C2v0sxrLBq','CgLJA09MzMu','BM93','yY40ideUnI4','lwnOAxaIpG','mtuIihj4psi','imk3iowkQEAuUYa','ttiUnsaXnI4',
'yxbWzw5Kq2G','nc43idmUosa','iK03ncW0nca','BMf0r29HBhm','lti4lc02mIa','i2zMzMfLyq','psiJzwfMzMy','yxjKlxLLyxi','lJe2kq','zgvSDgfZ',
'BguTAsiGAhi','5lMF5lIa5A6A5PYj5A6d','y2XHC3m9iMW','DhjPBq','zdzJmgmIlZ4','iow5ToIIQ+wiT+s4I+ADPq','Aw5LihGXpsi','AwnVBG',
'Dg9JB21WBgu','ihK9iJeXnci','psjLDI1Py28','Ec1HBgLHCYi','zw50pJWVzgu','6l+y6kAb6l+h5lIa5QYH6ycj5P2q6ykJ','ig1PBJ0Imsi','zwfYBMvK',
'ywrPzw50igK','iZe2mteWyq','ldGWigW2lc0','oca5lJzSltm','idiWBc0YlJu','ic8G','ihDVBG','CMDIysGWlda','Ew91DgHFCge','55sF5RAV57Ut5P2Fimk3ia',
'DwiTyMfYlw0','ihiGAgLKzs0','yw1L','DMLLD0jVEd0','ioADOEE7K+wXGdXZCge','zw5KAw5NC0e','5lIa5lIQ5A2P5A2q','ocbSltiSltC','yxrL',
'iNrYDwuIpJW','5lQg5BcX5Q+p5BM05lUo5l2G6lsM5lIk','ign4psiXnta','CMvHBs10zwe','C3m9iNrHzYa','zw5KAw5Nq2G','6l+h5lIa5QYH6k6T57Ud',
'iowiHUMsN+wjJq','yMX1zsi+iW','B290psjSzwy','BMf0q3m','DgfYz2v0','CMnSzsbJEd0','C2HPzwXK','B3zLCMXHEq','yMLSAwjPBgK','mIaYmcWZmIa',
'5lU2iokaLokaLcdKUidMNAhNLj8','iJi2iIb3Awq','yNv0Dg9UpJW','imk3ioI/M+EqGYa','Dgv4DenVBNq','AcbKpsjnmtm','Aw1Hz2uVD2u','C3bHBJ4',
'C3m9iNrYB3a','ihG9iJeWmci','y2HLy2TLza','AwXSpsiJzwe','DMvUDa','z2v0qxr0CMK','5lIk5lIa5lIw5lUa5lMi5lMF5RkH55wz','m2iYyIiVpJW',
'y29UDhjHy3q','l2XPpG','BcGJ','BwLUAs1IDg4','zwXSiJ48C3a','iIb3Awr0Ad0','B3jPz2LUqNK','mciGEti9iJi','pc9IpI/OTzVLRAm8','ihn0CM9Rzt0',
'BwfYCMLLza','C3m9iNrPBwu','ihn0B3aTy28','DgeIpG','C3rHzMzgzwu','ioEqGYaVia','B3zYyY1UB3q','phn0B3aGB2y','B3rD','osaTmtKSmJG',
'psi2mciGCNG','AgvPz2H0','ExbLpsjPBwe','ywCIpG','BMvQB2LUpsi','5OQ9562+5lIT','ios4LIdcTYdNU5mG5Bga','iIbKyxrHlw4','iZeYmtqWzG',
'imk3ioI6Q+wUTIa','mciGChjLC2u','C2vHC29Ulwi','igvTChr5','iZaWmdmIlZ4','lwrLC2mIpG','ywjZ','psi5iIbMAwW','CM93iJ48C3a','C3m9iMXNlxq',
'iowYGEMcO+w5TowCQos4GoI1TW','5QYN5yAG5yAG5yAB','idiGmI42idi','DNjJlwrVDa','psiXnIiGzMK','y2XZ','DhjVA2uTBgK','nsiGzMLSBd0',
'B250lwzHBwK','ldC2iemXnJe','ncaYlJuTms4','pJXIpG','Aw5dAgLUyq','6iEQ5A625lIa57Q/6zIF','zNqGwwfizwK','igLKpsi','6l+h55Qe5lYA5lQU6lw35P2L44cc5RkH',
'nJnLiI8+phm','ocWXncbnodq','iIb4mt0Imci','i2uYyJq1nG','5zU+6yM0pc9OmJ48','C3rVCc1JB2W','zt0ICMDIysG','iIbKyxrHlwe','Aw50iIb2Awu',
'lxjLzhvJzwq','lwDLBIi+56YSia','y2X1yKnVDw4','z29HBhm','psiJmdaWmYi','zIi+','B290psjYAwC','5PIV5OYj6lwB5A2J5lUy6zkX77Ym562+',
'Awr0Ad0ImIi','y2XHC3m9iMi','BgLUzsi+pgq','AxyGy2XHC3m','C3rHzMzhB24','y2XHC3m9iNq','yxnZpsjVDNi','yxnZzxrZl2O','yxj0iJ7KU47LPltMNAu',
'zwLNAhq9iJq','6lwW5lQgpc9ZCgfU','CcbVzMzZzxq','nsbOlte2igW','iNrHzY1YB3C','mZGXiIb3Awq','iJmWmciGAgu','B3CIpJXZCge','Dcb4psi2oci',
'phrLEhqGy2W','l3nWyw4+pc8','y291BNq','zcb0Bc1JB2W','nZmGmtm4ldC','77Ym6yEn5PAW5OQv5lIa5QYH6ioo','y2X1yIi+','sgvPiIWGC2e',
'CIi+','iZe3mtCXyW','lte0lc04ic0','lMjHCI1M','AwCIpG','y2XHBxa','iIbKyxrHlxa','DMvY','idq0ieW2osa','546W5zYO5OYh5A6A55Qe5PIVia',
'psi1mIiGCJ0','zgf0ys1TB2q','yxnZpsjWAwm','5B2t5yMn5A2y5QgJ5lYA5RIf5O6j77Ym','ihK9iJeWmci','ls13yxjUAw4','zgL2pJWVzgK','DgL0BguIpUIdVq',
'AhnSkhzHCIG','yxnL','B2zMC2v0v2K','BwfYEq','iZaWmdaWma','6z2s6k6T5REy5RgW','lwDYAwqIpG','C3rHCNq','ldi0idaGmca','ntfIiI8+pc8',
'CgfUpJXPBNa','DfjLy3q','iIbJEd0I','6i2J6kQj77YA5lIa5lIQ5RkH5PYj','iJeWmciGy3K','iJqWmciGAgu','y2fUuMv0Axi','iIb4mt0IlJe',
'zhrOpsi0mda','Ed0IndGIihK','BJ48C3bHBJ4','lwXHyMvSiJ4','psiXlJiIihm','BNrcEuLK','BMjHCG','ms43idmUns0','ig1HEd0IotK','DNjJlxbLywS',
'B2rKCW','yxbWBhLszxm','5RAV5y+Q5lYA6jc95yIW5yw25lIT5lIa','6Ag25Bcw77Ym5lMF5PYa5OYK','ncaYosbmnZC','psiYmIiGzMK','zsbJBgfZCZ0',
'psiYnIiGEt0','Dc1SywiIpG','psiYiI8+pgW','yxjJlwjHy2S','yNrUlwjHy2S','77Ym5ywi55sO44cm5l+D5A2y5zU+54Mh','tteXidz2mti','BgW9iImWmda',
'lw5VDguIpUINGq','6io96ycj5yIW5A6d55Qe6z2s6k6T6jcL','lw9MzMvYiJ4','ios4QUwTQEwTKa','Ag9VCa','mcWWldaSlJm','Bc0XlJmTnhO','Ag9ZDg5HBwu',
'ihKXpsiYmdC','C2HHCMuTDgK','iK00ncaXnsa','ign5psi1nIi','CMmTD2HLBIi','55Yl77Ym5lIn5B2X5zon5lU75l2v5PwW','yxnZpsjKzwW','ls1PBMzVksK',
'mJaWiIbMAwW','ihi9iJmIigy','Dd0I','DgHLBG','C3m9iMv2lwq','CxvHCNrLCNm','ieWXntGSnda','yMLNBwf0y2G','5Q+p5lIQ6lwB5A2J5lIa5QYH5yAZ562w',
'phjLy3qGEd0','CMLK','yY1Iyw5K','y3q9iNjLC3q','BMCIigrHDge','lwvUzgLUzZ0','CMLNAhq','C3m9iMfLlwK','5lIk5lIa5Q2L','otqSmte2ide',
'DgG9iJe5iIa','iI8+pgXPBMu','AwDPBL0','zJyIihn0CM8','iMrYzwfTlw4','BMf0uNvUCW','y2fSAg9ZDca','C3m9iMXPzMu','6iEQ55sX6lQR',
'iIbKyxrHlxm','ldaSlJu1kq','yNv0Dg9Uigm','zNvUy3rPB24','5l2G5zUE5P2L5lQg44cc6l+z5lIa5BM0','lteUnIaZlte','lM92CI1Iywq','psjKCMvHBs0',
'ign5psiXnta','tevbr1vfuW','BgfZCZ0IB3y','zwn0B3i','zgL2pJXIpG','55Qe56En5A2q5Qgg','CZ48y2XPCfa','BgL2zxm','y29TCa','BgvUz3rO',
'77Ym6l+y5y+V5lUL56EF5yE65y676lII','yY1SAw5LiIa','AYiGEd0I','y29KzxGTyxi','Bc1Uyw1LiJ4','zwzZpJXYzwm','nhjLBsaXzNi','zM9UDc1Myw0',
'DMLLDY1PBNq','zgLHBeDYywq','lNbUzYiGywW','y2fSBa','5OspiokaLokaLcdLPB3PMj/KVi0','DgvHBuLK','mtiYldCYide','5A2y5QgJ77Ym56gU5A6A77YF',
'y2XHC3m9iNi','CMfKAwfSr3i','iIb5psiXmJy','C2LSDMvY','C3m9iMnHCMu','ihi9iJe5iIa','iK01ncW2mca','ndaGtdy5idG','y3vYvgvHBq',
'ociGzMLSBd0','C2f2zq','5lIT5lIl5RI4','CJ0IiZbImZi','iIWGiK1Py3i','iJ7MJQxNNydOV5NKUidKUjBVViW','zxDcB3G9iJa','ign5psiXmIi',
'iImWmdaYiI8','y29SB3i9iIm','DMfSDwvpzG','igrYzwfT','ihG9iJC0iIa','zguUy2XHC3m','5RkH5lQ65P2L6zEU5lQg','ldiXncWXmZa','ociGAgvPz2G',
'ltmWlc0YmIa','psjHCMmTBw8','ios4LIdcTYdNLj/MTQ/NU5m','CZ0IC3qTDIi','DgfNiJ4','mJuGndyGmJa','B3zYlwjHzgC','zw1WDhKTy2e','ideGmtySmtu',
'lc0XmIaTmJi','yxnZpsjHCMm','i2yYzJzLzG','B24TBwLUAsi','ihn0EwXLpsi','igjHzci+','psi3mIiGEt0','BMC9iI41iJ4','Cci+','igDVDa',
'zwLNAhq9iJu','mcWZncbSmtm','Cc1JB2XVCJ0','y2vPBa','mJGSmZaGqZe','pc9IpJWVzgK','DxqGy2XHC3m','5zcm5lIa5Q615lQ655sFpc9Z','BM9YBuXLz2e',
'Es1JyxnLlwK','ihG9iJaIihK','5PIV5zUG5lI65lIa5Q615lQ655sF5y+Q','5zcmpc90zxH0pG','yxnZpsjJB2q','5lIa5BM0pc9KAxy+','zgf0ys1Hy3q',
'ttmGnMGXohy','BNqTC2L6zt0','mIiGAgvPz2G','z3jVD3rOuMe','lMnVBc1Jic4','AMvJDfvsta','C2v5ig1PBMK','CMDIysGYmJy','mtiGtteXnIW',
'5P2L5yEG5BM05zYO5zoQ5A626z2s6k6T','BM90zq','BguIpG','Dg90ywW','Ee1PzfLnAwq','ms40sdiUnxO','pJXKAxyGy2W','z2v0rgf0zq','Dg9vChbLCKm',
'lJq1iIbZDhi','CgfUpJXZCge','C2nYB2XSqNK','psjJCI1SiJ4','ldi1nsWYntu','Aw4Ty2fYzwu','psiXlJyIlZ4','D2fZAci+','6l+B6AAw6Ag15lQg',
'lc4WosK','y3rZ','Dgu9iM9MzIi','BgfZCZ0IyNq','ywrPzw50pJW','iIbZDg9Wlw8','5B+d5lUQpc9ZCgfU','Aw4TBMfTzq','Aw1N','BNqIpUw3SUMaIsa8',
'Cd0I','5yI277Ym5BEY57Up5AgR6l+B6AAw6Ag1','psjJB250Aw4','nIbmmZeGody','5lIl5y675RkH6k+05yE65y+J44ccpa','yxnZpsjHzs0','mtiIigHLAwC',
'DJ48l2rPDJ4','pc9KAxy+pha','77Ym6l+z6l656yo955Yl5zYO55Y86yEm','B3zYrw5K','psj0Bc1Ozwe','iowkQEAuUW','AgfZt3DUuhi','ios4QUI1M+wTOYdcTYa',
'z3LYC18','CgvKzxn0ywW','C2HHCMu','55sF5RAV5y6g5y+Y5QgJ5Qgi','ywuTzM9VDci','w2rHDgeTzxa','6yEn5PAW6ycj5yE66lQR77Ym5BYa5lIa',
'nJyGmJaGtdG','yMfYCYi+','B3b0','C3m9iNnLCc0','B3zYyY10Awm','psjJB2WTyIi','Dci+6zQp57Yypc9Z','BgWIihGXpsi','B3v0Aa','6ykJ5lIQ6zsU5zcN',
'C2v0psiWiIa','5lIw55wm5P2V5yAG5yAB','5zYO5zY65lIk56Uz5zoQ5ys/','y2X1yNm','zMzZzxq9iJe','5P2H57Ut5Bga','idiWife1mca','zd0ItteYidm',
'C3m9iMfLlwG','Bg90lxbJDci','C3m9iMv2lwG','ign5psi2mci','zwXPBMu9iM0','ic8G6lwB5A2Jpc9Z','C3bSAxrZ','nIaXndiSmtC','5A2P5A2q6yEm77Ym5l2G6lII5B6x566x',
'6k+L5lIk5OIq5BM06zIF5lQg','DhjHBNnMzxi','phbPy3r1CMu','AdmWigm0ldi','C3m9iMOTzMe','B2TLpsi','ywXZBY1LBMq','ywDL','BwvLDci+phq',
'DJmUnK00lJi','C2v0vhjHBNm','C3m9iMj0BI0','yxnZzxrZl3q','imk3ioELNUADPEAiKEs5N+AxPq','DguIpG','jtTIywnRz3i','ldC2idCWldC',
'pJXPBwCGy2W','y2XPCc1Wyxq','zwDHy3KTz2u','C3m9iNbHBMu','BNvSBa','AwnR','zeL0zw0','ihnT','phnWyw4+pgi','CM9WAgLLCY8','Bci+5lIQ5lQ65Aww6Ag5pa',
'i2zMzdC1zq','Ew91DgHuzwe','6io95yQBicS','mtyWieeYncW','yMfUBMvK','B2jQzwn0','iIb5psiWiIa','iK0Wlde0oca','ChqGEw91DgG','mY40lJmUoc0',
'AxzL','6l+z5lIQ6k6+5Ash5lIn5PsV5OYb5yIg','i0rbmJKXqW','5P2H5lU25lMF5RUH6lAZ5lQg44cb5y+Q','DhjVCgHPzxm','ldeWocbOnJa','Dg9UpJWVzgK',
'yxnZpsjIDg4','iIbZDg9Wlwm','DgHHBMTZlxi','6l+y5yMPia','z2H0','C3m9iMv2zw4','AwDODd0ImZa','Aw5NCYi+phm','mciGEt0ImZe','DMLLDY1Jyxi',
'BgfZCZ0Iy3i','5yMP5lIl55Qe6zEU6Aky5y+Q5PIV5y67','ns01idmUns0','C3m9iNjVDwW','DhrVBJ48C3a','B2zMC2v0psi','Aw5WDxqGy2W','5zcn5A2x5zkm5y+356cb5y+Q5PIV5Aw9',
'6l+z5y+W6k6+5Ash5lIk77Ym5RIf5lQg','BMqIlZ48CMu','yxrHlwfJDd0','44ccpc9KAxy+','DwnS','tte1idz2mti','y29UDgLUDwu','C3m9iMfYyY0',
'nwnMnJiIlZ4','Esi+5RkH5PYj5lU75l2v5OQL','tdiZidq0ieW','B290','iJu4iIb5psi','5lIT6lAf5yAG5yAB','zMfTzq','AgLUDci+','yNaIpG',
'z2uTDG','psiUmduIlZ4','C3m9iMHHBgW','C3m9iG','C3m9iNrSlxi','zJnJnciVpJW','u1vque9sveu','BwCIihnYyZ0','ns00lJj6iIa','jtT0B3a6',
'ywjZiJ4','y2HVB3nL','psiJzta4ytC','yNjVDYi+','C3rHCLbVD2u','C3rHz2vpzG','iemXnZGSmtq','C3vTBwfYEq','y29UDa','DMLLDY1JB2q',
'idmYldC2iem','C2v0psiUmZu','Aw4TC2vLza','mY42lteUnIa','Dd0ImtqIihi','y2XPCgjVyxi','iIb4mt0I','yY10AwnRiIa','5yE65zY6ia','pJWVAt7OGyZKUjO8',
'iI8+phn0B3a','DgWTy29SCYi','i0zcrteYmG','iI8+pgnPCMm','56En5A2q5BEY5Asn5yI277Ym5lMF5AgR','57Ud5B6x5B+R77Ym5lMF5yI35B6x54UG',
'yxnZpsjQlwK','ms4XidmUmY4','Es1JyxnLiJ4','5RkH5PYj5yAn6lci55Qe5Osp5OcD44cc','5BcX5yIW6l+z6yEm','B2XVCJ0IiZK','DMLLDY1Pzgu',
'nJGIigHLAwC','B25SB2fK','idaG','BIbYiJ4','i2u2zwnLna','Bwf4t3zY','C3m9iMPLCNm','z2vU','DhjVA2uIihm','y2XHC3m9iMq','5y6g5y+Y5QgJ5Qgipc9OmG',
'C3jJ','B2CGC2vJB24','C3m9iNnLzwq','iowYGtWVDgL0Ba','Bw92zq','ide4ncW0mca','zhjLyw1jza','BIi+pgrPDIa','DgG9iJC2iIa','AxbZzsbJEd0',
'EhrSAwzL','Aw1HCNKIigq','psjTB25LEs0','psiXmtGIihi','mIWZmIaXnJa','5zUI6zIFpgi+','ihnHBNmTC2u','y3K9iJm2mYi','zw5KAw5Nlwq',
'y3nS','mIa4mIW5mca','C2LTDwXHDgu','mtiG5Bkb6ykJ5Q2L5lIa5A6A','5zYO5Asw6z2I6lII55Qe5Q+p5lIa5zY6','psjMAwvSzc0','mciGD2LKDgG',
'yxrOigLKpsi','iJ48zgL2igm','AwzLlwLJBYi','lti4lc0Yoca','iJaIihK9iG','zNnLDd0IlJy','C3m9iNjLC3q','phbHDgGGy2W','psj0Bc1JBhu',
'B2TLpsiJnge','zw5ZiJ4','yMfJA2DYB3u','Bwf0y2HLCW','ihK9iJGWiIa','Aw4TBNvTyMu','yxjLpsi','EwXL','iowYGE+8JoIdVEwkM+wKN+w+L+EDGa',
'B3jK','57UT57QM77Ym5lIa5yIh54wN5PENia','zgf0ys1KCMu','iI8+pgLTywC','Ad0ImIi+phi','ic0ZnYWYoca','yY1TAwXLlwW','CgL0y2GTAgK',
'y3jLC3qIpG','pgXPpJXIpG','lw9WDciGzge','zMLSBd0Ii2y','lwnOAxaIpUESRa','mtyIlZ48CMu','iIbZCMm9iG','iIb4mJ0Imci','imk3iowUTUMhJowhUIa',
'jImZotS','562w77Ym5lIa5Bga5lQm5y2b5yIg6zkF','kcDNB25LjYK','psjJB2WTyYi','zwfTxq','CZ0IB3zYlwi','CgHHC2u','5P2Lpc9ZCgfUpG',
'zYKP','y3qGD2LKDgG','iIb5mt0I','y3jVD24','Edi9iI40iIa','yxj5pG','mciGyxjPys0','osaYlJGGms4','q1jfu1rFvvi','Bwf4','C291CMnLihm',
'ihbSyxrL','o2nVBg9YoIm','nIaXmZaSnZi','CMLTyxj5iIa','BMf0u3rHDhm','BwLKzgXLiIa','5zco5P2L5y+i6ygh5lIk5lQg5lQ6','C3m9iM1PiIa',
'iJ48Adi+5Aww5P2V','BJ48l3n1Bw0','owfHogiYiI8','igW4ldi4igG','iJ7OG73LIPS8l3nW','BwfW','zgL2pG','6l+B6AAw6Ag155Qe56En5A2q5Qgg',
'lxn2zYiGDMK','4OAsiowpKEMaGEE7MEACI+wpIYa','iIbMAwXSpsi','pgrLDgfPBhm','pVcFPyCG','iJeXnIiVpJW','AcbKpsjnnZi','imk3ia','C2vLza',
'Aw50zxjWB2W','psi5mIiGD2K','5RkH6lwW5yIW6ykJ5lIa5Q2L','CgvUzgLUzW','C3bPBG','Cgf0Ag5HBwu','5yIg5BYa6l+hia','z3jPzci+','iokaLokaLcdMJQxKUiVMNAxMMk8',
'5QYN6igu5yAG5yAB','Bci+pgrPDIa','yxnZpsjJB2W','C2vSzwn0zwq','C3rHzMy','lc40kq','CMLNAw49iG','Aw5Nq2XPzw4','ttmGmtrOmtG','BMnOB3i9iMu',
'owWTlJyGns4','pJWVzgvMCZ4','yw4+','DxjLpG','zMLSBd0IBM8','57Ut5Bgapc9KAxy+','77YF6l+z5lQB6k6W5B2v5y+Q5A2y5zYO','ntyGEIiGzMK',
'nsbrntaGmJe','psjLBMrPBMC','mZiGndaSndG','twLJCM9ZB2y','BNrmB2fKzwq','C3m9iMvTChq','iowYGsdcTYa','C3m9iNbPy2S','ywrKrxzLBNq',
'AwXSpsi','w2rHDgeTywm','C3rYB2TL','zw5KAw5NvgK','zgLUzY1LEwu','tde3ocWXmda','psiYnZqIlZ4','5y2b5lQm5Bkb','CMvWB3j0','yxnZpsjTAw4',
'vevbtvm','ihK9iJeYmIi','ksiVpJXYzwm','i2nMzdHKzq','BMq6','zg93BG','yI1Uyw1LiJ4','y2fUu2HHCMu','ywjLBci+5y+356cb','D29U','CMv2B2TLt2i',
'EhmIpG','BJ7MNidPQ5GGt1zs','5A625BQvicS','iZe2mtyWzG','ncbindaGqti','igXVywrPBMC','DxqIigLKpsi','nc41iIb3Awq','ksiVpJWVzZ4',
'nIWYic0XmIW','iIbJEt0I','odG0mJq1uNLJzxnP','psi4mdaIpG','pc9KzxrHAwW','zY10ywi','BhmG','6ykJ5lIa5BM077Ym5OQL5lU35y2v5lIk',
'CI1SiJ4','yMfKiJ4','44cm5l+D5A2y5zU+54Mh44cn77Ym5y+M','ksiVpG','C3rYB2TLpsi','BgvHz3vLqNK','B3b0CY1HBhq','Awr0Ad0Imta','zxq9iI41nsi',
'C3rHCG','Aw50CM8TD2K','l2j1DhrVBJ4','zxr0zsiGzge','ywnRiJ7LM57PPPBPOBu','6l+z5lIa5lIw5RkH55wz5lIl5lUa5lMi','w2rHDgeTC2G',
'yw4Gy2XHC3m','mtaWideYocW','5P2H5PE277Ym6lAk6z2G5lIk55Qe6lAk','z2uVD2vICci','Ag9YpsjTAwq','nc0YlJuGmY4','pUE7K+wXGowpQUIVTos4GoADOE+8Ja',
'CMv0AxjL','B24+pgrPDIa','5ywi566x44cc5Qch552a44cm5AsF5yIW','y2XVC2vqyxq','ihGYpsiUnYi','nZiIigHLAwC','i2zMztLIma','lJyP',
'pc9KAxy+pc8','ksiVpJXNigy','iIbJEt0ImJa','psiJyZLKmMm','DhbZiowsJcbSBW','ihn0CM9Rzs0','i2zMzMzMzG','DhK9iI44nsi','y29KzxGTyMe',
'AYi+5zUE6AAw6Ag1pc8','ue9tsvrjt04','zw5ZiJ7NGRNNKimG','BMf0','l3n1Bw1HCNK','Bw9UzxK','zwzZpG','zgf0ys1ZAge','zciVpJXWyxq',
'zxnJiJ7NU4/NUQRLHAW','C3m9iMrLBhq','tdaSmtK2ifO','rvzftLrt','psiJngfKztG','BhK9iKLUDgu','psjJCI12iJ4','6i2J6kQj77YA','yNrUlwfYy2G',
'BwuUy29T','qZG2ldeWnIa','BgLZDci+','BgXIywnRiIa','5lIQ5lQl5lU2','yxjKlw5HBwu','DwuIpJXKzwy','igfYAweTAgK','CMvWBgf5','Bgu9iNDPzhq',
'phn2zYbJBge','iMvWlwXPC3q','C3m9iNLVDxq','mdCIlZ48y2K','Dgv4Dcb4psi','y29Yzsi+','iJ48Adi+55sF5RAV','tdiWmcWXoca','55wz5lIl44cc',
'B2TLlwXPBMu','yNv0zq','ksi+phjLy3q','l3nWyw4+','y2L0Et0IlJC','ChGGiKLUDgu','nIiGD2LKDgG','rwXLBwvUDa','yxnZpsj0AhG','psiWiIb3Awq',
'CM9Rzt0I','lxrLEhqIpJW','5lMl5zco5lIa5lIQ5lQ6','z3vHBNHP','ioI/MEs6M+MdVEwgMEI/M+s6HUE7KW','zwvKiIbKyxq','AhKTAgfSBca','ChqTBgfIzwW',
'iIbKyxrHlwu','zgrLBJ0IDhi','psiZmdaIigy','5lU344ccpc9KAxy+','yNrUlwnVzgu','yw5JAg9Ypsi','AwXSpsjUB24','igq9iK0Znca','ncWYncaWida',
'AgfSzG','zM9UDa','57Ud5B6x5PU05B+R77Ym5l2g5PIo5BM0','BJ7MRRxNLj/MTQ88l3m','DgXL','Dc1SiJ7LKiJLKiW8','iowYGE+8JowZSowaVca',
'pc9SAw5Lyxi','Awr0Ad0Inci','6lwW6l+z5lU25lQl5BEY57Up5A6A5lQg','yxnZpsjLDI0','yxnZpsjZzwC','5Asw5lIK5lIQ6zsU55sO5lIn5lQg',
'iK03mcWZmca','6k+g5AsA5Bcr5lQ6','BMfTzq','ihnLBgvJDgu','otiIigzPBgW','iZrHnwe1ma','5P+C5PIV56M655Qepc9KAq','5BIM6lwWia','ldeWic01lde',
'5zYO5yw7552a77YA','z2XVyMu','CgH5lwL0zw0','6ycb5yE65zU96lII','mcW1ocbdody','5RkH6lw25lIk6l+h5AsN6lwB','otiIihDPzhq','562w77Ym6iQc5Awp6ycc5lIT',
'C3rHz2u','pc9WAwn0Dxi','yw1LiJ4','iJ7LPi3LIly8l2j1','tgvMDa','zMLSBd0I','u0fwrv9wrvi','5zU95yAfimk3ios6I+s7TG','AY1Uyw1LiJ4',
'mcaXmdaGmta','lweIpG','ios4LJWVyNv0Da','zw1WDhKTAge','Bg9Y','CxvLCNLtzwW','nsKIihn0CM8','zJiIlZ48Cge','AhqIpUwpS+IeMJWV',
'ywXWAgfIzxq','mcaZmdaGnde','y3jVC3m','yMfK','5PYj5lQ65OoZ6lEF5l2G562+57QM','y2fYzwvYrwe','mYiG','pJXKzwzZpJW','ios4QUI1M+wTOZWVzgK',
'pJXZDg9Wig8','qxjYyxLD','yMvZDci+','yY1HCMvHiIa','mJiYldeYocW','BMv3u3rHDgu','C2HHCMuTBMe','mJqIlZ48l2C','y2LYy2XL','CMvJDcb4psi',
'B3CGzg9Uzsa','CMvZ','zI1MzwuIpG','6zkX55wz552a','lJu1kq','Bg9HBKzYB20','y3jLyxrLt2i','AgfZAfn0CG','ihK9iJeZnci','psjHzs10Axq',
'ywjYB2fKswq','iJu4iIbMB24','lc40mIK','iK0XmdaSmJy','77Ym5PYa57Ug5PYa6zw/','mtqSocaZlc0','ysiGC3rYB2S','yxjKlxrYBYi','DcbzyuHLAsW',
'Et0IndiIihC','Bwf0y2G','lNDLyNaIihq','B3CGDgWTy28','zw5K','mcW1mcbSmte','mteYiIb5psi','phjLy3qGD2K','Dw0TAgvYBY0','5zci5zcm5yIW5PYF77Ym5l+X5lMq6yoO',
'y2XLign4psi','C2vLzci+phm','yxKIpUs7JUwKTowgJEADPq','psjPBNb1Dci','5yE65zU96kAb6iQXia','yNrUlwHLBha','5lIn562+5OIq5BM06zIF44cc5QkV6zIF',
'mZmWiI8+phm','5lIQ5lQ66lsI5A+m','nJiGqZe0osW','mZmSlteZic0','pgi+','iI4YiI8+phm','mdqIlZ4','57Ut6l+h5AMA','C3m9iNnLyxm','BwLU',
'ENv5AwjHz2e','igW3lde1ide','ios4LIdcTYa','C3r5Bgu9iNC','C2vNlw5VDgu','6zMi5yIxpc9OmJ48','CIiSicjqAw4','imk3igjPBgLI','osWXmIbA',
'lw9WywnPDhK','5lIa5lIw6l+y5AsF5yIW5lQgia','CfbHDgG+pgW','w2rHDgeTzM8','DgLLCG','BM9UzsiGyxi','mcWZncbdmtu','Dci+','C3m9iMXLz2e',
'ltmIihn0CM8','6l+z5lIQ5Rwp6kEi5zMO5A2y5lIn5lQg','AsbJBgfZCZ0','C3rHy2SIpJW','CgLJA1jPDMe','mIiGCJ0IlJy','y2XPy2S','As1IywrNzsa',
'Et0Imti0iIa','ys1Hy3q9iNm','DJGGsdu0ihO','AwnRlwXPC3q','5zU+54Mh5BEY5Asn5yI277Ym5y+V5lUL','C29YDa','ign5psi0nIi','tteWmcWXmca',
'ttqWlde2ieG','z3jLC3m','Dw5ZAgLMDa','C2HHzg93qMW','B25LCNjVCG','B3jKzxi9iNm','Ae5HBwu','ldmWiemZnIW','mtyIigHLAwC','zMLSBd0IDxi',
'C3rLCc10Axq','D2fNzq','mJaGocWTmty','6l2S5lYA56Qx','ltuSlte1ihO','mtiGmtySlti','zM10tw9UzxK','yY1OzwfKiJ4','zt48l3jLy3q',
'lwXLywqIpG','zgvMCZ48zYa','qtG4ldG4ida','zMLSBa','zgL2pJXKAxy','iI8+pc9SAw4','psjZDw0TAgu','5zU+54Mh5BEY5l+D5A2y','psjVDNjJlw0',
'mJqIihDPzhq','zw5KiJ48Asa','ksiVpJXWyxq','Bd0Ii2y0zJy','C3rLCc1WCM8','CZ0IyMfYlxy','ksiGC3rYB2S','5lIk5y2W5zoQ5lIa5P2H57Ut5Bgapa',
'5BY65BQM5AsN','CYi+phnWyw4','iZy2nG','CMvTB3zL','Dci+5BEM6isApc9I','5P2H5lIk77Ym5zcm5PE25RUH6lAZ5yEG','msiVpJWVBgK','lxyIpG',
'lw1VDgLVBJO','yY1NCMLKiIa','CZ0IzxyTAgu','zgf0ys1VChq','C3rYAw5NAwy','5ywi5lIn6k+3','zxzLBNrjza','yM90Dg9T','mca0mdaGmZa',
'CgfYzw50tM8','Ahq9iJi0iIa','psj0ywCTCM8','iIbKyxrHlw8','z2v0sxrLBq','C2HHzg93q28','5RkH6io96lwW5yIW6ykJ5lIa5Q2Lia','psjZDw0Ty2e',
'nYiGCJ0ImY4','5QYN5RsY6yEr6z20','psiXmYiGCNK','iIbVBMvYCM8','yxiIpJXKAxy','5PYa6AUy6io95yQBia','Bgu9iMLTzYi','5OM+5lIn5zUE5P2L44cc',
'psi0mIiVpJW','zM9VDa','mciGzMLSBd0','y2uGC3jJC2u','iM5VBMuIihm','Bg9JywXOB3m','idiGlJqGmY4','igzPBgW9iM4','y2X1yG','Ed0ImcaWidi',
'ldaGDJe2igm','psjYz2jHkda','CZ0IC3vTlwm','Ad0ImtyIigG','y2XHC3m9iNa','lMPLCNnLEs0','C3m9iM92CMm','6lAZ5lIa5OQklEEuN+A2R+AOOEAlNW',
'C3m9iNiIpG','Bw9Kzq','y29WEq','yY0XocWToca','CZ0IyMfYlxq','DMfSDwu','nZK3oda0t3jrsKD1','pc9ZDMC+','iJiWmciGEti','5lMF5y+V5lUL5lIn5OcL552a6lwW77Ym',
'yMfUr2fTzxm','y2fWpsjYB3u','y2X1yI1Iyxi','w2rHDgeTB3i','CMvHBs1UB24','y3v0','lc41kq','psjMB3jTlxi','iJyIihbSywm','Ed0I','B3jPz2LUswq',
'nsiGCgfPBNq','tgLZDc5Hzgq','5Qoa5Rwl5yIW5lIk5QYH55Qe5A2y5QgJ','yxjKlwXNiJ4','C3m9iNiGAgK','iJKYiIb5psi','lNrSlxjVDY4','B3i9iImWodi',
'DhrVBJ48l2q','iowCUIi+','ogmTlJmGmI4','BIi+56YSia','Dd0ImtaIihi','lJGP','C3m9iNn0lwm','lJyIlZ48y2K','twLKwu1HEca','6ycj5OUP5l+X5lMq6yoO4OcM',
'w2rHDgeTzhi','55Qe55s16k+D5y2k5BM05RkH5zon6l+h','B3CIpJXKAxy','CMfUAW','lJqGmJbSmI4','5PYa6AUy6lQR5lU3','6lAZ5y2p5P2V5yAG5yAB',
'zgf0ys1WB3m','iMfYyY1Iywm','mtjim3OIigy','5OYj56En5A2qia','yMfZzq','CMvZDwX0','CNmIpJXKAxy','5OIq5BM06zIF5lQg44cc','zw5KAw5NvMK',
'pJXJAxjJBgu','C3m9iNrSlw4','nsiGCNG9iJi','mte0ldeWnIa','jsi+pc9KAxy','C3m9iM5HCNi','DgeTyxjJpsi','vgvHBq','ChjVDg90Exa','6ykJ5lQB6yo95PIV5zco5P2L55Qe5lQl',
'iMzPzwXKlwW','BNvTyMvY','psiXmJyIihC','zguTEhmIpG','yMfJA0zYB20','DgGGzd0Itte','BMfTzsiGBwe','BMCTDgL0Bgu','EMu9iJKUnsi',
'w2rHDgeTB3a','z2v0tw9UDgG','y2HVAwnLCW','B3G9iJaGmca','imk3ioMBTUwWGsa','C25HCa','yI1Uyw1Llwi','AgLKzgvU','zgvZyW','zNjVBq',
'l3nWyw4+phm','pgCGzMLSBd0'];
_gam_0a=function(){return gW;
};
return _gam_0a();
}