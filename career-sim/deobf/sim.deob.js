...
return bF["length"]?ah(bF,function(bG){var cR=cP,bH=bG["weight"]||0x28;
return a2["usedEven"+'ts'][bG['id']]&&(bH*=0.45),bG['cn']&&bB&&(bH*=0.25),bG['cn']&&!bB&&(bH*=1.15),bH;
}):null;
...
!0x0;
}function aW(){var d8=cn,bx=a2["bigQ"][0x0],by=ad()<0.42?ad()<0.78?0x1:0x2:0x0,bz=ad()<0.44?ad()<0.8?0x1:0x2:0x0;
...
}function aX(bx,by,bz){var d9=cn,bA=aR[bx["kind"]]["side"]||bx["team"],bB=[];
...
bB["push"]("中场休息。更衣室"+"里教练在白板上画"+"了一通，最后转过"+"头看着你。"),bB;
...
bF>0x0&&(bG*=Math["max"](0.16,(0x64-a2["ovr"])/0x32)),bF<0x0&&(bG=a6("fitness")?0.8:0x1),a2["ovr"]=ac(a2["ovr"]+bF*bG,0x14,0x63),
...
a2["totals"]['ga']+=bz['ga'];
var bQ=aJ(bx,by,a2["ovr"]),bR=Math["round"](bQ*(a0["ROLES"][a2["role"]]["rank"]>=0x2?0x1:0.55));
...
if(!a2["banned"]&&a2["age"]>=0x12&&bx&&by){var bY=ac((a2["ovr"]-bW)/0xc,0x0,0x1),bZ=a0["ROLES"][a2["role"]]["rank"],c0=ac(bY*(bZ>=0x3?0x1:bZ>=0x2?0.6:0.2)*(by["rep"]>=0x4?1.15:by['cn']?0x1:by["rep"]>=0x2?0.8:0.35)*(a2["age"]>=0x1e?0.8:0x1),0x0,0.9);
bX=a2["cheat"]||ad()<c0;
}if(bX){var c1=ae(0x2,0x5),c2=aL(),c3=a2["seasons"]["length"]%0x4;
if(0x1!==c3&&0x3!==c3||(c1+=0x2),a2["caps"]+=c1,bz["caps"]=c1,a2["flags"]["_natCall"+'ed']||(a2["flags"]["_natCall"+'ed']=!0x0,b1("nat_firs"+"tcall")),
...
ch&&(a2["natStats"]["goals"]||b1("nat_firs"+"tgoal"),c9["natGoals"]=ch,a2["natStats"]["goals"]+=ch),ci&&(c9["natAssis"+'ts']=ci,
...
by["rep"]>=0x4&&a2["ovr"]>=0x55&&(!function(c9){var di=dd;
for(var ca=0x0;
ca<a2["awards"]["length"];
ca++)if(a2["awards"][ca]["name"]===c9)return!0x0;
return!0x1;
}(a0["AWARDS"]["ballon"])||ad()<0.35)&&b5(a0["AWARDS"]["ballon"]),a2["ovr"]>=0x54&&ad()<0.35&&b5(a0["AWARDS"]["afcpoy"]),by["rep"]>=0x4&&"att"===c6&&a2["ovr"]>=0x55&&ad()<0.45&&b5(a0["AWARDS"]["boot"]),
by["rep"]>=0x3&&'gk'===c6&&a2["ovr"]>=0x55&&ad()<0.45&&b5(a0["AWARDS"]["glove"]);
...
if(c9t>0x0){by["rep"]>=0x4&&a2["ovr"]>=0x55&&ad()<c8*(1+0.3*c9t)&&b5(a0["AWARDS"]["ballon"]),by["rep"]>=0x4&&"att"===c7&&bz["goals"]>=0x23&&ad()<0.8&&b5(a0["AWARDS"]["boot"]),
by['cn']&&a2["ovr"]>=0x4a&&ad()<0.3*(1+0.3*c9t)&&b5(a0["AWARDS"]["cslmvp"]),by['cn']&&"att"===c7&&bz["goals"]>=0x19&&ad()<0.45&&b5(a0["AWARDS"]["cslboot"]),
'gk'===c7&&by["rep"]>=0x3&&bz['cs']>=0x14&&ad()<0.3*(1+0.3*c9t)&&b5(a0["AWARDS"]["glove"]),!a2["banned"]&&a2["ovr"]>=0x54&&ad()<(0.03+0.18*aL())*(1+0.3*c9t)&&b5(a0["AWARDS"]["afcpoy"]);
...
}function be(){var dx=cn;
return a2["age"]>=0x22?0x1:a2["age"]>=0x1e?ae(0x1,0x3):ae(0x2,0x4);
...
bA>0x0&&(bB*=ac((0x41-a2["ovr"])/0x1c,0.35,0x1)),a2["ovr"]=ac(a2["ovr"]+bA*bB,0xc,0x63),a2["maxOvr"]=Math["max"](a2["maxOvr"],
a2["ovr"]);
...
}(a2["teamId"]&&(a2["capDone"]||[]).indexOf(a2["teamId"])<0x0&&a2["seasonsA"+"tClub"]>=0x2&&a0["ROLES"][a2["role"]]["rank"]>=0x3&&a2["ovr"]>=0x32+0x4*((ar()||{})["rep"]||0x0)&&"prime"===aB(a2["age"]))&&(a2["forceQ"]||(a2["forceQ"]=[]),
...
}))["length"])return ag(bA),void(a2["pending"]={'type':"transfer",'fired':!0x1,'offers':bA["slice"](0x0,0x3)["map"](function(bG){return bG['id'];
}),'canStay':!au()&&!!ar()&&(ar()["rep"]>=bB||aq(ar())["rep"]>=0x4),'canRetire':a2["age"]>=0x1e});
}var bC=bf(a6("analyst")?0x6:0x4),bD=ar();
bx&&!by&&b9(bD)&&(bx=!0x1,a2["lowSpell"]=0x0);
...
return/欧冠|世界杯|世俱杯|亚冠/["test"](bL["name"]);
...
},'newState':function(bx,by,bz,bA){return a2=function(bB,bC,bD,bE){var en=_sim_0b,bF=bC["origin"],bG=az(bE),cK=null;
...
'guanxi':ac(0x1e+bF["guanxi"]+(bG?bG["guanxi"]:0x0),0x0,0x64),'clean':0x50,'fame':0x5,'money':bF["money"]+(bG?bG["money"]:0x0),
'seasonWage':0x0,'peakAnnualWage':0x0,'careerEarnings':0x0,'teamId':null,'role':"sub",'roleAdjust':0x0,'seasonsAtClub':0x0,
...
'life':{'partner':null,'married':0x0,'kids':[],'splits':0x0},'flags':{},'staff':{},'pending':null,'usedEvents':{},'choices':[],
'rid':null};
}(bx,by,bz,bA),a2;
},'nextStep':bk,'doPeriod':bl,'choose':function(bx){var eo=cn,by,bz,bA=a2["pending"];
if(!bA)return!0x1;
if("random"===bA["type"]){var bB=bs(bx);
return null===bB?(bk(),!0x0):!!bB&&(bw(bB["res"]),!0x0);
...
if("stay"===bx)return bv(bx),a2["contract"+"Left"]=be(),a2["pending"]=null,bk(),!0x0;
...
return!!bF&&(bv(bx),b8(bF,!0x0),a2["pending"]=null,bk(),!0x0);
...
c0["push"]({'cls':'up','text':"关系+"+bJ["mood"]})),a2["pending"]["result"]={'won':bM,'log':bP,'deltas':c0,'score':[bN,bO],'pens':bS},
a2["bigQ"]=[],!0x0;
...
if(!by)return null;
var bz=aq(by),bA=aI(by);
return{'wage':Math["round"](aJ(by,bz,a2["ovr"])),'role':bA,'roleName':a0["ROLES"][bA]["name"]};
