...
/* 统一「旗帜/队徽+队名」行内格式：世界面板赛程/签表/小组赛/赛程 chips 共用（样式 .cr-tm）。
   国家队 id 带 n_ 前缀与俱乐部 id 隔离；国旗用本地 SVG（assets/flags/，由
...
if(nm){var _fs=nm["img"]?((window["_FLAG_DATA"]&&window["_FLAG_DATA"][nm["img"]])||'assets/flags/'+nm["img"]+".svg"):null;
return'<span class="cr-tm">'+(_fs?'<img class="cr-flag" src="'+_fs+'" alt="">':nm["f"]+' ')+ax(nm["n"])+'</span>';}
...
var lab=sn<0?(sn+100)+'岁':(isCur?'最新完赛':'第'+sn+'季');
...
if(!all||!all["length"])return'<div class="wl-empty">本会话内暂无当季签表</div>';
var h='<div class="wl-bracket">';
...
[["lg","联赛"],["cup","杯赛"],["cont","洲际"],["nat","国家队"]]["forEach"](function(t2){
h+='<button class="wl-chip wl-tab2'+(_wl["tab"]===t2[0]?" on":"")+'" data-wld="tab:'+t2[0]+'">'+t2[1]+'</button>';
});
h+='</div>';
if(_wl["tab"]==='lg'){
var ls=window["SIM"]["world"]({'q':'leagues'})["leagues"];
h+='<div class="wl-sub">';
...
if(!_wl["cup"]&&cs["length"])_wl["cup"]=cs[0]["name"];
h+='<div class="wl-sub">';
...
var _fg=(_f["homeId"]==="n_chn"||_f["hid"]==="n_chn")?_f["hg"]:_f["ag"];
var _fa=(_f["homeId"]==="n_chn"||_f["hid"]==="n_chn")?_f["ag"]:_f["hg"];
...
if(!a6["youthInvest"](_yk,_yc)){e["target"]["checked"]=!0x1;_yc&&bK("家里拿不出这笔钱");}
return aA(),ar();
}
...
if(!el)return;
var v=el["getAttribute"]("data-wld"),i2=v["indexOf"](':');
if(i2<0)return;
...
if(!bW){var bZ=ai(),c0=au["pending"]&&("academy"===au["pending"]["type"]||"transfer"===au["pending"]["type"]||"retire_f"+"orced"===au["pending"]["type"]),
...
+'<span class="tl-badges">'+(cClubs["map"](function(c5){return '<span class="mini-badge lnk" data-age="'+c2["age"]+'" data-view="club" title="查看'+c2["age"]+'岁赛季战报">'+ax(c5)+'</span>';}))["join"]('')+'</span>'
+'<span class="tl-badges">'+(cNats["map"](function(c5){return '<span class="mini-badge nat lnk" data-age="'+c2["age"]+'" data-view="nat" title="查看'+c2["age"]+'岁国家队战报">'+ax(c5)+'</span>';}))["join"]('')+'</span>'
...
'ord':null},bd=/^\s*(\d+)\s*%\s*([^\/]+?)\s*\/\s*(\d+)\s*%\s*(.+?)\s*$/;
function be(){
...
bX,bY,bZ,c0){var c1=a6["leagueOf"+"Team"](bX),c2=["保级队","中下游",'中游','争冠','豪门',"顶级豪门"][bX["rep"]],c3=c0?a6["offerBri"+'ef'](bX['id']):null;
...
"csl"],[/足协杯/,"cn-facup"],[/世俱杯/,"cwc"],[/金球/,"ballon"],[/金靴/,"boot"],[/金手套/,"glove"],[/最佳球员/,"best"],[/国王杯/,"kingcup"],
...
'label':"世界杯冠军",'rank':0x1},{'re':/^金球奖$/,'label':"金球奖",'rank':0x2},{'re':/^欧冠冠军$/,'label':"欧冠冠军",
'rank':0x3},{'re':/^亚洲杯冠军$/,



'label':"亚洲杯冠军",'rank':0x4},{'re':/^亚洲足球先生$/,'label':"亚洲足球先生",'rank':0x5},{'re':/^欧洲金靴$/,'label':"欧洲金靴",
'rank':0x6},{'re':/^金手套$/,



'label':"金手套",'rank':0x6},{'re':/^(英超|西甲|意甲|德甲|法甲)冠军$/,'label':"联赛冠军",'rank':0x7},{'re':/^欧联冠军$/,
'label':"欧联冠军",'rank':0x8},



{'re':/^世俱杯冠军$/,'label':"世俱杯冠军",'rank':0x9},{'re':/^中超最佳球员$/,'label':"中超最佳球员",'rank':0xa},{'re':/^中超冠军$/,
'label':"中超冠军",


'rank':0xb},
{'re':/^足协杯冠军$/,'label':"足协杯冠军",'rank':0xc},{'re':/英联杯冠军$/,'label':"英联杯冠军",'rank':0xd},
{'re':/^(社区盾|.*超级杯)冠军$/,
...
(function(){var bW=aG(),bX=aH(bW),bY="<div cla"+"ss=\"sect"+"ion-head"+"\"><h2>生涯"+"历史档案</h2"+"><span>"+(bW["length"]?bW["length"]+'\x20段':'')+("</span><"+"/div>");
...
bX,bY,bZ,c0,c1){
c1=Math["min"](c1,bZ/0x2,c0/0x2),bW["beginPat"+'h'](),bW["moveTo"](bX+c1,bY),bW["arcTo"](bX+bZ,bY,bX+bZ,
...
return{'name':au["name"],'gen':au["gen"]||0x1,'number':au["number"],'posId':ah(au["pos"])['id'],



'posName':ah(au["pos"])["name"],
'group':ah(au["pos"])["group"],'originName':ak(au["originId"])["name"],'maxOvr':Math["round"](au["maxOvr"]),



'age':au["age"],
'seasons':au["seasons"]["length"],'clubCount':au["clubsPla"+"yed"]["length"],'apps':au["totals"]["apps"],



'goals':au["totals"]["goals"],
'assists':au["totals"]["assists"],'cs':au["totals"]['cs'],'ga':au["totals"]['ga'],'caps':au["caps"],



'money':au["money"],'spells':bl()["map"](function(bW){
return{'teamId':bW["teamId"],'color':bW["color"]};
}),'honours':bk(),



'nat':ap(),'endingTitle':a0["endingVi"+'ew'](bm(au["ending"]),ah(au["pos"])["group"])["title"],'endingChoices':bD(au["endingsA"+'ll'],
...
var bY=(bW=bW||bG||bC())["group"],



bZ=bW["spells"],c0=bW["honours"],c1=bZ["map"](function(c2){var c3=ag(c2["teamId"]),c4=c3&&window["CREST_UR"+'LS']&&window["CREST_UR"+'LS'][c3['id']];
...
return bY&&(bY["textCont"+"ent"]=(bG["gen"]||0x1)>0x1?'第\x20'+bG["gen"]+(" 世 · 生涯结"+'束'):"生涯结束"),



function(bZ){var c0=aw("share-ne"+"xtlife");
...
bI(bG),aw("share-mo"+"dal")["classLis"+'t']["remove"]("hidden"),bE(bG);
}function bI(bW){var bX=aw("share-en"+"ding-pic"+'k');
if(bX){var bY=bW&&bW["endingCh"+"oices"]||[];
bX["classLis"+'t']["toggle"]("hidden",bY["length"]<0x2),



bY["length"]<0x2||(bX["innerHTM"+'L']="<div cla"+"ss=\"sep-"+"label\">卡"+"上印哪一条结局<"+"/div><di"+"v class="+"\"ep-list"+'\x22>'+bY["map"](function(bZ,c0){
...
bX?"荣誉："+bX:"荣誉：一个没有","结局："+bW["endingTi"+"tle"],a3+(" · bilib"+"ili 神来我也"+'日')]["join"]('\x0a');
}function bK(bW){var bX=document["createEl"+"ement"]("div");
bX["classNam"+'e']="toast",



bX["textCont"+"ent"]=bW,document["body"]["appendCh"+"ild"](bX),setTimeout(function(){
bX["remove"]();
},0x708);
}var bL=null;
function bM(bW){
...
"view-ide"+"ntity","view-car"+"eer","view-sum"+"mary","view-arc"+"hive","view-cod"+'ex']["forEach"](function(bY){
...
aw("actionba"+'r')["classLis"+'t']["toggle"]("hidden",



!bX),document["body"]["classLis"+'t']["toggle"]("no-actio"+"nbar",!bX),
document["body"]["classLis"+'t']["toggle"]("in-caree"+'r',



"view-car"+"eer"===bW),aw("app")["classLis"+'t']["toggle"]("wide",
"view-car"+"eer"===bW||"view-sum"+"mary"===bW||"view-cod"+'ex'===bW),



aw("app")["classLis"+'t']["toggle"]("intro-wi"+'de',"view-int"+'ro'===bW),
...
_mkt=_av;
return _av["length"];
})()?_mkt["map"](function(c4){var cf=a6["staffById"](c4);if(!cf)return'';
...
var crole=ce["role"]&&a0["ROLES"][ce["role"]]?a0["ROLES"][ce["role"]]["name"]:'';
/* 定位本季与上季记录（对比用） */
var _idx=-1;(au["seasons"]||[]).forEach(function(s,i){if(s["age"]===ce["age"])_idx=i;});
var _prev=_idx>0?au["seasons"][_idx-0x1]:null;
var _tA=0,_tG=0,_sn=au["seasons"]||[];
for(var _ti=0;_ti<=_idx&&_ti<_sn.length;_ti++){_tA+=(_sn[_ti]["apps"]||0);_tG+=(_sn[_ti]["goals"]||0);}
var _pA=_tA-(ce["apps"]||0),_pG=_tG-(ce["goals"]||0);
/* 联赛 tile */
var cgpCls=1===ce["leaguePos"]?" won":null!=ce["leaguePos"]&&ce["leaguePos"]>=0x10?" lost":'';
var _lgTile='';if(ce["leaguePos"]){_lgTile='<span class="sb-pos'+(cgpCls?' '+cgpCls:'')+'">'+ax(ce["league"]||'')+' 第'+ce["leaguePos"]+'名</span>'+(null!=ce["leagueW"]?'<span class="sb-rec">'+ce["leagueW"]+'-'+ce["leagueD"]+'-'+ce["leagueL"]+' · '+ce["leaguePts"]+'分</span>':'');}
/* 杯赛/洲际拆分 */
var _cupN={'欧冠':1,'欧联':1,'欧协联':1,'亚冠':1,'解放者杯':1,'南美杯':1,'世俱杯':1,'中北美冠军杯':1};
var _cups='',_conts='';
if(au["cupRuns"])for(var _ci2=0;_ci2<au["cupRuns"]["length"];_ci2++){var _c2=au["cupRuns"][_ci2];if(_c2["age"]!==ce["age"])continue;var _cr=_c2["result"]||'';var _ctag='<span class="sb-cup'+(0===_cr.indexOf('冠军')?" win":_cr?"":" dim")+'">'+ax(_c2["comp"])+(_cr?' '+ax(_cr):'')+'</span>';if(_cupN[_c2["comp"]])_conts+=_ctag;else _cups+=_ctag;}
/* 国家队 tile */
var _nat='';
if(ce["caps"]){_nat='<span class="sb-nat-line">'+'国家队 '+ce["caps"]+' 场'+(_gk?(ce["natCs"]?' · '+ce["natCs"]+' 零封':''):((ce["natGoals"]?' · '+ce["natGoals"]+' 球':'')+(ce["natAssists"]?' · '+ce["natAssists"]+' 助':'')))+'</span>'+(ce["nat"]?'<span class="sb-cup">'+ax(ce["nat"])+'</span>':'');}
/* 数据对比行 */
function _cmp(lab,cur,prv){if(cur==null)return'';var d=(prv!=null&&prv!=null)?cur-prv:null;var arr=d==null?'':(d>0?' <b class="sb-up">↑'+d+'</b>':d<0?' <b class="sb-dn">↓'+(-d)+'</b>':'');return'<span class="sb-cmp">'+lab+' '+cur+arr+'</span>';}
var _cmpRow=_gk?_cmp('出场',ce["apps"],_prev&&_prev["apps"])+_cmp('零封',ce["cs"],_prev&&_prev["cs"])+_cmp('丢球',ce["ga"],_prev&&_prev["ga"]):_cmp('出场',ce["apps"],_prev&&_prev["apps"])+_cmp('进球',ce["goals"],_prev&&_prev["goals"])+_cmp('助攻',ce["assists"],_prev&&_prev["assists"]);
try{var _vNow=ce["ovrEnd"]!=null?an(ce["ovrEnd"],ce["age"]+1):null,_vPrev=(_prev&&_prev["ovrEnd"]!=null)?an(_prev["ovrEnd"],_prev["age"]+1):null;
if(_vNow!=null){var _vArr=(_vPrev!=null&&_vNow!==_vPrev)?(_vNow>_vPrev?' <b class="sb-up">↑</b>':' <b class="sb-dn">↓</b>'):'';
...
if(tp.length){_pick('tp',['捧起{TP}','{TP}到手，奖杯柜+1','把{TP}带回家']);return;}
...
if(_dOvr>=3){_pick('oj',['能力暴涨（{D}）','身体里换了台发动机','一日千里，能力+{D}']);return;}
...
var _awC='';if(ce["awardN"]&&ce["awardN"]["length"])_awC=ce["awardN"]["map"](function(_t){return'<span class="sb-tro aw">🥇 '+ax(_t)+'</span>';})["join"]('');
var ch=(ce["note"]?"<span cl"+"ass=\"mi"+"ni-badge"+" bad\">"+ax(ce["note"])+"</span>":'')+(ce["nat"]?"<span cl"+"ass=\"mi"+"ni-badge"+" nat\">"+ax(ce["nat"])+"</span>":'')+(ce["move"]?"<span cl"+"ass=\"mi"+"ni-badge"+'\x20'+(0x0===ce["move"]["indexOf"]('升')?'up':"bad")+'\x22>'+ax(ce["move"])+"</span>":'')+(ce["staffGon"+'e']?"<span cl"+"ass=\"mi"+"ni-badge"+" bad\">"+ax(ce["staffGon"+'e'])+("走了</span"+'>'):'');
var dOvr=(ce["ovrEnd"]!=null&&ce["ovr"]!=null)?ce["ovrEnd"]-Math.round(ce["ovr"]):null;
return '<div class="sb-card">'
+'<div class="sb-hd"><span class="age-chip">'+ce["age"]+'</span>'
+(ce["teamId"]&&ag(ce["teamId"])?aT(ag(ce["teamId"])):'')
...
+(_conts?'<div class="sb-tile"><div class="sb-tile-t">洲际</div>'+_conts+'</div>':'')
+(_nat?'<div class="sb-tile"><div class="sb-tile-t">国家队</div>'+_nat+'</div>':'')
+'</div>':'')
+'<div class="sb-cmps">'+_cmpRow+'</div>'
+((ct||_awC||ch)?'<div class="sb-badges">'+ct+_awC+ch+'</div>':'')
+'</div>';
})["join"]('')+"</div>"+bb(["<button "+"class=\"b"+"tn btn-p"+"rimary\" "+"data-act"+"=\"contin"+"ue\">继续</"+"button>"])+"</div>";
}if("youth_pa"+'th'===bZ["type"])return "<div cla"+"ss=\"even"+"t\">"+b1('🧒',"十二岁","去哪儿练",!0x0)+("<div cla"+"ss=\"ev-d"+"esc\">同龄的"+"孩子里，你踢得算"+"好的那几个。接下"+"来几年在哪家青训"+"营，家里得拿个主"+"意 —— 好队伍"+"练得快，也刷得狠"+"。出国都要学费，"+"贵多少看队伍档"+"次。</div>")+bb(bZ["offers"]["map"](function(ce,cf){var cg=ag(ce),ch=a6["leagueOf"+"Team"](cg),ci=!ch['cn'];
...
if(au)return "summary"===au["phase"]?(bM("view-sum"+"mary"),



void(function(){var bW=aq(),bX=ah(au["pos"])["group"],bY=a0["endingVi"+'ew'](bm(au["ending"]),bX);
...
'list':bY,'count':bY["length"]};
}function cBch(cf){var cg=[];
for(var ch in cf["bonus"])Object["prototype"]["hasOwnProperty"]["call"](cf["bonus"],



ch)&&cg["push"](function(ch){var v=cf["bonus"][ch];
...
c2,c3,c4=(c2=(c1=aw("in-seed"))&&null!=c1["value"]?String(c1["value"])["trim"]():'')?/^\d{1,15}$/["test"](c2)?Number(c2):c2["slice"](0x0,
...
bK("按种子 "+c4+" 开局");
}(au=a6["newState"](bY,bZ,c5,c0,(aw("ach-boost")&&aw("ach-boost")["checked"]?cAch()["bonus"]:null)))["cheat"]||(c3=bZ)&&az("ident",



{'name':c3["name"],'number':c3["number"],'foot':c3["foot"],
'pos':c3["pos"]}),au["maxOvr"]=au["ovr"],au["rid"]=null,aA(),
...
}var sn=['李','王','张','刘','陈','杨','赵','黄','周','吴','徐','孙','胡','朱','高','林','何','郭','马','罗','郑','冯','蒋','沈','韩','秦','许','吕','施','孔','曹','严','华','金','魏','陶','姜','谢','邹','苏','潘','葛','范','彭','鲁','韦','方','任','袁','柳','鲍','史','唐','费','薛','雷','贺','倪','汤','滕','殷','郝','安','常','于','傅','齐','康','余','顾','孟','萧','尹','姚','邵','汪','毛','宋','熊','舒','祝','董','梁','杜','阮','蓝','季','贾','江','童','颜','梅','盛','钟','邱','骆','夏','蔡','田','樊','霍','万','卢','莫','房','丁','邓','单','洪','包','石','崔','龚','程','邢','裴','陆','荣','翁','甄','曲','段','焦','侯','全','秋','伊','宁','甘','武','叶','黎','白','蒲','赖','卓','屠','池','乔','谭','姬','申','冉','雍','桑','桂','牛','边','燕','尚','温','庄','晏','柴','阎','慕','连','习','艾','向','古','易','廖','文','欧','师','聂','冷','简','饶','曾','关','查','游'],gn=['伟','磊','军','洋','勇','明','强','杰','涛','超','斌','刚','平','辉','鹏','华','飞','凯','健','龙','峰','亮','宁','宇','航','晨','阳','浩','然','博','轩','豪','睿','泽','翔','毅','威','佳','嘉','家','志','建','国','俊','文','帅','鑫','坤','旭','岩'],gn2=['子轩','浩然','宇航','思远','俊杰','家豪','志强','建华','建国','国强','志明','文博','天宇','泽宇','明轩','一鸣','佳明','子豪','家俊','逸飞','浩宇','俊熙','晨阳','天佑','嘉豪','志远','志豪','立伟','国庆','小龙','志刚','文轩','明辉','俊豪','子墨','皓宇','雨泽','宇泽','泽楷','梓豪'];var _sn=sn[Math["floor"](Math["random"]()*sn["length"])];var _given=Math["random"]()<0.4?gn2[Math["floor"](Math["random"]()*gn2["length"])]:gn[Math["floor"](Math["random"]()*gn["length"])];return{'name':_sn+_given,'number':0x1+Math["floor"](Math["random"]()*0x63),'pos':pp,'foot':Math["random"]()<0.5?'left':'right'};
}
function bU(){var bW,
...
if("string"==typeof bW["name"]&&bW["name"]["trim"]())bs={'name':bW["name"]["trim"]()["slice"](0x0,0x6),



'number':ad(parseInt(bW["number"],0xa)||0xa,0x1,0x63),
...
'number':bs["number"],'foot':"left"===bW["foot"]?"left":"right",'dreamId':null,'pos':bs["pos"],'origin':a9[0x0]},
aX=0x0,
...
document["addEvent"+"Listener"]("DOMConte"+"ntLoaded",
...
if(_tr["ok"]){aA(),ar();}
return!0x1;}
if(_ta[0x0]==="hire"){_tn=a6["teamHire"](_ta[0x1]),_tn?bK("签下了 "+_tn):bK("现在签不了这个");}
else if(_ta[0x0]==="fire"){if(!confirm("辞退要付一赛季的违约金，确定？"))return!0x1;_tn=a6["teamFire"](_ta[0x1]),_tn&&bK(_tn+" 走了");}
return _tn?(aA(),ar(),!0x1):!0x1;}
...
h+="<div class=\"sm-league-row\"><span class=\"sm-league-rank\">"+(f0["leaguePos"]?"#"+f0["leaguePos"]:"—")+"</span><span class=\"sm-league-name\">"+ax(f0["league"]||"\u65e0")+"</span>"+(f0["leagueId"]?"<button class=\"sm-link\" data-wld=\"open:"+f0["leagueId"]+"\">积分榜 · 赛程</button>":"")+"</div>";
...
if(cup["group"]){var _fs=cup["group"]["fullStandings"];if(_fs&&_fs["length"]){h+=_grpBox("\u5c0f\u7ec4\u8d5b\u79ef\u5206",_fs,cup["group"]["matches"]||[],(cup["group"]["pos"]||1)-1,_fs[(cup["group"]["pos"]||1)-1]["i"]);}else if(cup["group"]["standings"])h+="<div class=\"sm-rounds\"><span class=\"sm-round\">\u5c0f\u7ec4\u7b2c"+cup["group"]["pos"]+"\u540d \u00b7 "+(cup["group"]["standings"]||[]).map(function(s){return ax(s);})["join"](" / ")+"</span></div>";}
...
if(tn["standings"]&&tn["standings"]["length"]){var _tp=-1;for(var _tp2=0;_tp2<tn["standings"]["length"];_tp2++)if(tn["standings"][_tp2]["i"]==="n_chn"){_tp=_tp2;break;}h+=_grpBox("\u5c0f\u7ec4\u8d5b\u79ef\u5206",tn["standings"],tn["matches"]||[],_tp,"n_chn");}
...
au["eventLog"]||(au["eventLog"]=[]),(au["seasons"]||[])["forEach"](function(c2){
...
ay("pid",null)||ay("save",null)||ay("mode",
null)?ay("news_ver",'')!==a4&&(aw("news-mod"+'al')["classLis"+'t']["remove"]("hidden"),



az("news_ver",a4)):az("news_ver",a4);
}),window["__SIMTES"+'T']={'origins':a9,'modes':a8,'start':function(bW,bX,bY){
return b2=null,



(au=a6["newState"](bW,bX,bY))["maxOvr"]=au["ovr"],as(),au;
},'state':function(){return au;
},'startOnline':function(bW,
bX,
